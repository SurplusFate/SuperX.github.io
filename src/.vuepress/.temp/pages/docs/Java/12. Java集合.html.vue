<template><div><h1 id="_12-java-集合" tabindex="-1"><a class="header-anchor" href="#_12-java-集合" aria-hidden="true">#</a> 12. Java 集合</h1>
<p>Java 集合类是一种特别有用的工具类，可以用于存储数量不等的多个对象，并可以实现常用的数据结构，如栈、队列等。除此之外，Java 集合还可用于保存具有映射关系的关联数组。Java 集合大致可分为 <code v-pre>Set</code>、<code v-pre>List</code> 和 <code v-pre>Map</code> 三种体系，其中 <code v-pre>Set</code> 代表无序、不可重复的集合；<code v-pre>List</code> 代表有序、重复的集合；而 <code v-pre>Map</code> 则代表具有映射关系的集合。从 Java 5 以后，Java 又增加了 <code v-pre>Queue</code> 体系集合，代表一种队列集合实现。</p>
<p>Java 集合就像一种容器，我们可以把多个对象（实际上是对象的引用，但习惯上都称对象）“丢进”该容器中。在 Java 5 之前，Java 集合会丢失容器中所有对象的数据类型，把所有对象都当成 <code v-pre>Object</code> 类型处理；从 Java 5 增加了泛型以后，Java 集合可以记住容器中对象的数据类型，从而可以编写出更简洁、健壮的代码。</p>
<h2 id="java-集合概述" tabindex="-1"><a class="header-anchor" href="#java-集合概述" aria-hidden="true">#</a> Java 集合概述</h2>
<p>在编程时，常常需要集中存放多个数据，我们可以使用数组来保存多个对象，但数组长度不可变化，一旦在初始化数组时指定了数组长度，这个数组长度就是不可变的，如果需要保存数量变化的数据，数组就有点无能为力了；而且数组无法保存具有映射关系的数据，如成绩表：语文—79，数学—80，这种数据看上去像两个数组，但这两个数组的元素之间有一定的关联关系。</p>
<p>为了保存数量不确定的数据，以及保存具有映射关系的数据（也被称为关联数组），Java 提供了集合类。集合类主要负责保存、盛装其他数据，因此集合类也被称为容器类。所有的集合类都位于 <code v-pre>java.util</code> 包下，后来为了处理多线程环境下的并发安全问题，Java 5 还在 <code v-pre>java.util.concurrent</code> 包下提供了一些多线程支持的集合类。</p>
<p>集合类和数组不一样，数组元素既可以是基本类型的值，也可以是对象（实际上保存的是对象的引用变量）；而集合里只能保存对象（实际上只是保存对象的引用变量，但通常习惯上认为集合里保存的是对象）。</p>
<p>Java 的集合类主要由两个接口派生而出：<code v-pre>Collection</code> 和 <code v-pre>Map</code>，<code v-pre>Collection</code> 和 <code v-pre>Map</code> 是 Java 集合框架的根接口，这两个接口又包含了一些子接口或实现类。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208110918598.jpeg" alt="img"></p>
<p>粗线圈出的 <code v-pre>Set</code> 和 <code v-pre>List</code> 接口是 <code v-pre>Collection</code> 接口派生的两个子接口，它们分别代表了无序集合和有序集合；<code v-pre>Queue</code> 是 Java 提供的队列实现，有点类似于 <code v-pre>List</code>。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208110920840.jpeg" alt="img"></p>
<p>如图所示是 <code v-pre>Map</code> 体系的继承树，所有的 <code v-pre>Map</code> 实现类用于保存具有映射关系的数据（也就是前面介绍的关联数组）。</p>
<p>图中显示了 <code v-pre>Map</code> 接口的众多实现类，这些实现类在功能、用法上存在一定的差异，但它们都有一个功能特征：<code v-pre>Map</code> 保存的每项数据都是 <code v-pre>key-value</code> 对，也就是由 <code v-pre>key</code> 和 <code v-pre>value</code> 两个值组成。就像前面介绍的成绩单：语文-79，数学-80，每项成绩都由两个值组成，即科目名和成绩。</p>
<p>对于一张成绩表而言，科目通常不会重复，而成绩是可重复的，通常习惯根据科目来查阅成绩，而不会根据成绩来查阅科目。<code v-pre>Map</code> 与此类似，<code v-pre>Map</code> 里的 <code v-pre>key</code> 是不可重复的，<code v-pre>key</code> 用于标识集合里的每项数据，如果需要查阅 <code v-pre>Map</code> 中的数据时，总是根据 <code v-pre>Map</code> 的 <code v-pre>key</code> 来获取。</p>
<p>根据以上俩图中粗线标识的 4 个接口，我们可以把 Java 的所有集合分成三大类，其中 <code v-pre>Set</code> 集合类似于一个罐子，把一个对象添加到 <code v-pre>Set</code> 集合时，<code v-pre>Set</code> 集合无法记住添加这个元素的顺序，所以 <code v-pre>Set</code> 里的元素不能重复（否则系统无法准确识别这个元素）；<code v-pre>List</code> 集合非常像一个数组，它可以记住每次添加元素的顺序，只是 <code v-pre>List</code> 的长度可变。<code v-pre>Map</code> 集合也像一个罐子，只是它里面的每项数据都由两个值组成。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208110923600.jpeg" alt="img"></p>
<p>如果访问 <code v-pre>List</code> 集合中的元素，可以直接根据元素的索引来访问；如果访问 <code v-pre>Map</code> 集合中的元素，可以根据每项元素的 <code v-pre>key</code> 来访问其 <code v-pre>value</code>；如果访问 <code v-pre>Set</code> 集合中的元素，则只能根据元素本身来访问（这也是 <code v-pre>Set</code> 集合里元素不允许重复的原因）。</p>
<p>对于 <code v-pre>Set</code>、<code v-pre>List</code>、<code v-pre>Queue</code> 和 <code v-pre>Map</code> 四种集合，最常用的实现类分别是 <code v-pre>HashSet</code>、<code v-pre>TreeSet</code>、<code v-pre>ArrayList</code>、<code v-pre>ArrayDeque</code>、<code v-pre>LinkedList</code> 和 <code v-pre>HashMap</code>、<code v-pre>TreeMap</code> 等实现类。</p>
<h2 id="collection-和-iterator" tabindex="-1"><a class="header-anchor" href="#collection-和-iterator" aria-hidden="true">#</a> <code v-pre>Collection</code> 和 <code v-pre>Iterator</code></h2>
<p><code v-pre>Collection</code> 接口是 <code v-pre>List</code>、<code v-pre>Set</code> 和 <code v-pre>Queue</code> 接口的父接口，该接口里定义的方法既可用于操作 <code v-pre>Set</code> 集合，也可用于操作 <code v-pre>List</code> 和 <code v-pre>Queue</code> 集合。</p>
<blockquote>
<ol>
<li><code v-pre>boolean add(Object o)</code>：该方法用于向集合里添加一个元素。如果集合对象被添加操作改变了，则返回 <code v-pre>true</code>。</li>
<li><code v-pre>boolean addAll(Collection c)</code>：该方法把集合 <code v-pre>c</code> 里的所有元素添加到指定集合里。如果集合对象被添加操作改变了，则返回 <code v-pre>true</code>。</li>
<li><code v-pre>void clear()</code>：清除集合里的所有元素，将集合长度变为 0。</li>
<li><code v-pre>boolean contains(Object o)</code>：返回集合里是否包含指定元素。</li>
<li><code v-pre>boolean containsAll(Collection c)</code>：返回集合里是否包含集合 <code v-pre>c</code> 里的所有元素。</li>
<li><code v-pre>boolean isEmpty()</code>：返回集合是否为空。当集合长度为 0 时返回 <code v-pre>true</code>，否则返回 <code v-pre>false</code>。</li>
<li><code v-pre>Iterator iterator()</code>：返回一个 <code v-pre>Iterator</code> 对象，用于遍历集合里的元素。</li>
<li><code v-pre>boolean remove(Object o)</code>：删除集合中的指定元素 <code v-pre>o</code>，当集合中包含了一个或多个元素 <code v-pre>o</code> 时，这些元素将被删除，该方法将返回 <code v-pre>true</code>。</li>
<li><code v-pre>boolean removeAll(Collection c)</code>：从集合中删除集合 <code v-pre>c</code> 里包含的所有元素（相当于用调用该方法的集合减集合 <code v-pre>c</code>），如果删除了一个或一个以上的元素，则该方法返回 <code v-pre>true</code>。</li>
<li><code v-pre>boolean retainAll(Collection c)</code>：从集合中删除集合 <code v-pre>c</code> 里不包含的元素（相当于把调用该方法的集合变成该集合和集合 <code v-pre>c</code> 的交集），如果该操作改变了调用该方法的集合，则该方法返回 <code v-pre>true</code>。</li>
<li><code v-pre>int size()</code>：该方法返回集合里元素的个数。</li>
<li><code v-pre>Object[] toArray()</code>：该方法把集合转换成一个数组，所有的集合元素变成对应的数组元素。</li>
</ol>
</blockquote>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">CollectionTest</span> <span class="token punctuation">{</span>
...
<span class="token comment">/*
    ...
    books集合的元素:[]
*/</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="iterator" tabindex="-1"><a class="header-anchor" href="#iterator" aria-hidden="true">#</a> <code v-pre>Iterator</code></h3>
<p><code v-pre>Collection</code> 系列集合、<code v-pre>Map</code> 系列集合主要用于盛装其他对象，而 <code v-pre>Iterator</code> 则主要用于遍历（即迭代访问）<code v-pre>Collection</code> 集合中的元素，<code v-pre>Iterator</code> 对象也被称为迭代器。</p>
<p><code v-pre>Iterator</code> 接口隐藏了各种 <code v-pre>Collection</code> 实现类的底层细节，向应用程序提供了遍历 <code v-pre>Collection</code> 集合元素的统一编程接口。</p>
<blockquote>
<ol>
<li><code v-pre>boolean hasNext()</code>：如果被迭代的集合元素还没有被遍历，则返回 <code v-pre>true</code>。</li>
<li><code v-pre>Object next()</code>：返回集合里的下一个元素。</li>
<li><code v-pre>void remove()</code>：删除集合里上一次 <code v-pre>next</code> 方法返回的元素。</li>
</ol>
</blockquote>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">IteratorTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<p><code v-pre>Iterator</code> 必须依附于 <code v-pre>Collection</code> 对象，若有一个 <code v-pre>Iterator</code> 对象，则必然有一个与之关联的 <code v-pre>Collection</code> 对象。</p>
<p><code v-pre>Iterator</code> 提供了两个方法来迭代访问 <code v-pre>Collection</code> 集合里的元素，并可通过 <code v-pre>remove()</code> 方法来删除集合中上一次 <code v-pre>next()</code> 方法返回的集合元素。</p>
</div>
<p>当使用 <code v-pre>Iterator</code> 对集合元素进行迭代时，<code v-pre>Iterator</code> 并不是把集合元素本身传给了迭代变量，而是把集合元素的值传给了迭代变量，所以修改迭代变量的值对集合元素本身没有任何影响。</p>
<p>当使用 <code v-pre>Iterator</code> 迭代访问 <code v-pre>Collection</code> 集合元素时，<code v-pre>Collection</code> 集合里的元素不能被改变，只有通过 <code v-pre>Iterator</code> 的 <code v-pre>remove</code> 方法删除上一次 <code v-pre>next</code> 方法返回的集合元素才可以；否则将会引发 <code v-pre>java.util.Concurrent ModificationException</code> 异常。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">IteratorErrorTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token comment">// 创建一个集合</span>
        <span class="token class-name">Collection</span> books <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">HashSet</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
        books<span class="token punctuation">.</span><span class="token function">add</span><span class="token punctuation">(</span><span class="token string">"轻量级Java EE企业应用实战"</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>Iterator</code> 迭代器采用的是快速失败（fail-fast）机制，一旦在迭代过程中检测到该集合已经被修改（通常是程序中的其他线程修改），程序立即引发 <code v-pre>ConcurrentModificationException</code> 异常，而不是显示修改后的结果，这样可以避免共享资源而引发的潜在问题。</p>
<h3 id="foreach" tabindex="-1"><a class="header-anchor" href="#foreach" aria-hidden="true">#</a> <code v-pre>foreach</code></h3>
<p>除了可以使用 <code v-pre>Iterator</code> 接口迭代访问 <code v-pre>Collection</code> 集合里的元素之外，使用 Java 5 提供的 <code v-pre>foreach</code> 循环迭代访问集合元素更加便捷，如下程序示范了使用 <code v-pre>foreach</code> 循环来迭代访问集合元素。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ForeachTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token comment">// 创建一个集合</span>
        <span class="token class-name">Collection</span> books <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">HashSet</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>foreach</code> 循环中的迭代变量也不是集合元素本身，系统只是依次把集合元素的值赋给迭代变量，因此在 <code v-pre>foreach</code> 循环中修改迭代变量的值也没有任何实际意义。</p>
<p>同样，当使用 <code v-pre>foreach</code> 循环迭代访问集合元素时，该集合也不能被改变，否则将引发 <code v-pre>Concurrent ModificationException</code> 异常。</p>
<h2 id="set-集合" tabindex="-1"><a class="header-anchor" href="#set-集合" aria-hidden="true">#</a> <code v-pre>Set</code> 集合</h2>
<p><code v-pre>Set</code> 集合与 <code v-pre>Collection</code> 基本上完全一样，它没有提供任何额外的方法。实际上 <code v-pre>Set</code> 就是 <code v-pre>Collection</code>，只是行为略有不同（<code v-pre>Set</code> 不允许包含重复元素）。</p>
<p><code v-pre>Set</code> 判断两个对象相同不是使用 <code v-pre>==</code> 运算符，而是根据 <code v-pre>equals</code> 方法。</p>
<p>也就是说，只要两个对象用 <code v-pre>equals</code> 方法比较返回 <code v-pre>true</code>，<code v-pre>Set</code> 就不会接受这两个对象；反之，只要两个对象用 <code v-pre>equals</code> 方法比较返回 <code v-pre>false</code>，<code v-pre>Set</code> 就会接受这两个对象（甚至这两个对象是同一个对象，<code v-pre>Set</code> 也可把它们当成两个对象处理，在后面程序中可以看到这种极端的情况）。</p>
<p>下面是使用普通 <code v-pre>Set</code> 的示例程序。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">SetTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="hashset-类" tabindex="-1"><a class="header-anchor" href="#hashset-类" aria-hidden="true">#</a> <code v-pre>HashSet</code> 类</h3>
<p><code v-pre>HashSet</code> 是 <code v-pre>Set</code> 接口的典型实现，大多数时候使用 <code v-pre>Set</code> 集合时就是使用这个实现类。<code v-pre>HashSet</code> 按 <code v-pre>Hash</code> 算法来存储集合中的元素，因此具有很好的存取和查找性能。</p>
<p><code v-pre>HashSet</code> 具有以下特点。</p>
<ol>
<li>不能保证元素的排列顺序，顺序有可能发生变化。</li>
<li><code v-pre>HashSet</code> 不是同步的，如果多个线程同时访问一个 <code v-pre>HashSet</code>，假设有两个或者两个以上线程同时修改了 <code v-pre>HashSet</code> 集合时，则必须通过代码来保证其同步。</li>
<li>集合元素值可以是 <code v-pre>null</code>。</li>
</ol>
<p>当向 <code v-pre>HashSet</code> 集合中存入一个元素时，<code v-pre>HashSet</code> 会调用该对象的 <code v-pre>hashCode()</code> 方法来得到该对象的 <code v-pre>hashCode</code> 值，然后根据该 <code v-pre>HashCode</code> 值决定该对象在 <code v-pre>HashSet</code> 中的存储位置。</p>
<p>如果有两个元素通过 <code v-pre>equals()</code> 方法比较返回 <code v-pre>true</code>，但它们的 <code v-pre>hashCode()</code> 方法返回值不相等，<code v-pre>HashSet</code> 将会把它们存储在不同的位置，依然可以添加成功。</p>
<p>简单地说，<code v-pre>HashSet</code> 集合判断两个元素相等的标准是两个对象通过 <code v-pre>equals()</code> 方法比较相等，并且两个对象的 <code v-pre>hashCode()</code> 方法返回值也相等。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 类A的equals()方法总是返回true，但没有重写其hashCode()方法</span>
...
*/</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上面程序中向 <code v-pre>books</code> 集合中分别添加了两个 A 对象、两个 B 对象和两个 C 对象，其中 C 类重写了 <code v-pre>equals()</code> 方法总是返回 <code v-pre>true</code>，<code v-pre>hashCode()</code> 方法总是返回 2，这将导致 <code v-pre>HashSet</code> 把两个 C 对象当成同一个对象。</p>
<div class="custom-container tip">
<p class="custom-container-title">提示</p>
<p>当把一个对象放入 <code v-pre>HashSet</code> 中时，如果需要重写该对象对应类的 <code v-pre>equals()</code> 方法，则也应该重写其 <code v-pre>hashCode()</code> 方法。</p>
<p>其规则是：如果两个对象通过 <code v-pre>equals()</code> 方法比较返回 <code v-pre>true</code>，这两个对象的 <code v-pre>hashCode</code> 值也应该相同。</p>
</div>
<p>如果两个对象通过 <code v-pre>equals()</code> 方法比较返回 <code v-pre>true</code>，但这两个对象的 <code v-pre>hashCode()</code> 方法返回不同的 <code v-pre>hashCode</code> 值时，这将导致 <code v-pre>HashSet</code> 会把这两个对象保存在 <code v-pre>Hash</code> 表的不同位置，从而使两个对象都可以添加成功，这就与 <code v-pre>Set</code> 集合的规则有些出入了。</p>
<p>如果两个对象的 <code v-pre>hashCode()</code> 方法返回的 <code v-pre>hashCode</code> 值相同，但它们通过 <code v-pre>equals()</code> 方法比较返回 <code v-pre>false</code> 时将更麻烦：因为两个对象的 <code v-pre>hashCode</code> 值相同，<code v-pre>HashSet</code> 将试图把它们保存在同一个位置，但又不行（否则将只剩下一个对象），所以实际上会在这个位置用链式结构来保存多个对象；而 <code v-pre>HashSet</code> 访问集合元素时也是根据元素的 <code v-pre>hashCode</code> 值来快速定位的，如果 <code v-pre>HashSet</code> 中两个以上的元素具有相同的 <code v-pre>hashCode</code> 值，将会导致性能下降。</p>
<div class="custom-container info">
<p class="custom-container-title">相关信息</p>
<p><code v-pre>HashSet</code> 中每个能存储元素的“槽位”（<code v-pre>slot</code>）通常称为“桶”（<code v-pre>bucket</code>），如果有多个元素的 <code v-pre>hashCode</code> 值相同，但它们通过 <code v-pre>equals()</code> 方法比较返回 <code v-pre>false</code>，就需要在一个“桶”里放多个元素，这样会导致性能下降。</p>
</div>
<p>重写 <code v-pre>hashCode()</code> 方法的基本规则。</p>
<blockquote>
<ol>
<li>在程序运行过程中，同一个对象多次调用 <code v-pre>hashCode()</code>方法应该返回相同的值。</li>
<li>当两个对象通过 <code v-pre>equals()</code>方法比较返回 <code v-pre>true</code> 时，这两个对象的 <code v-pre>hashCode()</code>方法应返回相等的值。</li>
<li>对象中用作 <code v-pre>equals()</code>方法比较标准的 Field，都应该用来计算 <code v-pre>hashCode</code> 值。</li>
</ol>
</blockquote>
<p>重写 <code v-pre>hashCode()</code> 方法的一般规则。</p>
<ul>
<li>把对象内每个有意义的 Field（即每个用做 <code v-pre>equals()</code> 方法比较标准的 Field）计算出一个 <code v-pre>int</code> 类型的 <code v-pre>hashCode</code> 值。</li>
</ul>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208111041736.jpeg" alt="img"></p>
<ul>
<li>用第 1 步计算出来的多个 <code v-pre>hashCode</code> 值组合计算出一个 <code v-pre>hashCode</code> 值返回。</li>
</ul>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>为了避免直接相加产生偶然相等（两个对象的 <code v-pre>f1</code>、<code v-pre>f2</code> Field 并不相等，但它们的和恰好相等），可以通过为各 Field 乘以任意一个质数后再相加。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>如果向 <code v-pre>HashSet</code> 中添加一个可变对象后，后面程序修改了该可变对象的 Field，则可能导致它与集合中的其他元素相同，这就有可能导致 <code v-pre>HashSet</code> 中包含两个相同的对象。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">class</span> <span class="token class-name">R</span> <span class="token punctuation">{</span>
...
*/</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>此时 <code v-pre>HashSet</code> 会比较混乱：</p>
<blockquote>
<p>当试图删除 <code v-pre>count</code> 为-3 的 <code v-pre>R</code> 对象时，<code v-pre>HashSet</code> 会计算出该对象的 <code v-pre>hashCode</code> 值，从而找出该对象在集合中的保存位置，然后把此处的对象与 <code v-pre>count</code> 为-3 的 <code v-pre>R</code> 对象通过 <code v-pre>equals()</code> 方法进行比较，如果相等则删除该对象——<code v-pre>HashSet</code> 只有第三个元素才满足该条件（第一个元素实际上保存在 <code v-pre>count</code> 为 5 的 <code v-pre>R</code> 对象对应的位置），所以第三个元素被删除。</p>
<p>至于第一个 <code v-pre>count</code> 为-3 的 <code v-pre>R</code> 对象，它保存在 <code v-pre>count</code> 为 <code v-pre>5</code> 的 <code v-pre>R</code> 对象对应的位置，但使用 <code v-pre>equals()</code> 方法拿它和 <code v-pre>count</code> 为 5 的 <code v-pre>R</code> 对象比较时又返回 <code v-pre>false</code>——这将导致 <code v-pre>HashSet</code> 不可能准确访问该元素。</p>
</blockquote>
<h3 id="linkedhashset-类" tabindex="-1"><a class="header-anchor" href="#linkedhashset-类" aria-hidden="true">#</a> <code v-pre>LinkedHashSet</code> 类</h3>
<p><code v-pre>HashSet</code> 还有一个子类 <code v-pre>LinkedHashSet</code>，<code v-pre>LinkedHashSet</code> 集合也是根据元素的 <code v-pre>hashCode</code> 值来决定元素的存储位置，但它同时使用链表维护元素的次序，这样使得元素看起来是以插入的顺序保存的。</p>
<p>也就是说，当遍历 <code v-pre>LinkedHashSet</code> 集合里的元素时，<code v-pre>LinkedHashSet</code> 将会按元素的添加顺序来访问集合里的元素。</p>
<p><code v-pre>LinkedHashSet</code> 需要维护元素的插入顺序，因此性能略低于 <code v-pre>HashSet</code> 的性能，但在迭代访问 <code v-pre>Set</code> 里的全部元素时将有很好的性能，因为它以链表来维护内部顺序。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">LinkedHashSetTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<p class="custom-container-title">提示</p>
<p>虽然 <code v-pre>LinkedHashSet</code> 使用了链表记录集合元素的添加顺序，但 <code v-pre>LinkedHashSet</code> 依然是 <code v-pre>HashSet</code>，因此它依然不允许集合元素重复。</p>
</div>
<h3 id="treeset-类" tabindex="-1"><a class="header-anchor" href="#treeset-类" aria-hidden="true">#</a> <code v-pre>TreeSet</code> 类</h3>
<p><code v-pre>TreeSet</code> 是 <code v-pre>SortedSet</code> 接口的实现类，正如 <code v-pre>SortedSet</code> 名字所暗示的，<code v-pre>TreeSet</code> 可以确保集合元素处于排序状态。与 <code v-pre>HashSet</code> 集合相比，<code v-pre>TreeSet</code> 还提供了如下几个额外的方法。</p>
<blockquote>
<ol>
<li><code v-pre>Comparator comparator()</code>：如果 <code v-pre>TreeSet</code> 采用了定制排序，则该方法返回定制排序所使用的 <code v-pre>Comparator</code>；如果 <code v-pre>TreeSet</code> 采用了自然排序，则返回 <code v-pre>null</code>。</li>
<li><code v-pre>Object first()</code>：返回集合中的第一个元素。</li>
<li><code v-pre>Object last()</code>：返回集合中的最后一个元素。</li>
<li><code v-pre>Object lower(Object e)</code>：返回集合中位于指定元素之前的元素（即小于指定元素的最大元素，参考元素不需要是 <code v-pre>TreeSet</code> 集合里的元素）。</li>
<li><code v-pre>Object higher (Object e)</code>：返回集合中位于指定元素之后的元素（即大于指定元素的最小元素，参考元素不需要是 <code v-pre>TreeSet</code> 集合里的元素）。</li>
<li><code v-pre>SortedSet subSet(fromElement, toElement)</code>：返回此 <code v-pre>Set</code> 的子集合，范围从 <code v-pre>fromElement</code>（包含）到 <code v-pre>toElement</code>（不包含）。</li>
<li><code v-pre>SortedSet headSet(toElement)</code>：返回此 <code v-pre>Set</code> 的子集，由小于 <code v-pre>toElement</code> 的元素组成。</li>
<li><code v-pre>SortedSet tailSet(fromElement)</code>：返回此 <code v-pre>Set</code> 的子集，由大于或等于 <code v-pre>fromElement</code> 的元素组成。</li>
</ol>
</blockquote>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">TreeSetTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token comment">/*
    ...
    [2]
*/</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>与 <code v-pre>HashSet</code> 集合采用 <code v-pre>hash</code> 算法来决定元素的存储位置不同，<code v-pre>TreeSet</code> 采用红黑树的数据结构来存储集合元素。<code v-pre>TreeSet</code> 支持两种排序方法：自然排序和定制排序。在默认情况下，<code v-pre>TreeSet</code> 采用自然排序。</p>
<h4 id="自然排序" tabindex="-1"><a class="header-anchor" href="#自然排序" aria-hidden="true">#</a> 自然排序</h4>
<p><code v-pre>TreeSet</code> 会调用集合元素的 <code v-pre>compareTo(Object obj)</code> 方法来比较元素之间的大小关系，然后将集合元素按升序排列，这种方式就是自然排序。</p>
<p>Java 提供了一个 <code v-pre>Comparable</code> 接口，该接口里定义了一个 <code v-pre>compareTo(Object obj)</code> 方法，该方法返回一个整数值，实现该接口的类必须实现该方法，实现了该接口的类的对象就可以比较大小。</p>
<p>当一个对象调用该方法与另一个对象进行比较时，例如 <code v-pre>obj1.compareTo(obj2)</code>，如果该方法返回 0，则表明这两个对象相等；如果该方法返回一个正整数，则表明 <code v-pre>obj1</code> 大于 <code v-pre>obj2</code>；如果该方法返回一个负整数，则表明 <code v-pre>obj1</code> 小于 <code v-pre>obj2</code>。</p>
<p>Java 的一些常用类已经实现了 <code v-pre>Comparable</code> 接口，并提供了比较大小的标准。</p>
<blockquote>
<ol>
<li><code v-pre>BigDecimal</code>、<code v-pre>BigInteger</code> 以及所有的数值型对应的包装类：按它们对应的数值大小进行比较。</li>
<li><code v-pre>Character</code>：按字符的 <code v-pre>UNICODE</code> 值进行比较。</li>
<li><code v-pre>Boolean</code>：<code v-pre>true</code> 对应的包装类实例大于 <code v-pre>false</code> 对应的包装类实例。</li>
<li><code v-pre>String</code>：按字符串中字符的 <code v-pre>UNICODE</code> 值进行比较。</li>
<li><code v-pre>Date</code>、<code v-pre>Time</code>：后面的时间、日期比前面的时间、日期大。</li>
</ol>
</blockquote>
<p>如果试图把一个对象添加到 <code v-pre>TreeSet</code> 时，则该对象的类必须实现 <code v-pre>Comparable</code> 接口，否则程序将会抛出异常。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">class</span> <span class="token class-name">Err</span> <span class="token punctuation">{</span>
<span class="token punctuation">}</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">TreeSetErrorTest</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>添加第一个对象时，<code v-pre>TreeSet</code> 里没有任何元素，所以不会出现任何问题；当添加第二个 <code v-pre>Err</code> 对象时，<code v-pre>TreeSet</code> 就会调用该对象的 <code v-pre>compareTo(Object obj)</code> 方法与集合中的其他元素进行比较——如果其对应的类没有实现 <code v-pre>Comparable</code> 接口，则会引发 <code v-pre>ClassCastException</code> 异常。</p>
<div class="custom-container tip">
<p class="custom-container-title">提示</p>
<p>向 <code v-pre>TreeSet</code> 集合中添加元素时，只有第一个元素无须实现 <code v-pre>Comparable</code> 接口，后面添加的所有元素都必须实现 <code v-pre>Comparable</code> 接口。当然这也不是一种好做法，当试图从 <code v-pre>TreeSet</code> 中取出元素时，依然会引发 <code v-pre>ClassCastException</code> 异常。</p>
</div>
<p>大部分类在实现 <code v-pre>compareTo(Object obj)</code> 方法时，都需要将被比较对象 <code v-pre>obj</code> 强制类型转换成相同类型，因为只有相同类的两个实例才会比较大小。当试图把一个对象添加到 <code v-pre>TreeSet</code> 集合时，<code v-pre>TreeSet</code> 会调用该对象的 <code v-pre>compareTo(Object obj)</code> 方法与集合中的其他元素进行比较——这就要求集合中的其他元素与该元素是同一个类的实例。</p>
<p>也就是说，向 <code v-pre>TreeSet</code> 中添加的应该是同一个类的对象，否则也会引发 <code v-pre>ClassCastException</code> 异常。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">TreeSetErrorTest2</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token class-name">TreeSet</span> ts <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">TreeSet</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果向 <code v-pre>TreeSet</code> 中添加的对象是程序员自定义类的对象，则可以向 <code v-pre>TreeSet</code> 中添加多种类型的对象，前提是用户自定义类实现了 <code v-pre>Comparable</code> 接口，实现该接口时实现的 <code v-pre>compareTo(Object obj)</code> 方法没有进行强制类型转换。但当试图取出 <code v-pre>TreeSet</code> 里的集合数据时，不同类型的元素依然会发生 <code v-pre>ClassCastException</code> 异常。</p>
<p>当把一个对象加入 <code v-pre>TreeSet</code> 集合中时，<code v-pre>TreeSet</code> 调用该对象的 <code v-pre>compareTo(Object obj)</code> 方法与容器中的其他对象比较大小，然后根据红黑树结构找到它的存储位置。</p>
<p>如果两个对象通过 <code v-pre>compareTo(Object obj)</code> 方法比较相等，新对象将无法添加到 <code v-pre>TreeSet</code> 集合中。</p>
<p>对于 <code v-pre>TreeSet</code> 集合而言，它判断两个对象是否相等的唯一标准是：两个对象通过 <code v-pre>compareTo(Object obj)</code> 方法比较是否返回 0——如果通过 <code v-pre>compareTo(Object obj)</code> 方法比较返回 0，<code v-pre>TreeSet</code> 则会认为它们相等；否则就认为它们不相等。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">class</span> <span class="token class-name">Z</span> <span class="token keyword">implements</span> <span class="token class-name">Comparable</span> <span class="token punctuation">{</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">TreeSetTest2</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>程序中 ① 代码行把同一个对象再次添加到 <code v-pre>TreeSet</code> 集合中，因为 <code v-pre>z1</code> 对象的 <code v-pre>compareTo(Object obj)</code> 方法总是返回 1，虽然它的 <code v-pre>equals()</code> 方法总是返回 <code v-pre>true</code>，但 <code v-pre>TreeSet</code> 会认为 <code v-pre>z1</code> 对象和它自己也不相等，因此 <code v-pre>TreeSet</code> 可以添加两个 <code v-pre>z1</code> 对象。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208111120994.jpeg" alt="img"></p>
<p>从图中可以看到 <code v-pre>TreeSet</code> 对象保存的两个元素引用，实际上是同一个元素。所以当修改 <code v-pre>TreeSet</code> 集合里第一个元素的 <code v-pre>age</code> 变量后，该 <code v-pre>TreeSet</code> 集合里最后一个元素的 <code v-pre>age</code> 变量也随之改变了。</p>
<p>当需要把一个对象放入 <code v-pre>TreeSet</code> 中，重写该对象对应类的 <code v-pre>equals()</code> 方法时，应保证该方法与 <code v-pre>compareTo(Object obj)</code>方法有一致的结果，其规则是：如果两个对象通过 <code v-pre>equals()</code>方法比较返回 true 时，这两个对象通过 <code v-pre>compareTo(Object obj)</code> 方法比较应返回 0。</p>
<p>如果两个对象通过 <code v-pre>compareTo(Object obj)</code> 方法比较返回 0 时，但它们通过 <code v-pre>equals()</code> 方法比较返回 <code v-pre>false</code> 将很麻烦，因为两个对象通过 <code v-pre>compareTo(Object obj)</code> 方法比较相等，<code v-pre>TreeSet</code> 不会让第二个元素添加进去，这就会与 <code v-pre>Set</code> 集合的规则产生冲突。</p>
<p>如果向 <code v-pre>TreeSet</code> 中添加一个可变对象后，并且后面程序修改了该可变对象的 Field，这将导致它与其他对象的大小顺序发生了改变，但 <code v-pre>TreeSet</code> 不会再次调整它们的顺序，甚至可能导致 <code v-pre>TreeSet</code> 中保存的这两个对象通过 <code v-pre>compareTo(Object obj)</code> 方法比较返回 0。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">class</span> <span class="token class-name">R1</span> <span class="token keyword">implements</span> <span class="token class-name">Comparable</span> <span class="token punctuation">{</span>
    <span class="token keyword">int</span> count<span class="token punctuation">;</span>
...
*/</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上面程序中的 <code v-pre>R</code> 对象对应的类正常重写了 <code v-pre>equals()</code> 方法和 <code v-pre>compareTo()</code> 方法，这两个方法都以 <code v-pre>R</code> 对象的 <code v-pre>count</code> 实例变量作为判断的依据。</p>
<p>当程序执行 ① 行代码时，看到程序输出的 <code v-pre>Set</code> 集合元素处于有序状态；因为 <code v-pre>R1</code> 类是一个可变类，因此可以改变 <code v-pre>R1</code> 对象的 <code v-pre>count</code> 实例变量的值，程序改变了该集合里第一个元素和最后一个元素的 <code v-pre>count</code> 实例变量的值。</p>
<p>当程序执行 ② 行代码输出时，将看到该集合处于无序状态，而且集合中包含了重复元素。</p>
<p>一旦改变了 <code v-pre>TreeSet</code> 集合里可变元素的 Field，当再试图删除该对象时，<code v-pre>TreeSet</code> 也会删除失败（甚至集合中原有的、Field 没被修改但与修改后元素相等的元素也无法删除），所以在上面程序的 ③ 代码处，删除 <code v-pre>count</code> 为-2 的 <code v-pre>R1</code> 对象时，没有任何元素被删除；程序执行 ④ 代码时，可以看到删除了 <code v-pre>count</code> 为 5 的 <code v-pre>R1</code> 对象，这表明 <code v-pre>TreeSet</code> 可以删除没有被修改 Field，且不与其他被修改 Field 的对象重复的对象。</p>
<div class="custom-container tip">
<p class="custom-container-title">提示</p>
<p>当执行了 ④ 代码后，<code v-pre>TreeSet</code> 会对集合中的元素重新索引（不是重新排序），接下来就可以删除 <code v-pre>TreeSet</code> 中的所有元素了，包括那些被修改过 Field 的元素。与 <code v-pre>HashSet</code> 类似的是，如果 <code v-pre>TreeSet</code> 中包含了可变对象，当可变对象的 Field 被修改时，<code v-pre>TreeSet</code> 在处理这些对象时将非常复杂，而且容易出错。</p>
<p>为了让程序更加健壮，推荐 <code v-pre>HashSet</code> 和 <code v-pre>TreeSet</code> 集合中只放入不可变对象。</p>
</div>
<h4 id="定制排序" tabindex="-1"><a class="header-anchor" href="#定制排序" aria-hidden="true">#</a> 定制排序</h4>
<p><code v-pre>TreeSet</code> 的自然排序是根据集合元素的大小，<code v-pre>TreeSet</code> 将它们以升序排列。如果需要实现定制排序，例如以降序排列，则可以通过 <code v-pre>Comparator</code> 接口的帮助。</p>
<p>该接口里包含一个 <code v-pre>int compare(T o1, T o2)</code> 方法，该方法用于比较 <code v-pre>o1</code> 和 <code v-pre>o2</code> 的大小：如果该方法返回正整数，则表明 <code v-pre>o1</code> 大于 <code v-pre>o2</code>；如果该方法返回 0，则表明 <code v-pre>o1</code> 等于 <code v-pre>o2</code>；如果该方法返回负整数，则表明 <code v-pre>o1</code> 小于 <code v-pre>o2</code>。</p>
<p>如果需要实现定制排序，则需要在创建 <code v-pre>TreeSet</code> 集合对象时，提供一个 <code v-pre>Comparator</code> 对象与该 <code v-pre>TreeSet</code> 集合关联，由该 <code v-pre>Comparator</code> 对象负责集合元素的排序逻辑。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">class</span> <span class="token class-name">M</span> <span class="token punctuation">{</span>
    <span class="token keyword">int</span> age<span class="token punctuation">;</span>
...
<p class="custom-container-title">提示</p>
<p>当通过 <code v-pre>Comparator</code> 对象来实现 <code v-pre>TreeSet</code> 的定制排序时，依然不可以向 <code v-pre>TreeSet</code> 中添加类型不同的对象，否则会引发 <code v-pre>ClassCastException</code> 异常。</p>
<p>使用定制排序时，<code v-pre>TreeSet</code> 对集合元素排序不管集合元素本身的大小，而是由 <code v-pre>Comparator</code> 对象负责集合元素的排序规则。</p>
<p><code v-pre>TreeSet</code> 判断两个集合元素相等的标准是：通过 <code v-pre>Comparator</code> 比较两个元素返回了 0，这样 <code v-pre>TreeSet</code> 不会把第二个元素添加到集合中。</p>
</div>
<h3 id="enumset-类" tabindex="-1"><a class="header-anchor" href="#enumset-类" aria-hidden="true">#</a> <code v-pre>EnumSet</code> 类</h3>
<p><code v-pre>EnumSet</code> 是一个专为枚举类设计的集合类，<code v-pre>EnumSet</code> 中的所有元素都必须是指定枚举类型的枚举值，该枚举类型在创建 <code v-pre>EnumSet</code> 时显式或隐式地指定。<code v-pre>EnumSet</code> 的集合元素也是有序的，<code v-pre>EnumSet</code> 以枚举值在 <code v-pre>Enum</code> 类内的定义顺序来决定集合元素的顺序。</p>
<p><code v-pre>EnumSet</code> 在内部以位向量的形式存储，这种存储形式非常紧凑、高效，因此 <code v-pre>EnumSet</code> 对象占用内存很小，而且运行效率很好。尤其是进行批量操作（如调用 <code v-pre>containsAll</code> 和 <code v-pre>retainAll</code> 方法）时，如果其参数也是 <code v-pre>EnumSet</code> 集合，则该批量操作的执行速度也非常快。</p>
<p><code v-pre>EnumSet</code> 集合不允许加入 <code v-pre>null</code> 元素，如果试图插入 <code v-pre>null</code> 元素，<code v-pre>EnumSet</code> 将抛出 <code v-pre>NullPointerException</code> 异常。如果只是想判断 <code v-pre>EnumSet</code> 是否包含 <code v-pre>null</code> 元素或试图删除 <code v-pre>null</code> 元素都不会抛出异常，只是删除操作将返回 <code v-pre>false</code>，因为没有任何 <code v-pre>null</code> 元素被删除。</p>
<p><code v-pre>EnumSet</code> 类没有暴露任何构造器来创建该类的实例，程序应该通过它提供的 <code v-pre>static</code> 方法来创建 <code v-pre>EnumSet</code> 对象。</p>
<p><code v-pre>EnumSet</code> 类它提供了如下常用的 <code v-pre>static</code> 方法来创建 <code v-pre>EnumSet</code> 对象。</p>
<blockquote>
<ol>
<li><code v-pre>static EnumSet allOf(Class elementType)</code>：创建一个包含指定枚举类里所有枚举值的 <code v-pre>EnumSet</code> 集合。</li>
<li><code v-pre>static EnumSet complementOf(EnumSet s)</code>：创建一个其元素类型与指定 <code v-pre>EnumSet</code> 里元素类型相同的 <code v-pre>EnumSet</code> 集合，新 <code v-pre>EnumSet</code> 集合包含原 <code v-pre>EnumSet</code> 集合所不包含的、此枚举类剩下的枚举值（即新 <code v-pre>EnumSet</code> 集合和原 <code v-pre>EnumSet</code> 集合的集合元素加起来就是该枚举类的所有枚举值）。</li>
<li><code v-pre>static EnumSet copyOf(Collection c)</code>：使用一个普通集合来创建 <code v-pre>EnumSet</code> 集合。</li>
<li><code v-pre>static EnumSet copyOf(EnumSet s)</code>：创建一个与指定 <code v-pre>EnumSet</code> 具有相同元素类型、相同集合元素的 <code v-pre>EnumSet</code> 集合。</li>
<li><code v-pre>static EnumSet noneOf(Class elementType)</code>：创建一个元素类型为指定枚举类型的空 <code v-pre>EnumSet</code>。</li>
<li><code v-pre>static EnumSet of(E first, E... rest)</code>：创建一个包含一个或多个枚举值的 <code v-pre>EnumSet</code> 集合，传入的多个枚举值必须属于同一个枚举类。</li>
<li><code v-pre>static EnumSet range(E from, E to)</code>：创建一个包含从 from 枚举值到 to 枚举值范围内所有枚举值的 <code v-pre>EnumSet</code> 集合。</li>
</ol>
</blockquote>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">enum</span> <span class="token class-name">Season</span> <span class="token punctuation">{</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">EnumSetTest</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上面程序示范了 <code v-pre>EnumSet</code> 集合的常规用法。除此之外，还可以复制另一个 <code v-pre>EnumSet</code> 集合中的所有元素来创建新的 <code v-pre>EnumSet</code> 集合，或者复制另一个 <code v-pre>Collection</code> 集合中的所有元素来创建新的 <code v-pre>EnumSet</code> 集合。</p>
<p>当复制 <code v-pre>Collection</code> 集合中的所有元素来创建新的 <code v-pre>EnumSet</code> 集合时，要求 <code v-pre>Collection</code> 集合中的所有元素必须是同一个枚举类的枚举值。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">EnumSetTest2</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="set-性能分析" tabindex="-1"><a class="header-anchor" href="#set-性能分析" aria-hidden="true">#</a> <code v-pre>Set</code> 性能分析</h3>
<p><code v-pre>HashSet</code> 和 <code v-pre>TreeSet</code> 是 <code v-pre>Set</code> 的两个典型实现，到底如何选择 <code v-pre>HashSet</code> 和 <code v-pre>TreeSet</code> 呢？</p>
<p><code v-pre>HashSet</code> 的性能总是比 <code v-pre>TreeSet</code> 好（特别是最常用的添加、查询元素等操作），因为 <code v-pre>TreeSet</code> 需要额外的红黑树算法来维护集合元素的次序。只有当需要一个保持排序的 <code v-pre>Set</code> 时，才应该使用 <code v-pre>TreeSet</code>，否则都应该使用 <code v-pre>HashSet</code>。</p>
<p><code v-pre>HashSet</code> 还有一个子类：<code v-pre>LinkedHashSet</code>，对于普通的插入、删除操作，<code v-pre>LinkedHashSet</code> 比 <code v-pre>HashSet</code> 要略微慢一点，这是由维护链表所带来的额外开销造成的；不过，因为有了链表，遍历 <code v-pre>LinkedHashSet</code> 会更快。</p>
<p><code v-pre>EnumSet</code> 是所有 <code v-pre>Set</code> 实现类中性能最好的，但它只能保存同一个枚举类的枚举值作为集合元素。</p>
<p><mark>必须指出的是，<code v-pre>Set</code> 的三个实现类 <code v-pre>HashSet</code>、<code v-pre>TreeSet</code> 和 <code v-pre>EnumSet</code> 都是线程不安全的。</mark></p>
<p>如果有多个线程同时访问一个 <code v-pre>Set</code> 集合，并且有超过一个线程修改了该 <code v-pre>Set</code> 集合，则必须手动保证该 <code v-pre>Set</code> 集合的同步性。通常可以通过 <code v-pre>Collections</code> 工具类的 <code v-pre>synchronizedSortedSet</code> 方法来“包装”该 <code v-pre>Set</code> 集合。</p>
<p>此操作最好在创建时进行，以防止对 <code v-pre>Set</code> 集合的意外非同步访问。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h2 id="list-集合" tabindex="-1"><a class="header-anchor" href="#list-集合" aria-hidden="true">#</a> <code v-pre>List</code> 集合</h2>
<p><code v-pre>List</code> 集合代表一个元素有序、可重复的集合，集合中每个元素都有其对应的顺序索引。<code v-pre>List</code> 集合允许使用重复元素，可以通过索引来访问指定位置的集合元素。<code v-pre>List</code> 集合默认按元素的添加顺序设置元素的索引，例如第一次添加的元素索引为 0，第二次添加的元素索引为 1……</p>
<h3 id="list-和-listiterator" tabindex="-1"><a class="header-anchor" href="#list-和-listiterator" aria-hidden="true">#</a> <code v-pre>List</code> 和 <code v-pre>ListIterator</code></h3>
<p><code v-pre>List</code> 作为 <code v-pre>Collection</code> 接口的子接口，当然可以使用 <code v-pre>Collection</code> 接口里的全部方法。而且由于 <code v-pre>List</code> 是有序集合，因此 <code v-pre>List</code> 集合里增加了一些根据索引来操作集合元素的方法。</p>
<blockquote>
<ol>
<li><code v-pre>void add(int index, Object element)</code>：将元素 <code v-pre>element</code> 插入到 <code v-pre>List</code> 集合的 <code v-pre>index</code> 处。</li>
<li><code v-pre>boolean addAll(int index, Collection c)</code>：将集合 <code v-pre>c</code> 所包含的所有元素都插入到 <code v-pre>List</code> 集合的 <code v-pre>index</code> 处。</li>
<li><code v-pre>Object get(int index)</code>：返回集合 <code v-pre>index</code> 索引处的元素。</li>
<li><code v-pre>int indexOf(Object o)</code>：返回对象 o 在 <code v-pre>List</code> 集合中第一次出现的位置索引。</li>
<li><code v-pre>int lastIndexOf(Object o)</code>：返回对象 o 在 <code v-pre>List</code> 集合中最后一次出现的位置索引。</li>
<li><code v-pre>Object remove(int index)</code>：删除并返回 <code v-pre>index</code> 索引处的元素。</li>
<li><code v-pre>Object set(int index, Object element)</code>：将 <code v-pre>index</code> 索引处的元素替换成 <code v-pre>element</code> 对象，返回新元素。</li>
<li><code v-pre>List subList(int fromIndex, int toIndex)</code>：返回从索引 <code v-pre>fromIndex</code>（包含）到索引 <code v-pre>toIndex</code>（不包含）处所有集合元素组成的子集合。</li>
</ol>
</blockquote>
<p>所有的 <code v-pre>List</code> 实现类都可以调用这些方法来操作集合元素。</p>
<p>与 <code v-pre>Set</code> 集合相比，<code v-pre>List</code> 增加了根据索引来插入、替换和删除集合元素的方法。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ListTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token comment">/*
    ...
    [疯狂Java讲义]
*/</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>注意 ① 行代码处，程序试图返回新字符串对象在 <code v-pre>List</code> 集合中的位置，实际上 <code v-pre>List</code> 集合中并未包含该字符串对象。</p>
<p>因为 <code v-pre>List</code> 集合添加字符串对象时，添加的是通过 <code v-pre>new</code> 关键字创建的新字符串对象，① 行代码处也是通过 <code v-pre>new</code> 关键字创建的新字符串对象，两个字符串显然不是同一个对象，但 <code v-pre>List</code> 的 <code v-pre>indexOf</code> 方法依然可以返回 1。</p>
<p><code v-pre>List</code> 判断两个对象相等的标准是什么呢？</p>
<p><code v-pre>List</code> 判断两个对象相等只要通过 <code v-pre>equals()</code>方法比较返回 <code v-pre>true</code> 即可。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">class</span> <span class="token class-name">A2</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">boolean</span> <span class="token function">equals</span><span class="token punctuation">(</span><span class="token class-name">Object</span> obj<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token keyword">return</span> <span class="token boolean">true</span><span class="token punctuation">;</span>
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
...
<span class="token comment">/*
    ...
    [疯狂Android讲义]
*/</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>与 <code v-pre>Set</code> 只提供了一个 <code v-pre>iterator()</code> 方法不同，<code v-pre>List</code> 还额外提供了一个 <code v-pre>listIterator()</code> 方法，该方法返回一个 <code v-pre>ListIterator</code> 对象，<code v-pre>ListIterator</code> 接口继承了 <code v-pre>Iterator</code> 接口，提供了专门操作 <code v-pre>List</code> 的方法。</p>
<p><code v-pre>ListIterator</code> 接口在 <code v-pre>Iterator</code> 接口基础上增加了如下方法。</p>
<blockquote>
<ol>
<li><code v-pre>boolean hasPrevious()</code>：返回该迭代器关联的集合是否还有上一个元素。</li>
<li><code v-pre>Object previous()</code>：返回该迭代器的上一个元素。</li>
<li><code v-pre>void add()</code>：在指定位置插入一个元素。</li>
</ol>
</blockquote>
<p>拿 <code v-pre>ListIterator</code> 与普通的 <code v-pre>Iterator</code> 进行对比，不难发现 <code v-pre>ListIterator</code> 增加了向前迭代的功能（<code v-pre>Iterator</code> 只能向后迭代），而且 <code v-pre>ListIterator</code> 还可通过 <code v-pre>add</code> 方法向 <code v-pre>List</code> 集合中添加元素（<code v-pre>Iterator</code> 只能删除元素）。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ListIteratorTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token comment">/*
    ...
    疯狂Java讲义
*/</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>使用 <code v-pre>ListIterator</code> 迭代 <code v-pre>List</code> 集合时，开始也需要采用正向迭代，即先使用 <code v-pre>next()</code> 方法进行迭代，在迭代过程中可以使用 <code v-pre>add()</code> 方法向上一次迭代元素的后面添加一个新元素。</p>
<h3 id="arraylist-和-vector" tabindex="-1"><a class="header-anchor" href="#arraylist-和-vector" aria-hidden="true">#</a> <code v-pre>ArrayList</code> 和 <code v-pre>Vector</code></h3>
<p><code v-pre>ArrayList</code> 和 <code v-pre>Vector</code> 作为 <code v-pre>List</code> 类的两个典型实现，完全支持前面介绍的 <code v-pre>List</code> 接口的全部功能。</p>
<p><code v-pre>ArrayList</code> 和 <code v-pre>Vector</code> 类都是基于数组实现的 <code v-pre>List</code> 类，所以 <code v-pre>ArrayList</code> 和 <code v-pre>Vector</code> 类封装了一个动态的、允许再分配的 <code v-pre>Object[]</code> 数组。</p>
<p><code v-pre>ArrayList</code> 或 <code v-pre>Vector</code> 对象使用 <code v-pre>initialCapacity</code> 参数来设置该数组的长度，当向 <code v-pre>ArrayList</code> 或 <code v-pre>Vector</code> 中添加元素超出了该数组的长度时，它们的 <code v-pre>initialCapacity</code> 会自动增加。</p>
<p>对于通常的编程场景，程序员无须关心 <code v-pre>ArrayList</code> 或 Vector 的 <code v-pre>initialCapacity</code>。但如果向 <code v-pre>ArrayList</code> 或 <code v-pre>Vector</code> 集合中添加大量元素时，可使用 <code v-pre>ensureCapacity(int minCapacity)</code> 方法一次性地增加 <code v-pre>initialCapacity</code>。这可以减少重分配的次数，从而提高性能。</p>
<p>如果开始就知道 <code v-pre>ArrayList</code> 或 <code v-pre>Vector</code> 集合需要保存多少个元素，则可以在创建它们时就指定 <code v-pre>initialCapacity</code> 大小。如果创建空的 <code v-pre>ArrayList</code> 或 <code v-pre>Vector</code> 集合时不指定 <code v-pre>initialCapacity</code> 参数，则 <code v-pre>Object[]</code> 数组的长度默认为 10。</p>
<p>除此之外，<code v-pre>ArrayList</code> 和 <code v-pre>Vector</code> 还提供了如下两个方法来重新分配 <code v-pre>Object[]</code> 数组。</p>
<blockquote>
<ol>
<li><code v-pre>void ensureCapacity(int minCapacity)</code>：将 <code v-pre>ArrayList</code> 或 <code v-pre>Vector</code> 集合的 <code v-pre>Object[]</code> 数组长度增加 <code v-pre>minCapacity</code>。</li>
<li><code v-pre>void trimToSize()</code>：调整 <code v-pre>ArrayList</code> 或 <code v-pre>Vector</code> 集合的 <code v-pre>Object[]</code> 数组长度为当前元素的个数。程序可调用该方法来减少 <code v-pre>ArrayList</code> 或 <code v-pre>Vector</code> 集合对象占用的存储空间。</li>
</ol>
</blockquote>
<p><code v-pre>ArrayList</code> 和 <code v-pre>Vector</code> 在用法上几乎完全相同，但由于 <code v-pre>Vector</code> 是一个古老的集合（从 JDK 1.0 就有了），那时候 Java 还没有提供系统的集合框架，所以 <code v-pre>Vector</code> 里提供了一些方法名很长的方法，例如 <code v-pre>addElement(Object obj)</code>，实际上这个方法与 <code v-pre>add(Object obj)</code> 没有任何区别。</p>
<p>从 JDK 1.2 以后，Java 提供了系统的集合框架，就将 <code v-pre>Vector</code> 改为实现 <code v-pre>List</code> 接口，作为 <code v-pre>List</code> 的实现之一，从而导致 <code v-pre>Vector</code> 里有一些功能重复的方法。</p>
<p><code v-pre>ArrayList</code> 和 <code v-pre>Vector</code> 的显著区别是：<code v-pre>ArrayList</code> 是线程不安全的，当多个线程访问同一个 <code v-pre>ArrayList</code> 集合时，如果有超过一个线程修改了 <code v-pre>ArrayList</code> 集合，则程序必须手动保证该集合的同步性；但 <code v-pre>Vector</code> 集合则是线程安全的，无须程序保证该集合的同步性。</p>
<p>因为 <code v-pre>Vector</code> 是线程安全的，所以 <code v-pre>Vector</code> 的性能比 <code v-pre>ArrayList</code> 的性能要低。实际上，即使需要保证 <code v-pre>List</code> 集合线程安全，也同样不推荐使用 <code v-pre>Vector</code> 实现类。</p>
<p><code v-pre>Vector</code> 还提供了一个 <code v-pre>Stack</code> 子类，它用于模拟“栈”这种数据结构，“栈”通常是指“后进先出”（LIFO）的容器。最后“push”进栈的元素，将最先被“pop”出栈。</p>
<p>与 Java 中的其他集合一样，进栈出栈的都是 <code v-pre>Object</code>，因此从栈中取出元素后必须进行类型转换，除非你只是使用 <code v-pre>Object</code> 具有的操作。</p>
<p>所以 <code v-pre>Stack</code> 类里提供了如下几个方法。</p>
<blockquote>
<ol>
<li><code v-pre>Object peek()</code>：返回“栈”的第一个元素，但并不将该元素“pop”出栈。</li>
<li><code v-pre>Object pop()</code>：返回“栈”的第一个元素，并将该元素“pop”出栈。</li>
<li><code v-pre>void push(Object item)</code>：将一个元素“push”进栈，最后一个进“栈”的元素总是位于“栈”顶。</li>
</ol>
</blockquote>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">VectorTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>由于 <code v-pre>Stack</code> 继承了 <code v-pre>Vector</code>，因此它也是一个非常古老的 Java 集合类，它是线程安全的，性能比较差，因此现在的程序中一般较少使用 <code v-pre>Stack</code> 类。如果程序需要使用“栈”这种数据结构，则可以考虑使用 <code v-pre>LinkedList</code>。</p>
<p><code v-pre>LinkedList</code> 也是 <code v-pre>List</code> 的实现类，它是一个基于链表实现的 <code v-pre>List</code> 类，对于顺序访问集合中的元素进行了优化，特别是插入、删除元素时速度非常快。<code v-pre>LinkedList</code> 既实现了 <code v-pre>List</code> 接口，也实现了 <code v-pre>Deque</code> 接口，由于实现了 <code v-pre>Deque</code> 接口，因此可以作为栈来使用。</p>
<h3 id="固定长度的-list" tabindex="-1"><a class="header-anchor" href="#固定长度的-list" aria-hidden="true">#</a> 固定长度的 <code v-pre>List</code></h3>
<p>讲数组时介绍了一个操作数组的工具类：<code v-pre>Arrays</code>，该工具类里提供了 <code v-pre>asList(Object... a)</code> 方法，该方法可以把一个数组或指定个数的对象转换成一个 <code v-pre>List</code> 集合，这个 <code v-pre>List</code> 集合既不是 <code v-pre>ArrayList</code> 实现类的实例，也不是 <code v-pre>Vector</code> 实现类的实例，而是 <code v-pre>Arrays</code> 的内部类 <code v-pre>ArrayList</code> 的实例。</p>
<p><code v-pre>Arrays.ArrayList</code> 是一个固定长度的 <code v-pre>List</code> 集合，程序只能遍历访问该集合里的元素，不可增加、删除该集合里的元素。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">FixedSizeList</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="queue-集合" tabindex="-1"><a class="header-anchor" href="#queue-集合" aria-hidden="true">#</a> <code v-pre>Queue</code> 集合</h2>
<p><code v-pre>Queue</code> 用于模拟队列这种数据结构，队列通常是指“先进先出”（FIFO）的容器。队列的头部保存在队列中存放时间最长的元素，队列的尾部保存在队列中存放时间最短的元素。新元素插入（offer）到队列的尾部，访问元素（poll）操作会返回队列头部的元素。通常，队列不允许随机访问队列中的元素。</p>
<p><code v-pre>Queue</code> 接口中定义了如下几个方法。</p>
<blockquote>
<ol>
<li><code v-pre>void add(Object e)</code>：将指定元素加入此队列的尾部。</li>
<li><code v-pre>Object element()</code>：获取队列头部的元素，但是不删除该元素。</li>
<li><code v-pre>boolean offer(Object e)</code>：将指定元素加入此队列的尾部。当使用有容量限制的队列时，此方法通常比 <code v-pre>add(Object e)</code> 方法更好。</li>
<li><code v-pre>Object peek()</code>：获取队列头部的元素，但是不删除该元素。如果此队列为空，则返回 null。</li>
<li><code v-pre>Object poll()</code>：获取队列头部的元素，并删除该元素。如果此队列为空，则返回 null。</li>
<li><code v-pre>Object remove()</code>：获取队列头部的元素，并删除该元素。</li>
</ol>
</blockquote>
<h3 id="priorityqueue-实现类" tabindex="-1"><a class="header-anchor" href="#priorityqueue-实现类" aria-hidden="true">#</a> <code v-pre>PriorityQueue</code> 实现类</h3>
<p><code v-pre>riorityQueue</code> 是一个比较标准的队列实现类。之所以说它是比较标准的队列实现，而不是绝对标准的队列实现，是因为 <code v-pre>PriorityQueue</code> 保存队列元素的顺序并不是按加入队列的顺序，而是按队列元素的大小进行重新排序。</p>
<p>因此当调用 <code v-pre>peek()</code> 方法或者 <code v-pre>poll()</code> 方法取出队列中的元素时，并不是取出最先进入队列的元素，而是取出队列中最小的元素。</p>
<p>从这个意义上来看，<code v-pre>PriorityQueue</code> 已经违反了队列的最基本规则：先进先出（FIFO）。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">PriorityQueueTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>运行上面程序直接输出 <code v-pre>PriorityQueue</code> 集合时，可能看到该队列里的元素并没有很好地按大小进行排序，但这只是受到 <code v-pre>PriorityQueue</code> 的 <code v-pre>toString()</code> 方法的返回值的影响。</p>
<p>实际上，程序多次调用 <code v-pre>PriorityQueue</code> 集合对象的 <code v-pre>poll()</code> 方法，即可看到元素按从小到大的顺序“移出队列”。</p>
<p><code v-pre>PriorityQueue</code> 不允许插入 <code v-pre>null</code> 元素，它还需要对队列元素进行排序，<code v-pre>PriorityQueue</code> 的元素有两种排序方式。</p>
<blockquote>
<ol>
<li>自然排序：采用自然顺序的 <code v-pre>PriorityQueue</code> 集合中的元素必须实现了 <code v-pre>Comparable</code> 接口，而且应该是同一个类的多个实例，否则可能导致 <code v-pre>ClassCastException</code> 异常。</li>
<li>定制排序：创建 <code v-pre>PriorityQueue</code> 队列时，传入一个 <code v-pre>Comparator</code> 对象，该对象负责对队列中的所有元素进行排序。采用定制排序时不要求队列元素实现 <code v-pre>Comparable</code> 接口。</li>
</ol>
</blockquote>
<p><code v-pre>PriorityQueue</code> 队列对元素的要求与 <code v-pre>TreeSet</code> 对元素的要求基本一致。</p>
<h3 id="deque-与-arraydeque" tabindex="-1"><a class="header-anchor" href="#deque-与-arraydeque" aria-hidden="true">#</a> <code v-pre>Deque</code> 与 <code v-pre>ArrayDeque</code></h3>
<p><code v-pre>Deque</code> 接口是 <code v-pre>Queue</code> 接口的子接口，它代表一个双端队列，<code v-pre>Deque</code> 接口里定义了一些双端队列的方法，这些方法允许从两端来操作队列的元素。</p>
<blockquote>
<ol>
<li><code v-pre>void addFirst(Object e)</code>：将指定元素插入该双端队列的开头。</li>
<li><code v-pre>void addLast(Object e)</code>：将指定元素插入该双端队列的末尾。</li>
<li><code v-pre>Iterator descendingIterator()</code>：返回该双端队列对应的迭代器，该迭代器将以逆向顺序来迭代队列中的元素。</li>
<li><code v-pre>Object getFirst()</code>：获取但不删除双端队列的第一个元素。</li>
<li><code v-pre>Object getLast()</code>：获取但不删除双端队列的最后一个元素。</li>
<li><code v-pre>boolean offerFirst(Object e)</code>：将指定元素插入该双端队列的开头。</li>
<li><code v-pre>boolean offerLast(Object e)</code>：将指定元素插入该双端队列的末尾。</li>
<li><code v-pre>Object peekFirst()</code>：获取但不删除该双端队列的第一个元素；如果此双端队列为空，则返回 null。</li>
<li><code v-pre>Object peekLast()</code>：获取但不删除该双端队列的最后一个元素；如果此双端队列为空，则返回 null。</li>
<li><code v-pre>Object pollFirst()</code>：获取并删除该双端队列的第一个元素；如果此双端队列为空，则返回 null。</li>
<li><code v-pre>Object pollLast()</code>：获取并删除该双端队列的最后一个元素；如果此双端队列为空，则返回 null。</li>
<li><code v-pre>Object pop()</code>（栈方法）：pop 出该双端队列所表示的栈的栈顶元素。相当于 <code v-pre>removeFirst()</code>。</li>
<li><code v-pre>void push(Object e)</code>（栈方法）：将一个元素 push 进该双端队列所表示的栈的栈顶。相当于 <code v-pre>addFirst(e)</code>。</li>
<li><code v-pre>Object removeFirst()</code>：获取并删除该双端队列的第一个元素。</li>
<li><code v-pre>Object removeFirstOccurrence(Object o)</code>：删除该双端队列的第一次出现的元素 o。</li>
<li><code v-pre>removeLast()</code>：获取并删除该双端队列的最后一个元素。</li>
<li><code v-pre>removeLastOccurrence(Object o)</code>：删除该双端队列的最后一次出现的元素 o。</li>
</ol>
</blockquote>
<p><code v-pre>Deque</code> 不仅可以当成双端队列使用，而且可以被当成栈来使用，因为该类里还包含了 pop（出栈）、push（入栈）两个方法。</p>
<p><code v-pre>Deque</code> 的方法与 <code v-pre>Queue</code> 的方法对照表。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208111345270.jpeg" alt="img"></p>
<p><code v-pre>Deque</code> 的方法与 <code v-pre>Stack</code> 的方法对照表。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208111345962.jpeg" alt="img"></p>
<p><code v-pre>Deque</code> 接口提供了一个典型的实现类：<code v-pre>ArrayDeque</code>，从该名称就可以看出，它是一个基于数组实现的双端队列，创建 <code v-pre>Deque</code> 时同样可指定一个 <code v-pre>numElements</code> 参数，该参数用于指定 <code v-pre>Object[]</code> 数组的长度；如果不指定 <code v-pre>numElements</code> 参数，<code v-pre>Deque</code> 底层数组的长度为 16。</p>
<div class="custom-container tip">
<p class="custom-container-title">提示</p>
<p><code v-pre>ArrayList</code> 和 <code v-pre>ArrayDeque</code> 两个集合类的实现机制基本相似，它们的底层都采用一个动态的、可重分配的 <code v-pre>Object[]</code> 数组来存储集合元素，当集合元素超出了该数组的容量时，系统会在底层重新分配一个 <code v-pre>Object[]</code> 数组来存储集合元素。</p>
</div>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ArrayDequeTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上面程序的运行结果与前面使用 <code v-pre>Stack</code> 的运行结果相似，不过使用 <code v-pre>ArrayDeque</code> 的性能会更加出色，因此现在的程序中需要使用“栈”这种数据结构时，推荐使用 <code v-pre>ArrayDeque</code> 或 <code v-pre>LinkedList</code>，而不是 <code v-pre>Stack</code>。</p>
<h3 id="linkedlist-实现类" tabindex="-1"><a class="header-anchor" href="#linkedlist-实现类" aria-hidden="true">#</a> <code v-pre>LinkedList</code> 实现类</h3>
<p><code v-pre>LinkedList</code> 类是 <code v-pre>List</code> 接口的实现类——这意味着它是一个 <code v-pre>List</code> 集合，可以根据索引来随机访问集合中的元素。除此之外，<code v-pre>LinkedList</code> 还实现了 <code v-pre>Deque</code> 接口，因此它可以被当成双端队列来使用，自然也可以被当成“栈”来使用了。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">LinkedListTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>LinkedList</code> 与 <code v-pre>ArrayList</code>、<code v-pre>ArrayDeque</code> 的实现机制完全不同，<code v-pre>ArrayList</code>、<code v-pre>ArrayDeque</code> 内部以数组的形式来保存集合中的元素，因此随机访问集合元素时有较好的性能；而 <code v-pre>LinkedList</code> 内部以链表的形式来保存集合中的元素，因此随机访问集合元素时性能较差，但在插入、删除元素时性能非常出色（只需改变指针所指的地址即可）。</p>
<p>需要指出的是，虽然 <code v-pre>Vector</code> 也是以数组的形式来存储集合元素的，但因为它实现了线程同步功能，所以各方面性能都有所下降。</p>
<div class="custom-container tip">
<p class="custom-container-title">提示</p>
<p>对于所有的内部基于数组的集合实现，例如 <code v-pre>ArrayList</code>、<code v-pre>ArrayDeque</code> 等，使用随机访问的性能比使用 <code v-pre>Iterator</code> 迭代访问的性能要好，因为随机访问会被映射成对数组元素的访问。</p>
</div>
<h3 id="各种线性表的性能分析" tabindex="-1"><a class="header-anchor" href="#各种线性表的性能分析" aria-hidden="true">#</a> 各种线性表的性能分析</h3>
<p>Java 提供的 <code v-pre>List</code> 就是一个线性表接口，而 <code v-pre>ArrayList</code>、<code v-pre>LinkedList</code> 又是线性表的两种典型实现：基于数组的线性表和基于链的线性表。</p>
<p><code v-pre>Queue</code> 代表了队列，<code v-pre>Deque</code> 代表了双端队列（既可作为队列使用，也可作为栈使用），接下来对各种实现类的性能进行分析。</p>
<p>初学者可以无须理会 <code v-pre>ArrayList</code> 和 <code v-pre>LinkedList</code> 之间的性能差异，只需要知道 <code v-pre>LinkedList</code> 集合不仅提供了 <code v-pre>List</code> 的功能，还提供了双端队列、栈的功能就行。</p>
<p>但在一些性能非常敏感的地方，可能需要慎重选择哪个 <code v-pre>List</code> 实现，下表列出了数组、<code v-pre>ArrayList</code>/<code v-pre>ArrayDeque</code>、<code v-pre>Vector</code>、<code v-pre>LinkedList</code> 的性能差异。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208111352180.jpeg" alt="img"></p>
<p>因为数组以一块连续内存区来保存所有的数组元素，所以数组在随机访问时性能最好。</p>
<p>所有的内部以数组作为底层实现的集合在随机访问时性能较好；而内部以链表作为底层实现的集合在执行插入、删除操作时有很好的性能；进行迭代操作时，以链表作为底层实现的集合比以数组作为底层实现的集合性能好。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">PerformanceTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上面程序中的 <code v-pre>currentTimeMillis</code> 用以获取系统当前时间——这个时间以 <code v-pre>long</code> 整数的形式返回，通常取得这种时间用于统计程序中一段代码的执行时间。</p>
<p>多次运行上面程序会发现，迭代 <code v-pre>ArrayList</code> 集合的时间略大于迭代 <code v-pre>LinkedList</code> 集合的时间。</p>
<p>因此，关于使用 <code v-pre>List</code> 集合有如下建议。</p>
<blockquote>
<ol>
<li>如果需要遍历 <code v-pre>List</code> 集合元素，对于 <code v-pre>ArrayList</code>、<code v-pre>Vector</code> 集合，应该使用随机访问方法 <code v-pre>get</code> 来遍历集合元素，这样性能更好；对于 <code v-pre>LinkedList</code> 集合，则应该采用迭代器 <code v-pre>Iterator</code> 来遍历集合元素。</li>
<li>如果需要经常执行插入、删除操作来改变 <code v-pre>List</code> 集合的大小，则应该使用 <code v-pre>LinkedList</code> 集合，而不是 <code v-pre>ArrayList</code>。使用 <code v-pre>ArrayList</code>、<code v-pre>Vector</code> 集合需要经常重新分配内部数组的大小，其时间开销常常是使用 <code v-pre>LinkedList</code> 的时间开销的几十倍，效果很差。</li>
<li>如果有多个线程需要同时访问 <code v-pre>List</code> 集合中的元素，开发者可考虑使用 <code v-pre>Collections</code> 将集合包装成线程安全的集合。</li>
</ol>
</blockquote>
<h2 id="map" tabindex="-1"><a class="header-anchor" href="#map" aria-hidden="true">#</a> <code v-pre>Map</code></h2>
<p><code v-pre>Map</code> 用于保存具有映射关系的数据，因此 <code v-pre>Map</code> 集合里保存着两组值，一组值用于保存 <code v-pre>Map</code> 里的 <code v-pre>key</code>，另外一组值用于保存 <code v-pre>Map</code> 里的 <code v-pre>value</code>，<code v-pre>key</code> 和 <code v-pre>value</code> 都可以是任何引用类型的数据。<code v-pre>Map</code> 的 <code v-pre>key</code> 不允许重复，即同一个 <code v-pre>Map</code> 对象的任何两个 <code v-pre>key</code> 通过 <code v-pre>equals</code> 方法比较总是返回 <code v-pre>false</code>。</p>
<p><code v-pre>key</code> 和 <code v-pre>value</code> 之间存在单向一对一关系，即通过指定的 <code v-pre>key</code>，总能找到唯一的、确定的 <code v-pre>value</code>。从 <code v-pre>Map</code> 中取出数据时，只要给出指定的 <code v-pre>key</code>，就可以取出对应的 <code v-pre>value</code>。</p>
<p>如果把 <code v-pre>Map</code> 的两组值拆开来看，<code v-pre>Map</code> 里的数据有如图所示的结构。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208111411344.jpeg" alt="img"></p>
<p>如果把 <code v-pre>Map</code> 里的所有 <code v-pre>key</code> 放在一起来看，它们就组成了一个 <code v-pre>Set</code> 集合（所有的 <code v-pre>key</code> 没有顺序，<code v-pre>key</code> 与 <code v-pre>key</code> 之间不能重复），实际上 <code v-pre>Map</code> 确实包含了一个 <code v-pre>keySet()</code> 方法，用于返回 <code v-pre>Map</code> 里所有 <code v-pre>key</code> 组成的 <code v-pre>Set</code> 集合。</p>
<p>不仅如此，<code v-pre>Map</code> 里 <code v-pre>key</code> 集和 <code v-pre>Set</code> 集合里元素的存储形式也很像，<code v-pre>Map</code> 子类和 <code v-pre>Set</code> 子类在名字上也惊人地相似，比如 <code v-pre>Set</code> 接口下有 <code v-pre>HashSet</code>、<code v-pre>LinkedHashSet</code>、<code v-pre>SortedSet</code>（接口）、<code v-pre>TreeSet</code>、<code v-pre>EnumSet</code> 等子接口和实现类，而 <code v-pre>Map</code> 接口下则有 <code v-pre>HashMap</code>、<code v-pre>LinkedHashMap</code>、<code v-pre>SortedMap</code>（接口）、<code v-pre>TreeMap</code>、<code v-pre>EnumMap</code> 等子接口和实现类。</p>
<p>正如它们的名字所暗示的，<code v-pre>Map</code> 的这些实现类和子接口中 <code v-pre>key</code> 集的存储形式和对应 <code v-pre>Set</code> 集合中元素的存储形式完全相同。</p>
<div class="custom-container info">
<p class="custom-container-title">相关信息</p>
<p><code v-pre>Set</code> 与 <code v-pre>Map</code> 之间的关系非常密切。</p>
<p>虽然 <code v-pre>Map</code> 中放的元素是 <code v-pre>key-value</code> 对，<code v-pre>Set</code> 集合中放的元素是单个对象，但如果我们把 <code v-pre>key-value</code> 对中的 <code v-pre>value</code> 当成 <code v-pre>key</code> 的附庸：<code v-pre>key</code> 在哪里，<code v-pre>value</code> 就跟在哪里。这样就可以像对待 <code v-pre>Set</code> 一样来对待 <code v-pre>Map</code> 了。</p>
<p>事实上，<code v-pre>Map</code> 提供了一个 <code v-pre>Entry</code> 内部类来封装 <code v-pre>key-value</code> 对，而计算 <code v-pre>Entry</code> 存储时则只考虑 <code v-pre>Entry</code> 封装的 <code v-pre>key</code>。</p>
<p>从 Java 源码来看， Java 是先实现了 <code v-pre>Map</code>，然后通过包装一个所有 <code v-pre>value</code> 都为 <code v-pre>null</code> 的 <code v-pre>Map</code> 就实现了 <code v-pre>Set</code> 集合。</p>
</div>
<p>如果把 <code v-pre>Map</code> 里的所有 <code v-pre>value</code> 放在一起来看，它们又非常类似于一个 <code v-pre>List</code>：</p>
<p>元素与元素之间可以重复，每个元素可以根据索引来查找，只是 <code v-pre>Map</code> 中的索引不再使用整数值，而是以另一个对象作为索引。</p>
<p>如果需要从 <code v-pre>List</code> 集合中取出元素，则需要提供该元素的数字索引；如果需要从 <code v-pre>Map</code> 中取出元素，则需要提供该元素的 <code v-pre>key</code> 索引。</p>
<p>因此，<code v-pre>Map</code> 有时也被称为字典，或关联数组。</p>
<p><code v-pre>Map</code> 接口中定义了如下常用的方法。</p>
<blockquote>
<ol>
<li><code v-pre>void clear()</code>：删除该 <code v-pre>Map</code> 对象中的所有 <code v-pre>key-value</code> 对。</li>
<li><code v-pre>boolean containsKey(Object key)</code>：查询 <code v-pre>Map</code> 中是否包含指定的 <code v-pre>key</code>，如果包含则返回 <code v-pre>true</code>。</li>
<li><code v-pre>boolean containsValue(Object value)</code>：查询 <code v-pre>Map</code> 中是否包含一个或多个 <code v-pre>value</code>，如果包含则返回 <code v-pre>true</code>。</li>
<li><code v-pre>Set entrySet()</code>：返回 <code v-pre>Map</code> 中包含的 <code v-pre>key-value</code> 对所组成的 <code v-pre>Set</code> 集合，每个集合元素都是 <code v-pre>Map.Entry</code> （<code v-pre>Entry</code> 是 <code v-pre>Map</code> 的内部类）对象。</li>
<li><code v-pre>Object get(Object key)</code>：返回指定 <code v-pre>key</code> 所对应的 <code v-pre>value</code>；如果此 <code v-pre>Map</code> 中不包含该 <code v-pre>key</code>，则返回 <code v-pre>null</code>。</li>
<li><code v-pre>boolean isEmpty()</code>：查询该 <code v-pre>Map</code> 是否为空（即不包含任何 <code v-pre>key-value</code> 对），如果为空则返回 <code v-pre>true</code>。</li>
<li><code v-pre>Set keySet()</code>：返回该 <code v-pre>Map</code> 中所有 <code v-pre>key</code> 组成的 <code v-pre>Set</code> 集合。</li>
<li><code v-pre>Object put(Object key, Object value)</code>：添加一个 <code v-pre>key-value</code> 对，如果当前 <code v-pre>Map</code> 中已有一个与该 <code v-pre>key</code> 相等的 <code v-pre>key-value</code> 对，则新的 <code v-pre>key-value</code> 对会覆盖原来的 <code v-pre>key-value</code> 对。</li>
<li><code v-pre>void putAll(Map m)</code>：将指定 <code v-pre>Map</code> 中的 <code v-pre>key-value</code> 对复制到本 <code v-pre>Map</code> 中。</li>
<li><code v-pre>Object remove(Object key)</code>：删除指定 <code v-pre>key</code> 所对应的 <code v-pre>key-value</code> 对，返回被删除 <code v-pre>key</code> 所关联的 <code v-pre>value</code>，如果该 <code v-pre>key</code> 不存在，则返回 <code v-pre>null</code>。</li>
<li><code v-pre>int size()</code>：返回该 <code v-pre>Map</code> 里的 <code v-pre>key-value</code> 对的个数。</li>
<li><code v-pre>Collection values()</code>：返回该 <code v-pre>Map</code> 里所有 <code v-pre>value</code> 组成的 <code v-pre>Collection</code>。</li>
</ol>
</blockquote>
<p><code v-pre>Map</code> 接口提供了大量的实现类，典型实现如 <code v-pre>HashMap</code> 和 <code v-pre>Hashtable</code> 等、<code v-pre>HashMap</code> 的子类 <code v-pre>LinkedHashMap</code>，还有 <code v-pre>SortedMap</code> 子接口及该接口的实现类 <code v-pre>TreeMap</code>，以及 <code v-pre>WeakHashMap</code>、<code v-pre>IdentityHashMap</code> 等。</p>
<p>下面将详细介绍 <code v-pre>Map</code> 接口实现类。</p>
<p><code v-pre>Map</code> 中包括一个内部类 <code v-pre>Entry</code>，该类封装了一个 <code v-pre>key-value</code> 对。</p>
<p><code v-pre>Entry</code> 包含如下三个方法。</p>
<blockquote>
<ol>
<li><code v-pre>Object getKey()</code>：返回该 Entry 里包含的 key 值。</li>
<li><code v-pre>Object getValue()</code>：返回该 Entry 里包含的 value 值。</li>
<li><code v-pre>Object setValue(V value)</code>：设置该 Entry 里包含的 value 值，并返回新设置的 value 值。</li>
</ol>
</blockquote>
<h3 id="hashmap-和-hashtable" tabindex="-1"><a class="header-anchor" href="#hashmap-和-hashtable" aria-hidden="true">#</a> <code v-pre>HashMap</code> 和 <code v-pre>Hashtable</code></h3>
<p><code v-pre>HashMap</code> 和 <code v-pre>Hashtable</code> 都是 <code v-pre>Map</code> 接口的典型实现类，它们之间的关系完全类似于 <code v-pre>ArrayList</code> 和 <code v-pre>Vector</code> 的关系：<code v-pre>Hashtable</code> 是一个古老的 <code v-pre>Map</code> 实现类，它从 JDK 1.0 起就已经出现了，当它出现时，Java 还没有提供 <code v-pre>Map</code> 接口，所以它包含了两个烦琐的方法，即 <code v-pre>elements()</code>（类似于 <code v-pre>Map</code> 接口定义的 <code v-pre>values()</code> 方法）和 <code v-pre>keys()</code>（类似于 <code v-pre>Map</code> 接口定义的 <code v-pre>keySet()</code> 方法），现在很少使用这两个方法。</p>
<p>除此之外，<code v-pre>Hashtable</code> 和 <code v-pre>HashMap</code> 存在两点典型区别。</p>
<blockquote>
<ol>
<li><code v-pre>Hashtable</code> 是一个线程安全的 Map 实现，但 <code v-pre>HashMap</code> 是线程不安全的实现，所以 <code v-pre>HashMap</code> 比 <code v-pre>Hashtable</code> 的性能高一点；但如果有多个线程访问同一个 <code v-pre>Map</code> 对象时，使用 <code v-pre>Hashtable</code> 实现类会更好。</li>
<li><code v-pre>Hashtable</code> 不允许使用 <code v-pre>null</code> 作为 <code v-pre>key</code> 和 <code v-pre>value</code>，如果试图把 <code v-pre>null</code> 值放进 <code v-pre>Hashtable</code> 中，将会引发 <code v-pre>NullPointerException</code> 异常；但 <code v-pre>HashMap</code> 可以使用 <code v-pre>null</code> 作为 <code v-pre>key</code> 或 <code v-pre>value</code>。</li>
</ol>
</blockquote>
<p>由于 <code v-pre>HashMap</code> 里的 <code v-pre>key</code> 不能重复，所以 <code v-pre>HashMap</code> 里最多只有一个 <code v-pre>key-value</code> 对的 <code v-pre>key</code> 为 <code v-pre>null</code>，但可以有无数多个 <code v-pre>key-value</code> 对的 <code v-pre>value</code> 为 <code v-pre>null</code>。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">NullInHashMap</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token comment">/*
 {null=null, a=null}
*/</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>所有的 <code v-pre>Map</code> 实现类都重写了 <code v-pre>toString()</code> 方法，调用 <code v-pre>Map</code> 对象的 <code v-pre>toString()</code> 方法总是返回如下格式的字符串：<code v-pre>{key1=value1,key2=value2...}</code>。</p>
<div class="custom-container info">
<p class="custom-container-title">相关信息</p>
<p>从 <code v-pre>Hashtable</code> 的类名上就可以看出它是一个古老的类，它的命名甚至没有遵守 Java 的命名规范：每个单词的首字母都应该大写。</p>
<p>也许当初开发 <code v-pre>Hashtable</code> 的工程师也没有注意到这一点，后来大量 Java 程序中使用了 <code v-pre>Hashtable</code> 类，所以这个类名也就不能改为 <code v-pre>HashTable</code> 了，否则将导致大量程序需要改写。</p>
<p>与 <code v-pre>Vector</code> 类似的是，尽量少用 <code v-pre>Hashtable</code> 实现类，即使需要创建线程安全的 <code v-pre>Map</code> 实现类，也无须使用 <code v-pre>Hashtable</code> 实现类</p>
</div>
<p>为了成功地在 <code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 中存储、获取对象，用作 <code v-pre>key</code> 的对象必须实现 <code v-pre>hashCode()</code> 方法和 <code v-pre>equals()</code> 方法。</p>
<p>与 <code v-pre>HashSet</code> 集合不能保证元素的顺序一样，<code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 也不能保证其中 <code v-pre>key-value</code> 对的顺序。类似于 <code v-pre>HashSet</code>，<code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 判断两个 <code v-pre>key</code> 相等的标准也是：两个 <code v-pre>key</code> 通过 <code v-pre>equals()</code> 方法比较返回 <code v-pre>true</code>，两个 <code v-pre>key</code> 的 <code v-pre>hashCode</code> 值也相等。</p>
<p>除此之外，<code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 中还包含一个 <code v-pre>containsValue()</code> 方法，用于判断是否包含指定的 <code v-pre>value</code>。</p>
<p>那么 <code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 如何判断两个 <code v-pre>value</code> 相等呢？</p>
<p><code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 判断两个 <code v-pre>value</code> 相等的标准更简单：只要两个对象通过 <code v-pre>equals()</code> 方法比较返回 <code v-pre>true</code> 即可。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">class</span> <span class="token class-name">A3</span> <span class="token punctuation">{</span>
    <span class="token keyword">int</span> count<span class="token punctuation">;</span>
...
<p class="custom-container-title">提示</p>
<p>当使用自定义类作为 <code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 的 <code v-pre>key</code> 时，如果重写该类的 <code v-pre>equals(Object obj)</code> 和 <code v-pre>hashCode()</code> 方法，则应该保证两个方法的判断标准一致——当两个 <code v-pre>key</code> 通过 <code v-pre>equals()</code> 方法比较返回 <code v-pre>true</code> 时，两个 <code v-pre>key</code> 的 <code v-pre>hashCode()</code> 返回值也应该相同。</p>
<p>因为 <code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 保存 <code v-pre>key</code> 的方式与 <code v-pre>HashSet</code> 保存集合元素的方式完全相同，所以 <code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 对 <code v-pre>key</code> 的要求与 <code v-pre>HashSet</code> 对集合元素的要求完全相同。</p>
</div>
<p>与 <code v-pre>HashSet</code> 类似的是，如果使用可变对象作为 <code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 的 <code v-pre>key</code>，并且程序修改了作为 <code v-pre>key</code> 的可变对象，则也可能出现与 <code v-pre>HashSet</code> 类似的情形：程序再也无法准确访问到 <code v-pre>Map</code> 中被修改过的 <code v-pre>key</code>。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">HashtableErrorTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token class-name">Hashtable</span> ht <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">Hashtable</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
...
<p class="custom-container-title">提示</p>
<p>与 <code v-pre>HashSet</code> 类似的是，尽量不要使用可变对象作为 <code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 的 <code v-pre>key</code>，如果确实需要使用可变对象作为 <code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 的 <code v-pre>key</code>，则尽量不要在程序中修改作为 <code v-pre>key</code> 的可变对象。</p>
</div>
<h3 id="linkedhashmap-实现类" tabindex="-1"><a class="header-anchor" href="#linkedhashmap-实现类" aria-hidden="true">#</a> <code v-pre>LinkedHashMap</code> 实现类</h3>
<p><code v-pre>HashSet</code> 有一个子类是 <code v-pre>LinkedHashSet</code>，<code v-pre>HashMap</code> 也有一个 <code v-pre>LinkedHashMap</code> 子类；<code v-pre>LinkedHashMap</code> 也使用双向链表来维护 <code v-pre>key-value</code> 对的次序（其实只需要考虑 <code v-pre>key</code> 的次序），该链表负责维护 <code v-pre>Map</code> 的迭代顺序，迭代顺序与 <code v-pre>key-value</code> 对的插入顺序保持一致。</p>
<p><code v-pre>LinkedHashMap</code> 可以避免对 <code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 里的 <code v-pre>key-value</code> 对进行排序（只要插入 <code v-pre>key-value</code> 对时保持顺序即可），同时又可避免使用 <code v-pre>TreeMap</code> 所增加的成本。</p>
<p><code v-pre>LinkedHashMap</code> 需要维护元素的插入顺序，因此性能略低于 <code v-pre>HashMap</code> 的性能；但因为它以链表来维护内部顺序，所以在迭代访问 <code v-pre>Map</code> 里的全部元素时将有较好的性能。</p>
<p>下面程序示范了 <code v-pre>LinkedHashMap</code> 的功能：</p>
<p>迭代输出 <code v-pre>LinkedHashMap</code> 的元素时，将会按添加 <code v-pre>key-value</code> 对的顺序输出。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">LinkedHashMapTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="properties" tabindex="-1"><a class="header-anchor" href="#properties" aria-hidden="true">#</a> <code v-pre>Properties</code></h3>
<p><code v-pre>Properties</code> 类是 <code v-pre>Hashtable</code> 类的子类，正如它的名字所暗示的，该对象在处理属性文件时特别方便（Windows 操作平台上的 <code v-pre>ini</code> 文件就是一种属性文件）。</p>
<p><code v-pre>Properties</code> 类可以把 <code v-pre>Map</code> 对象和属性文件关联起来，从而可以把 <code v-pre>Map</code> 对象中的 <code v-pre>key-value</code> 对写入属性文件中，也可以把属性文件中的“属性名=属性值”加载到 <code v-pre>Map</code> 对象中。由于属性文件里的属性名、属性值只能是字符串类型，所以 <code v-pre>Properties</code> 里的 <code v-pre>key</code>、<code v-pre>value</code> 都是字符串类型。</p>
<p>该类提供了如下三个方法来修改 <code v-pre>Properties</code> 里的 <code v-pre>key</code>、<code v-pre>value</code> 值。</p>
<blockquote>
<ol>
<li><code v-pre>String getProperty(String key)</code>：获取 <code v-pre>Properties</code> 中指定属性名对应的属性值，类似于 <code v-pre>Map</code> 的 <code v-pre>get(Object key)</code> 方法。</li>
<li><code v-pre>String getProperty(String key, String defaultValue)</code>：该方法与前一个方法基本相似。该方法多一个功能，如果 <code v-pre>Properties</code> 中不存在指定的 <code v-pre>key</code> 时，则该方法指定默认值。</li>
<li><code v-pre>Object setProperty(String key, String value)</code>：设置属性值，类似于 <code v-pre>Hashtable</code> 的 <code v-pre>put()</code> 方法。</li>
</ol>
</blockquote>
<p>除此之外，它还提供了两个读写 Field 文件的方法。</p>
<ol>
<li><code v-pre>void load(InputStream inStream)</code>：从属性文件（以输入流表示）中加载 <code v-pre>key-value</code> 对，把加载到的 <code v-pre>key-value</code> 对追加到 <code v-pre>Properties</code> 里（<code v-pre>Properties</code> 是 <code v-pre>Hashtable</code> 的子类，它不保证 <code v-pre>key-value</code> 对之间的次序）。</li>
<li><code v-pre>void store(OutputStream out, String comments)</code>：将 <code v-pre>Properties</code> 中的 <code v-pre>key-value</code> 对输出到指定的属性文件（以输出流表示）中。</li>
</ol>
<div class="custom-container tip">
<p class="custom-container-title">提示</p>
<p>上面两个方法中使用了 <code v-pre>InputStream</code> 类和 <code v-pre>OutputStream</code> 类，它们是 Java <code v-pre>IO</code> 体系中的两个基类。</p>
</div>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">PropertiesTest</span> <span class="token punctuation">{</span>
...
*/</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上面程序还在当前项目路径下生成了一个 <code v-pre>a.ini</code> 文件，该文件的内容如下：</p>
<div class="language-ini line-numbers-mode" data-ext="ini"><pre v-pre class="language-ini"><code><span class="token comment">#comment line</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="sortedmap-和-treemap" tabindex="-1"><a class="header-anchor" href="#sortedmap-和-treemap" aria-hidden="true">#</a> <code v-pre>SortedMap</code> 和 <code v-pre>TreeMap</code></h3>
<p>正如 <code v-pre>Set</code> 接口派生出 <code v-pre>SortedSet</code> 子接口，<code v-pre>SortedSet</code> 接口有一个 <code v-pre>TreeSet</code> 实现类一样，<code v-pre>Map</code> 接口也派生出一个 <code v-pre>SortedMap</code> 子接口，<code v-pre>SortedMap</code> 接口也有一个 <code v-pre>TreeMap</code> 实现类。</p>
<p><code v-pre>TreeMap</code> 就是一个红黑树数据结构，每个 <code v-pre>key-value</code> 对即作为红黑树的一个节点。<code v-pre>TreeMap</code> 存储 <code v-pre>key-value</code> 对（节点）时，需要根据 <code v-pre>key</code> 对节点进行排序。<code v-pre>TreeMap</code> 可以保证所有的 <code v-pre>key-value</code> 对处于有序状态。<code v-pre>TreeMap</code> 也有两种排序方式。</p>
<blockquote>
<ol>
<li>自然排序：<code v-pre>TreeMap</code> 的所有 <code v-pre>key</code> 必须实现 <code v-pre>Comparable</code> 接口，而且所有的 <code v-pre>key</code> 应该是同一个类的对象，否则将会抛出 <code v-pre>ClassCastException</code> 异常。</li>
<li>定制排序：创建 <code v-pre>TreeMap</code> 时，传入一个 <code v-pre>Comparator</code> 对象，该对象负责对 <code v-pre>TreeMap</code> 中的所有 <code v-pre>key</code> 进行排序。采用定制排序时不要求 <code v-pre>Map</code> 的 <code v-pre>key</code> 实现 <code v-pre>Comparable</code> 接口。</li>
</ol>
</blockquote>
<p>类似于 <code v-pre>TreeSet</code> 中判断两个元素相等的标准，<code v-pre>TreeMap</code> 中判断两个 <code v-pre>key</code> 相等的标准是：两个 <code v-pre>key</code> 通过 <code v-pre>compareTo()</code> 方法返回 0，<code v-pre>TreeMap</code> 即认为这两个 <code v-pre>key</code> 是相等的。</p>
<p>如果使用自定义类作为 <code v-pre>TreeMap</code> 的 <code v-pre>key</code>，且想让 <code v-pre>TreeMap</code> 良好地工作，则重写该类的 <code v-pre>equals()</code> 方法和 <code v-pre>compareTo()</code> 方法时应保持一致的返回结果：两个 <code v-pre>key</code> 通过 <code v-pre>equals()</code> 方法比较返回 <code v-pre>true</code> 时，它们通过 <code v-pre>compareTo()</code> 方法比较应该返回 0。如果 <code v-pre>equals()</code> 方法与 <code v-pre>compareTo()</code> 方法的返回结果不一致， <code v-pre>TreeMap</code> 与 <code v-pre>Map</code> 接口的规则就会冲突。</p>
<div class="custom-container tip">
<p class="custom-container-title">提示</p>
<p><code v-pre>Set</code> 和 <code v-pre>Map</code> 的关系十分密切，Java 源码就是先实现了 <code v-pre>HashMap</code>、<code v-pre>TreeMap</code> 等集合，然后通过包装一个所有的 <code v-pre>value</code> 都为 <code v-pre>null</code> 的 <code v-pre>Map</code> 集合实现了 <code v-pre>Set</code> 集合类。</p>
</div>
<p>与 <code v-pre>TreeSet</code> 类似的是，<code v-pre>TreeMap</code> 中也提供了一系列根据 <code v-pre>key</code> 顺序访问 <code v-pre>key-value</code> 对的方法。</p>
<blockquote>
<ol>
<li><code v-pre>Map.Entry firstEntry()</code>：返回该 <code v-pre>Map</code> 中最小 <code v-pre>key</code> 所对应的 <code v-pre>key-value</code> 对，如果该 <code v-pre>Map</code> 为空，则返回 <code v-pre>null</code>。</li>
<li><code v-pre>Object firstKey()</code>：返回该 <code v-pre>Map</code> 中的最小 <code v-pre>key</code> 值，如果该 <code v-pre>Map</code> 为空，则返回 <code v-pre>null</code>。</li>
<li><code v-pre>Map.Entry lastEntry()</code>：返回该 <code v-pre>Map</code> 中最大 <code v-pre>key</code> 所对应的 <code v-pre>key-value</code> 对，如果该 <code v-pre>Map</code> 为空或不存在这样的 <code v-pre>key-value</code> 对，则都返回 <code v-pre>null</code>。</li>
<li><code v-pre>Object lastKey()</code>：返回该 <code v-pre>Map</code> 中的最大 <code v-pre>key</code> 值，如果该 <code v-pre>Map</code> 为空或不存在这样的 <code v-pre>key</code>，则都返回 <code v-pre>null</code>。</li>
<li><code v-pre>Map.Entry higherEntry(Object key)</code>：返回该 <code v-pre>Map</code> 中位于 <code v-pre>key</code> 后一位的 <code v-pre>key-value</code> 对（即大于指定 <code v-pre>key</code> 的最小 <code v-pre>key</code> 所对应的 <code v-pre>key-value</code> 对）。如果该 <code v-pre>Map</code> 为空，则返回 <code v-pre>null</code>。</li>
<li><code v-pre>Object higherKey(Object key)</code>：返回该 <code v-pre>Map</code> 中位于 <code v-pre>key</code> 后一位的 <code v-pre>key</code> 值（即大于指定 <code v-pre>key</code> 的最小 <code v-pre>key</code> 值）。如果该 <code v-pre>Map</code> 为空或不存在这样的 <code v-pre>key-value</code> 对，则都返回 <code v-pre>null</code>。</li>
<li><code v-pre>Map.Entry lowerEntry(Object key)</code>：返回该 <code v-pre>Map</code> 中位于 <code v-pre>key</code> 前一位的 <code v-pre>key-value</code> 对（即小于指定 <code v-pre>key</code> 的最大 <code v-pre>key</code> 所对应的 <code v-pre>key-value</code> 对）。如果该 <code v-pre>Map</code> 为空或不存在这样的 <code v-pre>key-value</code> 对，则都返回 <code v-pre>null</code>。</li>
<li><code v-pre>Object lowerKey(Object key)</code>：返回该 <code v-pre>Map</code> 中位于 <code v-pre>key</code> 前一位的 <code v-pre>key</code> 值（即小于指定 <code v-pre>key</code> 的最大 <code v-pre>key</code> 值）。如果该 <code v-pre>Map</code> 为空或不存在这样的 <code v-pre>key</code>，则都返回 <code v-pre>null</code>。</li>
<li><code v-pre>NavigableMap subMap(Object fromKey, boolean fromInclusive, Object toKey, boolean toInclusive)</code>：返回该 <code v-pre>Map</code> 的子 <code v-pre>Map</code>，其 <code v-pre>key</code> 的范围是从 <code v-pre>fromKey</code>（是否包括取决于第二个参数）到 <code v-pre>toKey</code>（是否包括取决于第四个参数）。</li>
<li><code v-pre>SortedMap subMap(Object fromKey, Object toKey)</code>：返回该 <code v-pre>Map</code> 的子 <code v-pre>Map</code>，其 <code v-pre>key</code> 的范围是从 <code v-pre>fromKey</code>（包括）到 <code v-pre>toKey</code>（不包括）。</li>
<li><code v-pre>SortedMap tailMap(Object fromKey)</code>：返回该 <code v-pre>Map</code> 的子 <code v-pre>Map</code>，其 key 的范围是大于 <code v-pre>fromKey</code>（包括）的所有 <code v-pre>key</code>。</li>
<li><code v-pre>NavigableMap tailMap(Object fromKey, boolean inclusive)</code>：返回该 <code v-pre>Map</code> 的子 <code v-pre>Map</code>，其 <code v-pre>key</code> 的范围是大于 <code v-pre>fromKey</code>（是否包括取决于第二个参数）的所有 <code v-pre>key</code>。</li>
<li><code v-pre>SortedMap headMap(Object toKey)</code>：返回该 <code v-pre>Map</code> 的子 <code v-pre>Map</code>，其 <code v-pre>key</code> 的范围是小于 <code v-pre>toKey</code>（不包括）的所有 key。</li>
<li><code v-pre>NavigableMap headMap(Object toKey, boolean inclusive)</code>：返回该 <code v-pre>Map</code> 的子 <code v-pre>Map</code>，其 <code v-pre>key</code> 的范围是小于 <code v-pre>toKey</code>（是否包括取决于第二个参数）的所有 <code v-pre>key</code>。</li>
</ol>
</blockquote>
<p>以自然排序为例，介绍 <code v-pre>TreeMap</code> 的基本用法。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">class</span> <span class="token class-name">R3</span> <span class="token keyword">implements</span> <span class="token class-name">Comparable</span> <span class="token punctuation">{</span>
    <span class="token keyword">int</span> count<span class="token punctuation">;</span>
...
<span class="token comment">/*
    ...
    {}
*/</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="weakhashmap-实现类" tabindex="-1"><a class="header-anchor" href="#weakhashmap-实现类" aria-hidden="true">#</a> <code v-pre>WeakHashMap</code> 实现类</h3>
<p><code v-pre>WeakHashMap</code> 与 <code v-pre>HashMap</code> 的用法基本相似。与 <code v-pre>HashMap</code> 的区别在于，<code v-pre>HashMap</code> 的 <code v-pre>key</code> 保留了对实际对象的强引用，这意味着只要该 <code v-pre>HashMap</code> 对象不被销毁，该 <code v-pre>HashMap</code> 的所有 <code v-pre>key</code> 所引用的对象就不会被垃圾回收，<code v-pre>HashMap</code> 也不会自动删除这些 <code v-pre>key</code> 所对应的 <code v-pre>key-value</code> 对；但 <code v-pre>WeakHashMap</code> 的 <code v-pre>key</code> 只保留了对实际对象的弱引用，这意味着如果 <code v-pre>WeakHashMap</code> 对象的 <code v-pre>key</code> 所引用的对象没有被其他强引用变量所引用，则这些 <code v-pre>key</code> 所引用的对象可能被垃圾回收，<code v-pre>WeakHashMap</code> 也可能自动删除这些 <code v-pre>key</code> 所对应的 <code v-pre>key-value</code> 对。</p>
<p><code v-pre>WeakHashMap</code> 中的每个 <code v-pre>key</code> 对象只持有对实际对象的弱引用，因此，当垃圾回收了该 <code v-pre>key</code> 所对应的实际对象之后，<code v-pre>WeakHashMap</code> 会自动删除该 <code v-pre>key</code> 对应的 <code v-pre>key-value</code> 对。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">WeakHashMapTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token comment">/*
    ...
    {java=中等}
*/</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>当系统进行垃圾回收时，删除了 <code v-pre>WeakHashMap</code> 对象的前三个 <code v-pre>key-value</code> 对。这是因为添加前三个 <code v-pre>key-value</code> 对时，这三个 <code v-pre>key</code> 都是匿名的字符串对象， <code v-pre>WeakHashMap</code> 只保留了对它们的弱引用，这样垃圾回收时会自动删除这三个 <code v-pre>key-value</code> 对。</p>
<p><code v-pre>WeakHashMap</code> 对象中第 4 个组 <code v-pre>key-value</code> 对的 <code v-pre>key</code> 是一个字符串直接量，（系统会自动保留对该字符串对象的强引用），所以垃圾回收时不会回收它。</p>
<div class="custom-container tip">
<p class="custom-container-title">提示</p>
<p>如果需要使用 <code v-pre>WeakHashMap</code> 的 <code v-pre>key</code> 来保留对象的弱引用，则不要让该 <code v-pre>key</code> 所引用的对象具有任何强引用，否则将失去使用 <code v-pre>WeakHashMap</code> 的意义。</p>
</div>
<h3 id="identityhashmap-实现类" tabindex="-1"><a class="header-anchor" href="#identityhashmap-实现类" aria-hidden="true">#</a> <code v-pre>IdentityHashMap</code> 实现类</h3>
<p>这个 <code v-pre>Map</code> 实现类的实现机制与 <code v-pre>HashMap</code> 基本相似，但它在处理两个 <code v-pre>key</code> 相等时比较独特：在 <code v-pre>IdentityHashMap</code> 中，当且仅当两个 <code v-pre>key</code> 严格相等（<code v-pre>key1==key2</code>）时，<code v-pre>IdentityHashMap</code> 才认为两个 <code v-pre>key</code> 相等；对于普通的 <code v-pre>HashMap</code> 而言，只要 <code v-pre>key1</code> 和 <code v-pre>key2</code> 通过 <code v-pre>equals()</code> 方法比较返回 <code v-pre>true</code>，且它们的 <code v-pre>hashCode</code> 值相等即可。</p>
<div class="custom-container tip">
<p class="custom-container-title">提示</p>
<p><code v-pre>IdentityHashMap</code> 是一个特殊的 <code v-pre>Map</code> 实现！</p>
<p>此类实现 <code v-pre>Map</code> 接口时，它有意违反 <code v-pre>Map</code> 的通常规范：<code v-pre>IdentityHashMap</code> 要求两个 <code v-pre>key</code> 严格相等时才认为两个 <code v-pre>key</code> 相等。</p>
</div>
<p><code v-pre>IdentityHashMap</code> 提供了与 <code v-pre>HashMap</code> 基本相似的方法，也允许使用 <code v-pre>null</code> 作为 <code v-pre>key</code> 和 <code v-pre>value</code>。与 <code v-pre>HashMap</code> 相似：<code v-pre>IdentityHashMap</code> 也不保证 <code v-pre>key-value</code> 对之间的顺序，更不能保证它们的顺序随时间的推移保持不变。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">IdentityHashMapTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token comment">/*
    {java=98, 语文=89, 语文=78}
*/</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="enummap-实现类" tabindex="-1"><a class="header-anchor" href="#enummap-实现类" aria-hidden="true">#</a> <code v-pre>EnumMap</code> 实现类</h3>
<p><code v-pre>EnumMap</code> 是一个与枚举类一起使用的 <code v-pre>Map</code> 实现，<code v-pre>EnumMap</code> 中的所有 <code v-pre>key</code> 都必须是单个枚举类的枚举值。创建 <code v-pre>EnumMap</code> 时必须显式或隐式指定它对应的枚举类。</p>
<p><code v-pre>EnumMap</code> 在内部以数组形式保存，所以这种实现形式非常紧凑、高效。</p>
<p><code v-pre>EnumMap</code> 根据 <code v-pre>key</code> 的自然顺序（即枚举值在枚举类中的定义顺序）来维护 <code v-pre>key-value</code> 对的顺序。当程序通过 <code v-pre>keySet()</code>、<code v-pre>entrySet()</code>、<code v-pre>values()</code> 等方法遍历 <code v-pre>EnumMap</code> 时可以看到这种顺序。</p>
<p><code v-pre>EnumMap</code> 不允许使用 <code v-pre>null</code> 作为 <code v-pre>key</code>，但允许使用 <code v-pre>null</code> 作为 <code v-pre>value</code>。如果试图使用 <code v-pre>null</code> 作为 <code v-pre>key</code> 时将抛出 <code v-pre>NullPointerException</code> 异常。如果只是查询是否包含值为 <code v-pre>null</code> 的 <code v-pre>key</code>，或只是删除值为 <code v-pre>null</code> 的 <code v-pre>key</code>，都不会抛出异常。</p>
<p>与创建普通的 <code v-pre>Map</code> 有所区别的是，创建 <code v-pre>EnumMap</code> 时必须指定一个枚举类，从而将该 <code v-pre>EnumMap</code> 和指定枚举类关联起来。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">enum</span> <span class="token class-name">Season1</span> <span class="token punctuation">{</span>
    <span class="token constant">SPRING</span><span class="token punctuation">,</span> <span class="token constant">SUMMER</span><span class="token punctuation">,</span> <span class="token constant">FALL</span><span class="token punctuation">,</span> <span class="token constant">WINTER</span>
<span class="token punctuation">}</span>
...
*/</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="map-性能分析" tabindex="-1"><a class="header-anchor" href="#map-性能分析" aria-hidden="true">#</a> <code v-pre>Map</code> 性能分析</h3>
<p>对于 <code v-pre>Map</code> 的常用实现类而言，<code v-pre>HashMap</code> 和 <code v-pre>Hashtable</code> 的效率大致相同，因为它们的实现机制几乎完全一样；但 <code v-pre>HashMap</code> 通常比 <code v-pre>Hashtable</code> 要快一点，因为 <code v-pre>Hashtable</code> 需要额外的线程同步控制。</p>
<p><code v-pre>TreeMap</code> 通常比 <code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 要慢（尤其在插入、删除 <code v-pre>key-value</code> 对时更慢），因为 <code v-pre>TreeMap</code> 底层采用红黑树来管理 <code v-pre>key-value</code> 对（红黑树的每个节点就是一个 <code v-pre>key-value</code> 对）。</p>
<p>使用 <code v-pre>TreeMap</code> 有一个好处：<code v-pre>TreeMap</code> 中的 <code v-pre>key-value</code> 对总是处于有序状态，无须专门进行排序操作。当 <code v-pre>TreeMap</code> 被填充之后，就可以调用 <code v-pre>keySet()</code>，取得由 <code v-pre>key</code> 组成的 <code v-pre>Set</code>，然后使用 <code v-pre>toArray()</code> 方法生成 <code v-pre>key</code> 的数组，接下来使用 <code v-pre>Arrays</code> 的 <code v-pre>binarySearch()</code> 方法在已排序的数组中快速地查询对象。</p>
<p>对于一般的应用场景，程序应该多考虑使用 <code v-pre>HashMap</code>，因为 <code v-pre>HashMap</code> 正是为快速查询设计的（<code v-pre>HashMap</code> 底层其实也是采用数组来存储 <code v-pre>key-value</code> 对）。但如果程序需要一个总是排好序的 <code v-pre>Map</code> 时，则可以考虑使用 <code v-pre>TreeMap</code>。</p>
<p><code v-pre>LinkedHashMap</code> 比 <code v-pre>HashMap</code> 慢一点，因为它需要维护链表来保持 <code v-pre>Map</code> 中 <code v-pre>key-value</code> 时的添加顺序。<code v-pre>IdentityHashMap</code> 性能没有特别出色之处，因为它采用与 <code v-pre>HashMap</code> 基本相似的实现，只是它使用 <code v-pre>==</code> 而不是 <code v-pre>equals()</code> 方法来判断元素相等。<code v-pre>EnumMap</code> 的性能最好，但它只能使用同一个枚举类的枚举值作为 <code v-pre>key</code>。</p>
<h2 id="hashset-和-hashmap" tabindex="-1"><a class="header-anchor" href="#hashset-和-hashmap" aria-hidden="true">#</a> <code v-pre>HashSet</code> 和 <code v-pre>HashMap</code></h2>
<p>对于 <code v-pre>HashSet</code> 及其子类而言，它们采用 <code v-pre>hash</code> 算法来决定集合中元素的存储位置，并通过 <code v-pre>hash</code> 算法来控制集合的大小；对于 <code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 及其子类而言，它们采用 <code v-pre>hash</code> 算法来决定 <code v-pre>Map</code> 中 <code v-pre>key</code> 的存储，并通过 <code v-pre>hash</code> 算法来增加 <code v-pre>key</code> 集合的大小。</p>
<p><code v-pre>hash</code> 表里可以存储元素的位置被称为“桶（<code v-pre>bucket</code>）”，在通常情况下，单个“桶”里存储一个元素，此时有最好的性能：<code v-pre>hash</code> 算法可以根据 <code v-pre>hashCode</code> 值计算出“桶”的存储位置，接着从“桶”中取出元素。</p>
<p>但 <code v-pre>hash</code> 表的状态为 <code v-pre>open</code>：在发生“<code v-pre>hash</code> 冲突”的情况下，单个桶会存储多个元素，这些元素以链表形式存储，必须按顺序搜索。</p>
<p>如图所示是 <code v-pre>hash</code> 表保存各元素，且发生“<code v-pre>hash</code> 冲突”的示意图。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208111527890.jpeg" alt="img"></p>
<p>因为 <code v-pre>HashSet</code> 和 <code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 都使用 <code v-pre>hash</code> 算法来决定其元素（<code v-pre>HashMap</code> 则只考虑 <code v-pre>key</code>）的存储，因此 <code v-pre>HashSet</code>、<code v-pre>HashMap</code> 的 <code v-pre>hash</code> 表包含如下属性。</p>
<blockquote>
<ol>
<li>容量（capacity）：<code v-pre>hash</code> 表中桶的数量。</li>
<li>初始化容量（initial capacity）：创建 <code v-pre>hash</code> 表时桶的数量。<code v-pre>HashMap</code> 和 <code v-pre>HashSet</code> 都允许在构造器中指定初始化容量。</li>
<li>尺寸（size）：当前 <code v-pre>hash</code> 表中记录的数量。</li>
<li>负载因子（load factor）：负载因子等于“<code v-pre>size/capacity</code>”。负载因子为 0，表示空的 <code v-pre>hash</code> 表，0.5 表示半满的散列表，依此类推。轻负载的散列表具有冲突少、适宜插入与查询的特点（但是使用 <code v-pre>Iterator</code> 迭代元素时比较慢）。</li>
</ol>
</blockquote>
<p>除此之外，<code v-pre>hash</code> 表里还有一个“负载极限”，“负载极限”是一个 0~1 的数值，“负载极限”决定了 <code v-pre>hash</code> 表的最大填满程度。当 <code v-pre>hash</code> 表中的负载因子达到指定的“负载极限”时，<code v-pre>hash</code> 表会自动成倍地增加容量（桶的数量），并将原有的对象重新分配，放入新的桶内，这称为 <code v-pre>rehashing</code>。</p>
<p><code v-pre>HashSet</code> 和 <code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 的构造器允许指定一个负载极限，<code v-pre>HashSet</code> 和 <code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 默认的“负载极限”为 0.75，这表明当该 <code v-pre>hash</code> 表的 3/4 已经被填满时，<code v-pre>hash</code> 表会发生 <code v-pre>rehashing</code>。</p>
<p>“负载极限”的默认值（0.75）是时间和空间成本上的一种折中：较高的“负载极限”可以降低 <code v-pre>hash</code> 表所占用的内存空间，但会增加查询数据的时间开销，而查询是最频繁的操作（<code v-pre>HashMap</code> 的 <code v-pre>get()</code> 与 <code v-pre>put()</code> 方法都要用到查询）；较低的“负载极限”会提高查询数据的性能，但会增加 <code v-pre>hash</code> 表所占用的内存开销。</p>
<p>程序员可以根据实际情况来调整 <code v-pre>HashSet</code> 和 <code v-pre>HashMap</code> 的“负载极限”值。</p>
<p>如果开始就知道 <code v-pre>HashSet</code> 和 <code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 会保存很多记录，则可以在创建时就使用较大的初始化容量，如果初始化容量始终大于 <code v-pre>HashSet</code> 和 <code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 所包含的最大记录数除以负载极限，就不会发生 <code v-pre>rehashing</code>。</p>
<p>使用足够大的初始化容量创建 <code v-pre>HashSet</code> 和 <code v-pre>HashMap</code>、<code v-pre>Hashtable</code> 时，可以更高效地增加记录，但将初始化容量设置太高可能会浪费空间，因此通常不要将初始化容量设置得太高。</p>
<h2 id="工具类-collections" tabindex="-1"><a class="header-anchor" href="#工具类-collections" aria-hidden="true">#</a> 工具类：<code v-pre>Collections</code></h2>
<p>Java 提供了一个操作 <code v-pre>Set</code>、<code v-pre>List</code> 和 <code v-pre>Map</code> 等集合的工具类：<code v-pre>Collections</code>，该工具类里提供了大量方法对集合元素进行排序、查询和修改等操作，还提供了将集合对象设置为不可变、对集合对象实现同步控制等方法。</p>
<h3 id="排序操作" tabindex="-1"><a class="header-anchor" href="#排序操作" aria-hidden="true">#</a> 排序操作</h3>
<p><code v-pre>Collections</code> 提供了如下几个方法用于对 <code v-pre>List</code> 集合元素进行排序。</p>
<blockquote>
<ol>
<li><code v-pre>static void reverse(List list)</code>：反转指定 <code v-pre>List</code> 集合中元素的顺序。</li>
<li><code v-pre>static void shuffle(List list)</code>：对 <code v-pre>List</code> 集合元素进行随机排序（<code v-pre>shuffle</code> 方法模拟了“洗牌”动作）。</li>
<li><code v-pre>static void sort(List list)</code>：根据元素的自然顺序对指定 <code v-pre>List</code> 集合的元素按升序进行排序。</li>
<li><code v-pre>static void sort(List list, Comparator c)</code>：根据指定 <code v-pre>Comparator</code> 产生的顺序对 <code v-pre>List</code> 集合元素进行排序。</li>
<li><code v-pre>static void swap(List list, int i, int j)</code>：将指定 <code v-pre>List</code> 集合中的 <code v-pre>i</code> 处元素和 <code v-pre>j</code> 处元素进行交换。</li>
<li><code v-pre>static void rotate(List list , int distance)</code>：当 <code v-pre>distance</code> 为正数时，将 <code v-pre>list</code> 集合的后 <code v-pre>distance</code> 个元素“整体”移到前面；当 <code v-pre>distance</code> 为负数时，将 <code v-pre>list</code> 集合的前 <code v-pre>distance</code> 个元素“整体”移到后面。该方法不会改变集合的长度。</li>
</ol>
</blockquote>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">SortTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<p><code v-pre>Collections</code> 还提供了如下用于查找、替换集合元素的常用方法。</p>
<blockquote>
<ol>
<li><code v-pre>static int binarySearch(List list, Object key)</code>：使用二分搜索法搜索指定的 <code v-pre>List</code> 集合，以获得指定对象在 <code v-pre>List</code> 集合中的索引。如果要使该方法可以正常工作，则必须保证 <code v-pre>List</code> 中的元素已经处于有序状态。</li>
<li><code v-pre>static Object max(Collection coll)</code>：根据元素的自然顺序，返回给定集合中的最大元素。</li>
<li><code v-pre>static Object max(Collection coll, Comparator comp)</code>：根据 <code v-pre>Comparator</code> 指定的顺序，返回给定集合中的最大元素。</li>
<li><code v-pre>static Object min(Collection coll)</code>：根据元素的自然顺序，返回给定集合中的最小元素。</li>
<li><code v-pre>static Object min(Collection coll, Comparator comp)</code>：根据 <code v-pre>Comparator</code> 指定的顺序，返回给定集合中的最小元素。</li>
<li><code v-pre>static Object min(Collection coll, Comparator comp)</code>：根据 <code v-pre>Comparator</code> 指定的顺序，返回给定集合中的最小元素。</li>
<li><code v-pre>static void fill(List list, Object obj)</code>：使用指定元素 <code v-pre>obj</code> 替换指定 <code v-pre>List</code> 集合中的所有元素。</li>
<li><code v-pre>static int frequency(Collection c, Object o)</code>：返回指定集合中指定元素的出现次数。</li>
<li><code v-pre>static int indexOfSubList(List source, List target)</code>：返回子 <code v-pre>List</code> 对象在父 <code v-pre>List</code> 对象中第一次出现的位置索引；如果父 <code v-pre>List</code> 中没有出现这样的子 <code v-pre>List</code>，则返回-1。</li>
<li><code v-pre>static int lastIndexOfSubList(List source, List target)</code>：返回子 <code v-pre>List</code> 对象在父 <code v-pre>List</code> 对象中最后一次出现的位置索引；如果父 <code v-pre>List</code> 中没有出现这样的子 <code v-pre>List</code>，则返回-1。</li>
<li><code v-pre>static boolean replaceAll(List list, Object oldVal, Object newVal)</code>：使用一个新值 <code v-pre>newVal</code> 替换 <code v-pre>List</code> 对象的所有旧值 <code v-pre>oldVal</code>。</li>
</ol>
</blockquote>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">SearchTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token class-name">ArrayList</span> nums <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">ArrayList</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
        nums<span class="token punctuation">.</span><span class="token function">add</span><span class="token punctuation">(</span><span class="token number">2</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
        nums<span class="token punctuation">.</span><span class="token function">add</span><span class="token punctuation">(</span><span class="token operator">-</span><span class="token number">5</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
...
<p><code v-pre>Collections</code> 类中提供了多个 <code v-pre>synchronizedXxx()</code> 方法，该方法可以将指定集合包装成线程同步的集合，从而可以解决多线程并发访问集合时的线程安全问题。</p>
<p>Java 中常用的集合框架中的实现类 <code v-pre>HashSet</code>、<code v-pre>TreeSet</code>、<code v-pre>ArrayList</code>、<code v-pre>ArrayDeque</code>、<code v-pre>LinkedList</code>、<code v-pre>HashMap</code> 和 <code v-pre>TreeMap</code> 都是线程不安全的。</p>
<p>如果有多个线程访问它们，而且有超过一个的线程试图修改它们，则可能出现错误。</p>
<p><code v-pre>Collections</code> 提供了多个静态方法可以把它们包装成线程同步的集合。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">SynchronizedTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="设置不可变集合" tabindex="-1"><a class="header-anchor" href="#设置不可变集合" aria-hidden="true">#</a> 设置不可变集合</h3>
<p><code v-pre>Collections</code> 提供了如下三类方法来返回一个不可变的集合。</p>
<blockquote>
<ol>
<li><code v-pre>emptyXxx()</code>：返回一个空的、不可变的集合对象，此处的集合既可以是 <code v-pre>List</code>，也可以是 <code v-pre>Set</code>，还可以是 <code v-pre>Map</code>。</li>
<li><code v-pre>singletonXxx()</code>：返回一个只包含指定对象（只有一个或一项元素）的、不可变的集合对象，此处的集合既可以是 <code v-pre>List</code>，也可以是 <code v-pre>Set</code>，还可以是 <code v-pre>Map</code>。</li>
<li><code v-pre>unmodifiableXxx</code>：返回指定集合对象的不可变视图，此处的集合既可以是 <code v-pre>List</code>，也可以是 <code v-pre>Set</code>，还可以是 <code v-pre>Map</code>。</li>
</ol>
</blockquote>
<p>三类方法的参数是原有的集合对象，返回值是该集合的“只读”版本。</p>
<p>通过 <code v-pre>Collections</code> 提供的三类方法，可以生成“只读”的 <code v-pre>Collection</code> 或 <code v-pre>Map</code>。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">UnmodifiableTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="enumeration" tabindex="-1"><a class="header-anchor" href="#enumeration" aria-hidden="true">#</a> <code v-pre>Enumeration</code></h3>
<p><code v-pre>Enumeration</code> 接口是 <code v-pre>Iterator</code> 迭代器的“古老版本”，从 JDK 1.0 开始，<code v-pre>Enumeration</code> 接口就已经存在了（Iterator 从 JDK 1.2 才出现）。<code v-pre>Enumeration</code> 接口只有两个名字很长的方法。</p>
<blockquote>
<ol>
<li><code v-pre>boolean hasMoreElements()</code>：如果此迭代器还有剩下的元素，则返回 <code v-pre>true</code>。</li>
<li><code v-pre>Object nextElement()</code>：返回该迭代器的下一个元素，如果还有的话（否则抛出异常）。</li>
</ol>
</blockquote>
<p><code v-pre>Enumeration</code> 接口中的方法名称冗长，难以记忆，而且没有提供 <code v-pre>Iterator</code> 的 <code v-pre>remove()</code> 方法。如果现在编写 Java 程序，应该尽量采用 <code v-pre>Iterator</code> 迭代器，而不是用 <code v-pre>Enumeration</code> 迭代器。</p>
<div class="custom-container info">
<p class="custom-container-title">相关信息</p>
<p>Java 之所以保留 <code v-pre>Enumeration</code> 接口，主要是为了照顾以前那些“古老”的程序，那些程序里大量使用了 <code v-pre>Enumeration</code> 接口，如果新版本的 Java 里直接删除 <code v-pre>Enumeration</code> 接口，将会导致那些程序全部出错。</p>
<p>在计算机行业有一条规则：加入任何规则都必须慎之又慎，因为以后无法删除规则。</p>
</div>
<p>实际上，前面介绍的 <code v-pre>Vector</code>（包括其子类 <code v-pre>Stack</code>）、<code v-pre>Hashtable</code> 两个集合类，以及另一个极少使用的 <code v-pre>BitSet</code>，都是从 JDK 1.0 遗留下来的集合类，而 <code v-pre>Enumeration</code> 接口可用于遍历这些“古老”的集合类。</p>
<p>对于 <code v-pre>ArrayList</code>、<code v-pre>HashMap</code> 等集合类，不再支持使用 <code v-pre>Enumeration</code> 迭代器。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">EnumerationTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
