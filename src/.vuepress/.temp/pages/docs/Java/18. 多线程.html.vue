<template><div><h1 id="_18-多线程" tabindex="-1"><a class="header-anchor" href="#_18-多线程" aria-hidden="true">#</a> 18. 多线程</h1>
<h2 id="线程概述" tabindex="-1"><a class="header-anchor" href="#线程概述" aria-hidden="true">#</a> 线程概述</h2>
<p>几乎所有的操作系统都支持同时运行多个任务，一个任务通常就是一个程序，每个运行中的程序就是一个进程。当一个进程运行时，内部可能包含了多个顺序执行流，每个顺序执行流就是一个线程。</p>
<h3 id="线程和进程" tabindex="-1"><a class="header-anchor" href="#线程和进程" aria-hidden="true">#</a> 线程和进程</h3>
<h4 id="线程" tabindex="-1"><a class="header-anchor" href="#线程" aria-hidden="true">#</a> 线程</h4>
<p>现代操作系统（Windows，macOS，Linux）都可以执行多任务。多任务就是同时运行多个任务，例如：浏览器、QQ、音乐。</p>
<p>CPU 执行代码都是一条一条顺序执行的，但是，即使是单核 CPU，也可以同时运行多个任务。因为操作系统执行多任务实际上就是让 CPU 对多个任务轮流交替执行。</p>
<p>例如，假设我们有语文、数学、英语 3 门作业要做，每个作业需要 30 分钟。我们把这 3 门作业看成是 3 个任务，可以做 1 分钟语文作业，再做 1 分钟数学作业，再做 1 分钟英语作业：这样轮流做下去，在某些人眼里看来，做作业的速度就非常快，看上去就像同时在做 3 门作业一样。</p>
<p>类似的，操作系统轮流让多个任务交替执行，例如，让浏览器执行 0.001 秒，让 QQ 执行 0.001 秒，再让音乐播放器执行 0.001 秒，在人看来，CPU 就是在同时执行多个任务。</p>
<p>即使是多核 CPU，因为通常任务的数量远远多于 CPU 的核数，所以任务也是交替执行的。</p>
<h4 id="进程" tabindex="-1"><a class="header-anchor" href="#进程" aria-hidden="true">#</a> 进程</h4>
<p>计算机中，我们把一个任务称为一个进程，浏览器就是一个进程，视频播放器是另一个进程，类似的，音乐播放器和 Word 都是进程。</p>
<p>某些进程内部还需要同时执行多个子任务。例如，我们在使用 Word 时，Word 可以让我们一边打字，一边进行拼写检查，同时还可以在后台进行打印，我们把子任务称为线程。</p>
...
│└────────┘│ │└────────┘│ │└────────┘│
└──────────┘ └──────────┘ └──────────┘
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>多线程模式（一个进程有多个线程）：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>┌────────────────────┐
│Process             │
│┌────────┐┌────────┐│
││ Thread ││ Thread ││
...
│└────────┘││└────────┘││└────────┘│
└──────────┘└──────────┘└──────────┘
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="进程-vs-线程" tabindex="-1"><a class="header-anchor" href="#进程-vs-线程" aria-hidden="true">#</a> 进程 vs 线程</h4>
<p>进程和线程是包含关系，但是多任务既可以由多进程实现，也可以由单进程内的多线程实现，还可以混合多进程＋多线程。</p>
<p>具体采用哪种方式，要考虑到进程和线程的特点。</p>
<p>和多线程相比，多进程的缺点在于：</p>
<ul>
<li>创建进程比创建线程开销大，尤其是在 Windows 系统上；</li>
<li>进程间通信比线程间通信要慢，因为线程间通信就是读写同一个变量，速度很快。</li>
</ul>
<p>而多进程的优点在于：</p>
<p>多进程稳定性比多线程高，因为在多进程的情况下，一个进程崩溃不会影响其他进程，而在多线程的情况下，任何一个线程崩溃会直接导致整个进程崩溃。</p>
<h4 id="多线程" tabindex="-1"><a class="header-anchor" href="#多线程" aria-hidden="true">#</a> 多线程</h4>
<p>Java 语言内置了多线程支持：一个 Java 程序实际上是一个 JVM 进程，JVM 进程用一个主线程来执行<code v-pre>main()</code>方法，在<code v-pre>main()</code>方法内部，我们又可以启动多个线程。此外，JVM 还有负责垃圾回收的其他工作线程等。</p>
<p>因此，对于大多数 Java 程序来说，我们说多任务，实际上是说如何使用多线程实现多任务。</p>
<p>和单线程相比，多线程编程的特点在于：多线程经常需要读写共享数据，并且需要同步。例如，播放电影时，就必须由一个线程播放视频，另一个线程播放音频，两个线程需要协调运行，否则画面和声音就不同步。因此，多线程编程的复杂度高，调试更困难。</p>
<p>Java 多线程编程的特点又在于：</p>
<ul>
<li>多线程模型是 Java 程序最基本的并发模型；</li>
<li>后续读写网络、数据库、Web 开发等都依赖 Java 多线程模型。</li>
</ul>
<h2 id="创建多线程" tabindex="-1"><a class="header-anchor" href="#创建多线程" aria-hidden="true">#</a> 创建多线程</h2>
<p>Java 语言内置了多线程支持。当 Java 程序启动的时候，实际上是启动了一个 JVM 进程，然后，JVM 启动主线程来执行<code v-pre>main()</code>方法。在<code v-pre>main()</code>方法中，我们又可以启动其他线程。</p>
<h3 id="thread-类创建线程类" tabindex="-1"><a class="header-anchor" href="#thread-类创建线程类" aria-hidden="true">#</a> Thread 类创建线程类</h3>
<p>通过继承<code v-pre>Thread</code>类来创建并启动多线程的步骤如下。</p>
<ol>
<li>定义<code v-pre>Thread</code>类的子类，并重写该类的<code v-pre>run()</code>方法，该<code v-pre>run()</code>方法的方法体就代表了线程需要完成的任务。因此把<code v-pre>run()</code>方法称为线程执行体。</li>
<li>创建<code v-pre>Thread</code>子类的实例，即创建了线程对象。</li>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">FirstThread</span> <span class="token keyword">extends</span> <span class="token class-name">Thread</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>程序显式创建了 2 个子线程和主线程，<code v-pre>main()</code>方法的方法体代表主线程的线程执行体。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208201138582.png" alt="image-20220820104824187"></p>
<p>程序用到的线程两个方法。</p>
<p><code v-pre>Thread.currentThread()</code>：<code v-pre>currentThread()</code>是 Thread 类的静态方法，该方法总是返回当前正在执行的线程对象。</p>
<p><code v-pre>getName()</code>：该方法是 Thread 类的实例方法，该方法返回调用该方法的线程名字。</p>
<div class="custom-container tip">
<p class="custom-container-title">提示</p>
<p>程序可以通过<code v-pre>setName(String name)</code>方法为线程设置名字，也可以通过<code v-pre>getName()</code>方法返回指定线程的名字。</p>
<p>在默认情况下，主线程的名字为<code v-pre>main</code>，用户启动的多个线程的名字依次为<code v-pre>Thread-0</code>、<code v-pre>Thread-1</code>、<code v-pre>Thread-2</code>、…、<code v-pre>Thread-n</code>等。</p>
</div>
<p>通过实例化一个<code v-pre>Thread</code>实例，然后调用它的<code v-pre>start()</code>方法启动多线程：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Main</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>但是这个线程启动后实际上什么也不做就立刻结束了。我们希望新线程能执行指定的代码，有以下几种方法：</p>
<p>方法一：从<code v-pre>Thread</code>派生一个自定义类，然后覆写<code v-pre>run()</code>方法：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Main</span> <span class="token punctuation">{</span>
...
<span class="token keyword">class</span> <span class="token class-name">MyThread</span> <span class="token keyword">extends</span> <span class="token class-name">Thread</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>执行上述代码，注意到<code v-pre>start()</code>方法会在内部自动调用实例的<code v-pre>run()</code>方法。</p>
<p>方法二：创建<code v-pre>Thread</code>实例时，传入一个<code v-pre>Runnable</code>实例：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Main</span> <span class="token punctuation">{</span>
...
<span class="token keyword">class</span> <span class="token class-name">MyRunnable</span> <span class="token keyword">implements</span> <span class="token class-name">Runnable</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>或者用 Java8 引入的 lambda 语法进一步简写为：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Main</span> <span class="token punctuation">{</span>
  <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Main</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>main</code>线程执行的代码有 4 行，首先打印<code v-pre>main start</code>，然后创建<code v-pre>Thread</code>对象，紧接着调用<code v-pre>start()</code>启动新线程。当<code v-pre>start()</code>方法被调用时，JVM 就创建了一个新线程，通过实例变量<code v-pre>t</code>来表示这个新线程对象，并开始执行。</p>
<p>接着，<code v-pre>main</code>线程继续执行打印<code v-pre>main end</code>语句，而<code v-pre>t</code>线程在<code v-pre>main</code>线程执行的同时会并发执行，打印<code v-pre>thread run</code>和<code v-pre>thread end</code>语句。</p>
<p>当<code v-pre>run()</code>方法结束时，新线程就结束了。而<code v-pre>main()</code>方法结束时，主线程也结束了。</p>
<p>线程的执行顺序：</p>
<ol>
<li><code v-pre>main</code>线程肯定是先打印<code v-pre>main start</code>，再打印<code v-pre>main end</code>；</li>
<li><code v-pre>t</code>线程肯定是先打印<code v-pre>thread run</code>，再打印<code v-pre>thread end</code>。</li>
</ol>
<p>但是，除了可以肯定，<code v-pre>main start</code>会先打印外，<code v-pre>main end</code>打印在<code v-pre>thread run</code>之前、<code v-pre>thread end</code>之后或者之间，都无法确定。因为从<code v-pre>t</code>线程开始运行以后，两个线程就开始同时运行了，并且由操作系统调度，程序本身无法确定线程的调度顺序。</p>
<p>可以在线程中调用<code v-pre>Thread.sleep()</code>，强迫当前线程暂停一段时间：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Main</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>sleep()</code>传入的参数是毫秒。调整暂停时间的大小，我们可以看到<code v-pre>main</code>线程和<code v-pre>t</code>线程执行的先后顺序。</p>
<p>要特别注意：直接调用<code v-pre>Thread</code>实例的<code v-pre>run()</code>方法是无效的：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Main</span> <span class="token punctuation">{</span>
...
<span class="token keyword">class</span> <span class="token class-name">MyThread</span> <span class="token keyword">extends</span> <span class="token class-name">Thread</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>直接调用<code v-pre>run()</code>方法，相当于调用了一个普通的 Java 方法，当前线程并没有任何改变，也不会启动新线程。上述代码实际上是在<code v-pre>main()</code>方法内部又调用了<code v-pre>run()</code>方法，打印<code v-pre>hello</code>语句是在<code v-pre>main</code>线程中执行的，没有任何新线程被创建。</p>
<p>必须调用<code v-pre>Thread</code>实例的<code v-pre>start()</code>方法才能启动新线程，查看<code v-pre>Thread</code>类的源代码，会看到<code v-pre>start()</code>方法内部调用了一个<code v-pre>private native void start0()</code>方法，<code v-pre>native</code>修饰符表示这个方法是由 JVM 虚拟机内部的 C 代码实现的，不是由 Java 代码实现的。</p>
<h3 id="实现-runnable-接口创建线程类" tabindex="-1"><a class="header-anchor" href="#实现-runnable-接口创建线程类" aria-hidden="true">#</a> 实现 Runnable 接口创建线程类</h3>
<p>实现<code v-pre>Runnable</code>接口来创建并启动多线程的步骤如下。</p>
<ol>
<li>定义<code v-pre>Runnable</code>接口的实现类，并重写该接口的<code v-pre>run()</code>方法，该<code v-pre>run()</code>方法的方法体同样是该线程的线程执行体。</li>
<li>创建<code v-pre>Runnable</code>实现类的实例，并以此实例作为<code v-pre>Thread</code>的<code v-pre>target</code>来创建<code v-pre>Thread</code>对象，该<code v-pre>Thread</code>对象才是真正的线程对象。</li>
<li>调用线程对象的<code v-pre>start()</code>方法来启动该线程。</li>
</ol>
<p>代码如下所示：</p>
...
<p class="custom-container-title">提示</p>
<p><code v-pre>Runnable</code>对象仅仅作为<code v-pre>Thread</code>对象的<code v-pre>target</code>，<code v-pre>Runnable</code>实现类里包含的<code v-pre>run()</code>方法仅作为线程执行体。</p>
<p>而实际的线程对象依然是<code v-pre>Thread</code>实例，只是该<code v-pre>Thread</code>线程负责执行其<code v-pre>target</code>的<code v-pre>run()</code>方法。</p>
<p>通过实现<code v-pre>Runnable</code>接口来获得当前线程对象，则必须使用<code v-pre>Thread.currentThread()</code>方法。</p>
</div>
<p>程序打印出的结果中，三个子线程的<code v-pre>i</code>变量的值是连续的（但有重复），也就是采用<code v-pre>Runnable</code>接口的方式创建的多个线程可以共享线程类的实例属性。这是因为在这种方式下，程序所创建的<code v-pre>Runnable</code>对象只是线程的<code v-pre>target</code>，而多个线程可以共享同一个<code v-pre>target</code>，所以多个线程可以共享同一个线程类（实际上应该是线程的<code v-pre>target</code>类）的实例属性。</p>
<h3 id="使用-callable-和-future-创建线程" tabindex="-1"><a class="header-anchor" href="#使用-callable-和-future-创建线程" aria-hidden="true">#</a> 使用 Callable 和 Future 创建线程</h3>
<p>从 Java 5 开始，Java 提供了<code v-pre>Callable</code>接口，该接口提供了一个<code v-pre>call()</code>方法可以作为线程执行体，但<code v-pre>call()</code>方法比<code v-pre>run()</code>方法功能更强大。</p>
<ol>
<li><code v-pre>call()</code>方法可以有返回值。</li>
<li><code v-pre>call()</code>方法可以声明抛出异常。</li>
</ol>
<p><code v-pre>Callable</code>接口是 Java5 新增的接口，而且它不是<code v-pre>Runnable</code>接口的子接口，所以<code v-pre>Callable</code>对象不能直接作为<code v-pre>Thread</code>的<code v-pre>target</code>。Java 5 提供了<code v-pre>Future</code>接口来代表<code v-pre>Callable</code>接口里<code v-pre>call()</code>方法的返回值，并为<code v-pre>Future</code>接口提供了一个<code v-pre>FutureTask</code>实现类，该实现类实现了<code v-pre>Future</code>接口，并实现了<code v-pre>Runnable</code>接口——可以作为<code v-pre>Thread</code>类的<code v-pre>target</code>。</p>
<p>在<code v-pre>Future</code>接口里定义了如下几个公共方法来控制它关联的<code v-pre>Callable</code>任务。</p>
<ol>
<li><code v-pre>boolean cancel(boolean mayInterruptIfRunning)</code>：试图取消该<code v-pre>Future</code>里关联的<code v-pre>Callable</code>任务。</li>
<li><code v-pre>V get()</code>：返回<code v-pre>Callable</code>任务里<code v-pre>call()</code>方法的返回值。调用该方法将导致程序阻塞，必须等到子线程结束后才会得到返回值。</li>
<li><code v-pre>V get(long timeout, TimeUnit unit)</code>：返回<code v-pre>Callable</code>任务里<code v-pre>call()</code>方法的返回值。该方法让程序最多阻塞<code v-pre>timeout</code>和<code v-pre>unit</code>指定的时间，如果经过指定时间后<code v-pre>Callable</code>任务依然没有返回值，将会抛出<code v-pre>TimeoutException</code>异常。</li>
<li><code v-pre>boolean isCancelled()</code>：如果在<code v-pre>Callable</code>任务正常完成前被取消，则返回<code v-pre>true</code>。</li>
<li><code v-pre>boolean isDone()</code>：如果<code v-pre>Callable</code>任务已完成，则返回<code v-pre>true</code>。</li>
</ol>
<div class="custom-container tip">
<p class="custom-container-title">提示</p>
<p><code v-pre>Callable</code>接口有泛型限制，<code v-pre>Callable</code>接口里的泛型形参类型与<code v-pre>call()</code>方法返回值类型相同。</p>
</div>
<p>创建并启动有返回值的线程的步骤如下。</p>
<ol>
<li>创建<code v-pre>Callable</code>接口的实现类，并实现<code v-pre>call()</code>方法，该<code v-pre>call()</code>方法将作为线程执行体，且该<code v-pre>call()</code>方法有返回值。</li>
<li>创建<code v-pre>Callable</code>实现类的实例，使用<code v-pre>FutureTask</code>类来包装<code v-pre>Callable</code>对象，该<code v-pre>FutureTask</code>对象封装了该<code v-pre>Callable</code>对象的<code v-pre>call()</code>方法的返回值。</li>
<li>使用<code v-pre>FutureTask</code>对象作为<code v-pre>Thread</code>对象的<code v-pre>target</code>创建并启动新线程。</li>
<li>调用<code v-pre>FutureTask</code>对象的<code v-pre>get()</code>方法来获得子线程执行结束后的返回值。</li>
</ol>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ThirdThread</span> <span class="token keyword">implements</span> <span class="token class-name">Callable</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="创建线程的三种方式对比" tabindex="-1"><a class="header-anchor" href="#创建线程的三种方式对比" aria-hidden="true">#</a> 创建线程的三种方式对比</h3>
<p>实现<code v-pre>Runnable</code>接口与实现<code v-pre>Callable</code>接口的方式基本相同，因此可以将实现<code v-pre>Runnable</code>接口和实现<code v-pre>Callable</code>接口归为一种方式。这种方式与继承<code v-pre>Thread</code>方式之间的主要差别如下。</p>
<p>采用实现<code v-pre>Runnable</code>、<code v-pre>Callable</code>接口的方式创建多线程：</p>
<ul>
<li>线程类只是实现了<code v-pre>Runnable</code>接口或<code v-pre>Callable</code>接口，还可以继承其他类。</li>
<li>在这种方式下，多个线程可以共享同一个<code v-pre>target</code>对象，所以非常适合多个相同线程来处理同一份资源的情况，从而可以将 CPU、代码和数据分开，形成清晰的模型，较好地体现了面向对象的思想。</li>
<li>劣势是：编程稍稍复杂，如果需要访问当前线程，则必须使用<code v-pre>Thread.currentThread()</code>方法。</li>
</ul>
<p>采用继承<code v-pre>Thread</code>类的方式创建多线程：</p>
<ul>
<li>劣势是：因为线程类已经继承了<code v-pre>Thread</code>类，所以不能再继承其他父类。</li>
<li>优势是：编写简单，如果需要访问当前线程，则无须使用<code v-pre>Thread.currentThread()</code>方法，直接使用<code v-pre>this</code>即可获得当前线程。</li>
</ul>
<p>一般推荐采用实现<code v-pre>Runnable</code>接口、<code v-pre>Callable</code>接口的方式来创建多线程。</p>
<h2 id="线程的生命周期" tabindex="-1"><a class="header-anchor" href="#线程的生命周期" aria-hidden="true">#</a> 线程的生命周期</h2>
<p>在线程的生命周期中，它要经过新建（<code v-pre>New</code>）、就绪（<code v-pre>Runnable</code>）、运行（<code v-pre>Running</code>）、阻塞（<code v-pre>Blocked</code>）和死亡（<code v-pre>Dead</code>） 5 种状态。</p>
<p>用一个状态转移图表示如下：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>         ┌─────────────┐
         │     New     │
         └─────────────┘
...
│└─────────────┘ └─────────────┘│
 ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─
         ...
         │ Terminated  │
         └─────────────┘
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>当线程启动后，它可以在 <code v-pre>Running</code>、<code v-pre>Blocked</code>、<code v-pre>Waiting</code> 和 <code v-pre>Timed Waiting</code> 这几个状态之间切换，直到最后变成 <code v-pre>Terminated</code> 状态，线程终止。</p>
<h3 id="新建和就绪状态" tabindex="-1"><a class="header-anchor" href="#新建和就绪状态" aria-hidden="true">#</a> 新建和就绪状态</h3>
<p>当程序使用<code v-pre>new</code>关键字创建了一个线程之后，该线程就处于新建状态，此时它和其他的 Java 对象一样，仅仅由 Java 虚拟机为其分配内存，并初始化其成员变量的值。</p>
<p>当线程对象调用了<code v-pre>start()</code>方法之后，该线程处于就绪状态，Java 虚拟机会为其创建方法调用栈和程序计数器，处于这个状态中的线程并没有开始运行，只是表示该线程可以运行了。可以通过再次运行<code v-pre>FirstThread</code>或<code v-pre>SecondThread</code>来证明。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208201713228.png" alt="image-20220820171342935"></p>
<div class="custom-container warning">
<p class="custom-container-title">注意</p>
<p>只能对处于新建状态的线程调用<code v-pre>start()</code>方法，否则将引发<code v-pre>IllegalThreadStateException</code>异常。</p>
</div>
<h3 id="运行和阻塞状态" tabindex="-1"><a class="header-anchor" href="#运行和阻塞状态" aria-hidden="true">#</a> 运行和阻塞状态</h3>
<p>当处于就绪状态的线程获得了 CPU 开始执行<code v-pre>run()</code>方法的线程执行体时，该线程处于运行状态，如果计算机只有一个 CPU，那么在任何时刻只有一个线程处于运行状态。当然，在一个多处理器的机器上，将会有多个线程并行（注意是并行：parallel）执行；当线程数大于处理器数时，依然会存在多个线程在同一个 CPU 上轮换的现象。</p>
<p>当一个线程开始运行后，它不可能一直处于运行状态（除非它的线程执行体足够短，瞬间就执行结束了），线程在运行过程中需要被中断，目的是使其他线程获得执行的机会，线程调度的细节取决于底层平台所采用的策略。</p>
<p>对于采用抢占式策略的系统而言，系统会给每个可执行的线程一个小时间段来处理任务；当该时间段用完后，系统就会剥夺该线程所占用的资源，让其他线程获得执行的机会。在选择下一个线程时，系统会考虑线程的优先级。</p>
<p>当发生如下情况时，线程将会进入阻塞状态。</p>
<ul>
<li>线程调用<code v-pre>sleep()</code>方法主动放弃所占用的处理器资源。</li>
<li>线程调用了一个阻塞式<code v-pre>IO</code>方法，在该方法返回之前，该线程被阻塞。</li>
<li>线程试图获得一个同步监视器，但该同步监视器正被其他线程所持有。关于同步监视器的知识、后面将有更深入的介绍。</li>
<li>线程在等待某个通知（notify）。</li>
<li>程序调用了线程的<code v-pre>suspend()</code>方法将该线程挂起。但这个方法容易导致死锁，所以应该尽量避免使用该方法。</li>
</ul>
<p>当发生如下特定的情况时可以解除上面的阻塞，让该线程重新进入就绪状态。</p>
<ul>
<li>调用<code v-pre>sleep()</code>方法的线程经过了指定时间。</li>
<li>线程调用的阻塞式<code v-pre>IO</code>方法已经返回。</li>
<li>线程成功地获得了试图取得的同步监视器。</li>
<li>线程正在等待某个通知时，其他线程发出了一个通知。</li>
<li>处于挂起状态的线程被调用了<code v-pre>resume()</code>恢复方法。</li>
</ul>
<p>线程状态转换图。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208201853226.jpeg" alt="epub_681336_2144"></p>
<p>线程从阻塞状态只能进入就绪状态，无法直接进入运行状态。而就绪和运行状态之间的转换通常不受程序控制，而是由系统线程调度所决定，当处于就绪状态的线程获得处理器资源时，该线程进入运行状态；当处于运行状态的线程失去处理器资源时，该线程进入就绪状态。</p>
<p>直接调用<code v-pre>yield()</code>方法也可以让运行状态的线程转入就绪状态。</p>
<h3 id="线程死亡" tabindex="-1"><a class="header-anchor" href="#线程死亡" aria-hidden="true">#</a> 线程死亡</h3>
<p>线程会以如下 3 种方式结束，结束后就处于死亡状态。</p>
<ul>
<li><code v-pre>run()</code>或<code v-pre>call()</code>方法执行完成，线程正常结束。</li>
<li>线程抛出一个未捕获的<code v-pre>Exception</code>或<code v-pre>Error</code>。</li>
<li>直接调用该线程的<code v-pre>stop()</code>方法来结束该线程——该方法容易导致死锁，通常不推荐使用。</li>
</ul>
<div class="custom-container tip">
<p class="custom-container-title">提示</p>
<p>当主线程结束时，其他线程不受任何影响，并不会随之结束。一旦子线程启动起来后，它就拥有和主线程相同的地位，它不会受主线程的影响。</p>
</div>
<p>当线程处于就绪、运行、阻塞 3 种状态时，可以调用线程对象的<code v-pre>isAlive()</code>方法，该方法将返回 true；当线程处于新建、死亡 2 种状态时，该方法将返回 false。</p>
<p>尝试对处于死亡状态的线程再次调用<code v-pre>start()</code>方法。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">StartDead</span> <span class="token keyword">extends</span> <span class="token class-name">Thread</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>运行上面程序，将引发<code v-pre>IllegalThreadStateException</code>异常。</p>
<div class="custom-container tip">
<p class="custom-container-title">提示</p>
<p>如果子线程在主线程之后结束则程序会正常执行，可以将子线程的循环打印次数增加至和主线程一样增加这种概率。</p>
</div>
<h2 id="控制线程" tabindex="-1"><a class="header-anchor" href="#控制线程" aria-hidden="true">#</a> 控制线程</h2>
<p>Java 的线程支持提供了一些便捷的工具方法，通过这些便捷的工具方法可以很好地控制线程的执行。</p>
<h3 id="join-线程" tabindex="-1"><a class="header-anchor" href="#join-线程" aria-hidden="true">#</a> join 线程</h3>
<p>当在某个程序执行流中调用其他线程的<code v-pre>join()</code>方法时，调用线程将被阻塞，直到被<code v-pre>join()</code>方法加入的<code v-pre>join</code>线程执行完为止。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">JoinThread</span> <span class="token keyword">extends</span> <span class="token class-name">Thread</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>“被 Join 的线程”的线程，该线程不会和<code v-pre>main</code>线程并发执行，<code v-pre>main</code>线程必须等该线程执行结束后才可以向下执行。“被 Join 的线程”的线程执行时，实际上只有 2 个子线程并发执行，主线程处于等待状态。</p>
<p><code v-pre>join()</code>方法有如下 3 种重载形式。</p>
<ul>
<li><code v-pre>join()</code>：等待被<code v-pre>join</code>的线程执行完成。</li>
<li><code v-pre>join(long millis)</code>：等待被<code v-pre>join</code>的线程的时间最长为<code v-pre>millis</code>毫秒。如果在<code v-pre>millis</code>毫秒内被<code v-pre>join</code>的线程还没有执行结束，则不再等待。</li>
<li><code v-pre>join(long millis, int nanos)</code>：等待被<code v-pre>join</code>的线程的时间最长为<code v-pre>millis</code>毫秒 + <code v-pre>nanos</code>毫微秒。</li>
</ul>
<h3 id="后台线程" tabindex="-1"><a class="header-anchor" href="#后台线程" aria-hidden="true">#</a> 后台线程</h3>
<p>有一种线程，它是在后台运行的，它的任务是为其他的线程提供服务，这种线程被称为“后台线程（Daemon Thread）”，又称为“守护线程”或“精灵线程”。</p>
<p>后台线程有个特征：如果所有的前台线程都死亡，后台线程会自动死亡。</p>
<p>调用<code v-pre>Thread</code>对象的<code v-pre>setDaemon(true)</code>方法可将指定线程设置成后台线程。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">DaemonThread</span> <span class="token keyword">extends</span> <span class="token class-name">Thread</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Thread 类还提供了一个<code v-pre>isDaemon()</code>方法，用于判断指定线程是否为后台线程。</p>
<p>要将某个线程设置为后台线程，必须在该线程启动之前设置，也就是说， <code v-pre>setDaemon(true)</code>必须在<code v-pre>start()</code>方法之前调用，否则会引发<code v-pre>IllegalThreadStateException</code>异常。</p>
<div class="custom-container warning">
<p class="custom-container-title">注意</p>
<p>在守护线程中，编写代码要注意：守护线程不能持有任何需要关闭的资源，例如打开文件等，因为虚拟机退出时，守护线程没有任何机会来关闭文件，这会导致数据丢失。</p>
</div>
<h3 id="线程睡眠-sleep" tabindex="-1"><a class="header-anchor" href="#线程睡眠-sleep" aria-hidden="true">#</a> 线程睡眠：sleep</h3>
<p>如果需要让当前正在执行的线程暂停一段时间，并进入阻塞状态，则可以通过调用<code v-pre>Thread</code>类的静态<code v-pre>sleep()</code>方法来实现。<code v-pre>sleep()</code>方法有两种重载形式。</p>
<ul>
<li><code v-pre>static void sleep(long millis)</code>：让当前正在执行的线程暂停<code v-pre>millis</code>毫秒，并进入阻塞状态，该方法受到系统计时器和线程调度器的精度与准确度的影响。</li>
<li><code v-pre>static void sleep(long millis, int nanos)</code>：让当前正在执行的线程暂停<code v-pre>millis</code>毫秒 + <code v-pre>nanos</code>毫微秒，并进入阻塞状态，该方法受到系统计时器和线程调度器的精度与准确度的影响。</li>
</ul>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">SleepTest</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="线程让步-yield" tabindex="-1"><a class="header-anchor" href="#线程让步-yield" aria-hidden="true">#</a> 线程让步：yield</h3>
<p><code v-pre>yield()</code>方法是一个和<code v-pre>sleep()</code>方法有点相似的方法，它也是<code v-pre>Thread</code>类提供的一个静态方法，它也可以让当前正在执行的线程暂停，但它不会阻塞该线程，它只是将该线程转入就绪状态。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">YieldTest</span> <span class="token keyword">extends</span> <span class="token class-name">Thread</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>yield()</code>只是让当前线程暂停一下，让系统的线程调度器重新调度一次，完全可能的情况是：当某个线程调用了<code v-pre>yield()</code>方法暂停之后，线程调度器又将其调度出来重新执行。</p>
<p>当某个线程调用了<code v-pre>yield()</code>方法暂停之后，只有优先级与当前线程相同，或者优先级比当前线程更高的处于就绪状态的线程才会获得执行的机会。</p>
<p><code v-pre>sleep()</code>方法和<code v-pre>yield()</code>方法的区别如下。</p>
<ul>
<li><code v-pre>sleep()</code>方法暂停当前线程后，会给其他线程执行机会，不会理会其他线程的优先级；但<code v-pre>yield()</code>方法只会给优先级相同，或优先级更高的线程执行机会。</li>
<li><code v-pre>sleep()</code>方法会将线程转入阻塞状态，直到经过阻塞时间才会转入就绪状态；而<code v-pre>yield()</code>不会将线程转入阻塞状态，它只是强制当前线程进入就绪状态。因此完全有可能某个线程调用<code v-pre>yield()</code>方法暂停之后，立即再次获得处理器资源被执行。</li>
<li><code v-pre>sleep()</code>方法声明抛出了<code v-pre>InterruptedException</code>异常，所以调用<code v-pre>sleep()</code>方法时要么捕捉该异常，要么显式声明抛出该异常；而<code v-pre>yield()</code>方法则没有声明抛出任何异常。</li>
<li><code v-pre>sleep()</code>方法比<code v-pre>yield()</code>方法有更好的可移植性，通常不建议使用<code v-pre>yield()</code>方法来控制并发线程的执行。</li>
</ul>
<h3 id="改变线程优先级" tabindex="-1"><a class="header-anchor" href="#改变线程优先级" aria-hidden="true">#</a> 改变线程优先级</h3>
<p>每个线程执行时都具有一定的优先级，优先级高的线程获得较多的执行机会，而优先级低的线程则获得较少的执行机会。</p>
<p>每个线程默认的优先级都与创建它的父线程的优先级相同，在默认情况下，<code v-pre>main</code>线程具有普通优先级，由<code v-pre>main</code>线程创建的子线程也具有普通优先级。</p>
<p><code v-pre>Thread</code>类提供了<code v-pre>setPriority(int newPriority)</code>、<code v-pre>getPriority()</code>方法来设置和返回指定线程的优先级，其中<code v-pre>setPriority()</code>方法的参数可以是一个整数，范围是 1 ～ 10 之间，也可以使用<code v-pre>Thread</code>类的如下 3 个静态常量。</p>
<ul>
<li>MAX_PRIORITY：其值是 10。</li>
<li>MIN_PRIORITY：其值是 1。</li>
<li>NORM_PRIORITY：其值是 5。</li>
</ul>
...
<p>虽然 Java 提供了 10 个优先级级别，但这些优先级级别需要操作系统的支持。遗憾的是，不同操作系统上的优先级并不相同，而且也不能很好地和 Java 的 10 个优先级对应，例如 Windows 2000 仅提供了 7 个优先级。</p>
<p>在这种情况下，我们应该尽量避免直接为线程指定优先级，而应该使用 MAX_PRIORITY、MIN_PRIORITY 和 NORM_PRIORITY 三个静态常量来设置优先级，这样才可以保证程序具有最好的可移植性。</p>
</div>
<h2 id="线程同步" tabindex="-1"><a class="header-anchor" href="#线程同步" aria-hidden="true">#</a> 线程同步</h2>
...
<h3 id="线程安全问题" tabindex="-1"><a class="header-anchor" href="#线程安全问题" aria-hidden="true">#</a> 线程安全问题</h3>
<p>关于线程安全的一个经典问题——银行取钱。银行取钱的基本流程基本上可以分为如下几个步骤。</p>
<ol>
...
<p>使用两个线程来模拟取钱操作，模拟两个人使用同一个账户并发取钱的问题。</p>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">DrawThread</span> <span class="token keyword">extends</span> <span class="token class-name">Thread</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>创建一个账户，并启动两个线程从该账户中取钱。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">DrawTest</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>多次运行上面程序，很有可能都会看到如图所示的错误结果。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208201910336.png" alt="image-20220820191008047"></p>
<h3 id="同步代码块" tabindex="-1"><a class="header-anchor" href="#同步代码块" aria-hidden="true">#</a> 同步代码块</h3>
<p>为了解决同步问题，Java 的多线程支持引入了同步监视器来解决这个问题，使用同步监视器的通用方法就是同步代码块。</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>synchronized</code>后括号里的<code v-pre>obj</code>就是同步监视器，上面代码的含义是：线程开始执行同步代码块之前，必须先获得对同步监视器的锁定。</p>
<p>任何时刻只能有一个线程可以获得对同步监视器的锁定，当同步代码块执行完成后，该线程会释放对该同步监视器的锁定。</p>
<p>通常推荐使用可能被并发访问的共享资源充当同步监视器。</p>
<p>对于上面的取钱模拟程序，使用账户（<code v-pre>account</code>）作为同步监视器。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">DrawThread</span> <span class="token keyword">extends</span> <span class="token class-name">Thread</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="同步方法" tabindex="-1"><a class="header-anchor" href="#同步方法" aria-hidden="true">#</a> 同步方法</h3>
<p>同步方法就是使用<code v-pre>synchronized</code>关键字来修饰某个方法，则该方法称为同步方法。对于同步方法而言，无须显式指定同步监视器，同步方法的同步监视器是<code v-pre>this</code>，也就是该对象本身。</p>
<p>通过使用同步方法可以非常方便地实现线程安全的类，线程安全的类具有如下特征。</p>
<ul>
<li>该类的对象可以被多个线程安全地访问。</li>
<li>每个线程调用该对象的任意方法之后都将得到正确结果。</li>
<li>每个线程调用该对象的任意方法之后，该对象状态依然保持合理状态。</li>
</ul>
<p>将<code v-pre>Account</code>类对<code v-pre>balance</code>的访问设置成线程安全的，把<code v-pre>balance</code>的方法修改成同步方法。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@Data</span>
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Account</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>修改<code v-pre>DrawThread</code>线程类，该线程类的<code v-pre>run()</code>方法只要调用<code v-pre>Account</code>对象的<code v-pre>draw()</code>方法即可执行取钱操作。</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>可变类的线程安全是以降低程序的运行效率作为代价的，为了减少线程安全所带来的负面影响，程序可以采用如下策略。</p>
<ul>
<li>不要对线程安全类的所有方法都进行同步，只对那些会是共享资源的方法进行同步。例如上面<code v-pre>Account</code>类中的<code v-pre>accountNo</code>属性就无须同步，所以程序只对<code v-pre>draw()</code>方法进行了同步控制。</li>
<li>如果可变类有两种运行环境：单线程环境和多线程环境，则应该为该可变类提供两种版本，即线程不安全版本和线程安全版本。在单线程环境中使用线程不安全版本以保证性能，在多线程环境中使用线程安全版本。</li>
</ul>
<h3 id="释放同步监视器的锁定" tabindex="-1"><a class="header-anchor" href="#释放同步监视器的锁定" aria-hidden="true">#</a> 释放同步监视器的锁定</h3>
<p>程序无法显式释放对同步监视器的锁定，线程会在如下几种情况下释放对同步监视器的锁定。</p>
<ol>
<li>当前线程的同步方法、同步代码块执行结束，当前线程即释放同步监视器。</li>
<li>当前线程在同步代码块、同步方法中遇到<code v-pre>break</code>、<code v-pre>return</code>终止了该代码块、该方法的继续执行，当前线程将会释放同步监视器。</li>
<li>当前线程在同步代码块、同步方法中出现了未处理的<code v-pre>Error</code>或<code v-pre>Exception</code>，导致了该代码块、该方法异常结束时，当前线程将会释放同步监视器。</li>
<li>当前线程执行同步代码块或同步方法时，程序执行了同步监视器对象的<code v-pre>wait()</code>方法，则当前线程暂停，并释放同步监视器。</li>
</ol>
<p>在如下所示的情况下，线程不会释放同步监视器。</p>
<ul>
<li>线程执行同步代码块或同步方法时，程序调用<code v-pre>Thread.sleep()</code>、<code v-pre>Thread.yield()</code>方法来暂停当前线程的执行，当前线程不会释放同步监视器。</li>
<li>线程执行同步代码块时，其他线程调用了该线程的<code v-pre>suspend()</code>方法将该线程挂起，该线程不会释放同步监视器。尽量避免使用<code v-pre>suspend()</code>和<code v-pre>resume()</code>方法来控制线程。</li>
</ul>
<h3 id="同步锁-lock" tabindex="-1"><a class="header-anchor" href="#同步锁-lock" aria-hidden="true">#</a> 同步锁（Lock）</h3>
<p>从 Java 5 开始，Java 提供了一种功能更强大的线程同步机制——通过显式定义同步锁对象来实现同步，在这种机制下，同步锁使用<code v-pre>Lock</code>对象充当。</p>
<p><code v-pre>Lock</code>提供了比<code v-pre>synchronized</code>方法和<code v-pre>synchronized</code>代码块更广泛的锁定操作，<code v-pre>Lock</code>实现允许更灵活的结构，可以具有差别很大的属性，并且支持多个相关的<code v-pre>Condition</code>对象。<code v-pre>Lock</code>是控制多个线程对共享资源进行访问的工具。</p>
<p>通常，锁提供了对共享资源的独占访问，每次只能有一个线程对<code v-pre>Lock</code>对象加锁，线程开始访问共享资源之前应先获得<code v-pre>Lock</code>对象。</p>
<p>某些锁可能允许对共享资源并发访问，如<code v-pre>ReadWriteLock</code>（读写锁），<code v-pre>Lock</code>、<code v-pre>ReadWriteLock</code>是 Java5 新提供的两个根接口，并为<code v-pre>Lock</code>提供了<code v-pre>ReentrantLock</code>（可重入锁）实现类；为<code v-pre>ReadWriteLock</code>提供了<code v-pre>ReentrantReadWriteLock</code>实现类。</p>
<p>使用该<code v-pre>Lock</code>对象可以显式地加锁、释放锁，通常使用<code v-pre>ReentrantLock</code>的代码格式如下：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">class</span> <span class="token class-name">X</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>使用<code v-pre>ReentrantLock</code>对象来进行同步，加锁和释放锁出现在不同的作用范围内时，通常建议使用<code v-pre>finally</code>块来确保在必要时释放锁。</p>
<p>通过使用<code v-pre>ReentrantLock</code>对象修改<code v-pre>Account</code>。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@Data</span>
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Account</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>Lock</code>提供了同步方法和同步代码块所没有的其他功能，包括用于非块结构的<code v-pre>tryLock()</code>方法，以及试图获取可中断锁的<code v-pre>lockInterruptibly()</code>方法，还有获取超时失效锁的<code v-pre>tryLock(long, TimeUnit)</code>方法。</p>
<p><code v-pre>ReentrantLock</code>锁具有可重入性，一个线程可以对已被加锁的<code v-pre>ReentrantLock</code>锁再次加锁， <code v-pre>ReentrantLock</code>对象会维持一个计数器来追踪<code v-pre>lock()</code>方法的嵌套调用，线程在每次调用<code v-pre>lock()</code>加锁后，必须显式调用<code v-pre>unlock()</code>来释放锁，所以一段被锁保护的代码可以调用另一个被相同锁保护的方法。</p>
<h3 id="死锁" tabindex="-1"><a class="header-anchor" href="#死锁" aria-hidden="true">#</a> 死锁</h3>
<p>当两个线程相互等待对方释放同步监视器时就会发生死锁，一旦出现死锁，整个程序既不会发生任何异常，也不会给出任何提示，只是所有线程处于阻塞状态，无法继续。</p>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">DeadLock</span> <span class="token keyword">implements</span> <span class="token class-name">Runnable</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>程序既无法向下执行，也不会抛出任何异常。</p>
<p>程序中<code v-pre>A</code>对象和<code v-pre>B</code>对象都是同步锁。程序中两个线程执行，一个线程的线程执行体是<code v-pre>DeadLock</code>类的<code v-pre>run()</code>方法，另一个线程的线程执行体是<code v-pre>DeadLock</code>的<code v-pre>init()</code>方法（主线程调用了<code v-pre>init()</code>方法）。</p>
<p>其中<code v-pre>run()</code>方法中让<code v-pre>B</code>对象调用<code v-pre>bar()</code>方法，而<code v-pre>init()</code>方法让<code v-pre>A</code>对象调用<code v-pre>foo()</code>方法。</p>
<ul>
<li><code v-pre>init()</code>方法先执行，调用了<code v-pre>A</code>对象的<code v-pre>foo()</code>方法，进入<code v-pre>foo()</code>方法之前，该线程对<code v-pre>A</code>对象加锁——当程序执行到 ① 号代码时，主线程暂停 200ms；</li>
<li>CPU 切换到执行另一个线程，让<code v-pre>B</code>对象执行<code v-pre>bar()</code>方法，进入<code v-pre>bar()</code>方法之前，该线程对<code v-pre>B</code>对象加锁——当程序执行到 ② 号代码时，副线程也暂停 200ms；</li>
<li>接下来主线程会先醒过来，继续向下执行，直到 ③ 号代码处希望调用<code v-pre>B</code>对象的<code v-pre>last()</code>方法——执行该方法之前必须先对<code v-pre>B</code>对象加锁，但此时副线程正保持着<code v-pre>B</code>对象的锁，所以主线程阻塞；</li>
<li>接下来副线程应该也醒过来了，继续向下执行，直到 ④ 号代码处希望调用<code v-pre>A</code>对象的<code v-pre>last()</code>方法——执行该方法之前必须先对<code v-pre>A</code>对象加锁，但此时主线程没有释放对<code v-pre>A</code>对象的锁——至此，就出现了主线程保持着<code v-pre>A</code>对象的锁，等待对<code v-pre>B</code>对象加锁，而副线程保持着<code v-pre>B</code>对象的锁，等待对<code v-pre>A</code>对象加锁，两个线程互相等待对方先释放，所以就出现了死锁。</li>
</ul>
<h2 id="线程通信" tabindex="-1"><a class="header-anchor" href="#线程通信" aria-hidden="true">#</a> 线程通信</h2>
<p>当线程在系统内运行时，线程的调度具有一定的透明性，程序通常无法准确控制线程的轮换执行，但我们可以通过一些机制来保证线程协调运行。</p>
<h3 id="传统的线程通信" tabindex="-1"><a class="header-anchor" href="#传统的线程通信" aria-hidden="true">#</a> 传统的线程通信</h3>
<p>假设现在系统中有两个线程，这两个线程分别代表存款者和取钱者——现在假设系统有一种特殊的要求，系统要求存款者和取钱者不断地重复存款、取钱的动作，而且要求每当存款者将钱存入指定账户后，取钱者就立即取出该笔钱。不允许存款者连续两次存钱，也不允许取钱者连续两次取钱。</p>
<p><code v-pre>Object</code>类的 3 个方法：</p>
<p><code v-pre>wait()</code>：导致当前线程等待，直到其他线程调用该同步监视器的<code v-pre>notify()</code>方法或<code v-pre>notifyAll()</code>方法来唤醒该线程。该<code v-pre>wait()</code>方法有 3 种形式——无时间参数的<code v-pre>wait</code>（一直等待，直到其他线程通知），带毫秒参数的<code v-pre>wait</code>和带毫秒、毫微秒参数的<code v-pre>wait</code>（这两种方法都是等待指定时间后自动苏醒）。调用<code v-pre>wait()</code>方法的当前线程会释放对该同步监视器的锁定。</p>
<p><code v-pre>notify()</code>：唤醒在此同步监视器上等待的单个线程。如果所有线程都在此同步监视器上等待，则会选择唤醒其中一个线程。选择是任意性的。只有当前线程放弃对该同步监视器的锁定后（使用<code v-pre>wait()</code>方法），才可以执行被唤醒的线程。</p>
<p><code v-pre>notifyAll()</code>：唤醒在此同步监视器上等待的所有线程。只有当前线程放弃对该同步监视器的锁定后，才可以执行被唤醒的线程。</p>
<p>但这 3 个方法必须由同步监视器对象来调用，这可分成以下两种情况。</p>
<ul>
<li>对于使用<code v-pre>synchronized</code>修饰的同步方法，因为该类的默认实例（<code v-pre>this</code>）就是同步监视器，所以可以在同步方法中直接调用这 3 个方法。</li>
<li>对于使用<code v-pre>synchronized</code>修饰的同步代码块，同步监视器是<code v-pre>synchronized</code>后括号里的对象，所以必须使用该对象调用这 3 个方法。</li>
</ul>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@Data</span>
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Account</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>程序中的存款者线程循环 100 次重复存款，而取钱者线程则循环 100 次重复取钱，存款者线程和取钱者线程分别调用<code v-pre>Account</code>对象的<code v-pre>deposit()</code>、<code v-pre>draw()</code>方法来实现。</p>
<p>取钱账户。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">DrawThread</span> <span class="token keyword">extends</span> <span class="token class-name">Thread</span> <span class="token punctuation">{</span>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">DrawTest</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>运行该程序，会看到如图所示的结果。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208202048023.png" alt="image-20220820204844705"></p>
<div class="custom-container warning">
<p class="custom-container-title">注意</p>
<p>阻塞并不是死锁，对于这种情况，取钱者线程已经执行结束，而存款者线程只是在等待其他线程来取钱而已，并不是等待其他线程释放同步监视器。</p>
</div>
<h3 id="使用-condition-控制线程通信" tabindex="-1"><a class="header-anchor" href="#使用-condition-控制线程通信" aria-hidden="true">#</a> 使用 Condition 控制线程通信</h3>
<p>当使用<code v-pre>Lock</code>对象来保证同步时，Java 提供了一个<code v-pre>Condition</code>类来保持协调，使用<code v-pre>Condition</code>可以让那些已经得到 Lock 对象却无法继续执行的线程释放<code v-pre>Lock</code>对象，<code v-pre>Condition</code>对象也可以唤醒其他处于等待的线程。</p>
<p><code v-pre>Condition</code>实例被绑定在一个<code v-pre>Lock</code>对象上。要获得特定<code v-pre>Lock</code>实例的<code v-pre>Condition</code>实例，调用<code v-pre>Lock</code>对象的<code v-pre>newCondition()</code>方法即可。<code v-pre>Condition</code>类提供了如下 3 个方法。</p>
<ul>
<li><code v-pre>await()</code>：类似于隐式同步监视器上的<code v-pre>wait()</code>方法，导致当前线程等待，直到其他线程调用该<code v-pre>Condition的signal()</code>方法或<code v-pre>signalAll()</code>方法来唤醒该线程。该<code v-pre>await()</code>方法有更多变体，如<code v-pre>long awaitNanos(long nanosTimeout)</code>、<code v-pre>void awaitUninterruptibly()</code>、<code v-pre>awaitUntil(Date deadline)</code>等，可以完成更丰富的等待操作。</li>
<li><code v-pre>signal()</code>：唤醒在此 Lock 对象上等待的单个线程。如果所有线程都在该<code v-pre>Lock</code>对象上等待，则会选择唤醒其中一个线程。选择是任意性的。只有当前线程放弃对该<code v-pre>Lock</code>对象的锁定后（使用<code v-pre>await()</code>方法），才可以执行被唤醒的线程。</li>
<li><code v-pre>signalAll()</code>：唤醒在此 Lock 对象上等待的所有线程。只有当前线程放弃对该<code v-pre>Lock</code>对象的锁定后，才可以执行被唤醒的线程。</li>
</ul>
<p>修改<code v-pre>Account</code>类：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@Data</span>
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Account</span> <span class="token punctuation">{</span>
...
<p><code v-pre>Condition</code>将同步监视器方法（<code v-pre>wait()</code>、<code v-pre>notify()</code>和<code v-pre>notifyAll()</code>）分解成截然不同的对象，以便通过将这些对象与<code v-pre>Lock</code>对象组合使用，为每个对象提供多个等待集（<code v-pre>wait-set</code>）。在这种情况下，<code v-pre>Lock</code>替代了同步方法或同步代码块，<code v-pre>Condition</code>替代了同步监视器的功能。</p>
<h3 id="使用阻塞队列-blockingqueue-控制线程通信" tabindex="-1"><a class="header-anchor" href="#使用阻塞队列-blockingqueue-控制线程通信" aria-hidden="true">#</a> 使用阻塞队列（<code v-pre>BlockingQueue</code>）控制线程通信</h3>
<p>Java 5 提供了一个<code v-pre>BlockingQueue</code>接口，虽然<code v-pre>BlockingQueue</code>也是<code v-pre>Queue</code>的子接口，但它的主要用途并不是作为容器，而是作为线程同步的工具。</p>
<p><code v-pre>BlockingQueue</code>具有一个特征：当生产者线程试图向<code v-pre>BlockingQueue</code>中放入元素时，如果该队列已满，则该线程被阻塞；当消费者线程试图从<code v-pre>BlockingQueue</code>中取出元素时，如果该队列已空，则该线程被阻塞。</p>
<p><code v-pre>BlockingQueue</code>提供如下两个支持阻塞的方法。</p>
<ul>
<li><code v-pre>put(E e)</code>：尝试把<code v-pre>E</code>元素放入<code v-pre>BlockingQueue</code>中，如果该队列的元素已满，则阻塞该线程。</li>
<li><code v-pre>take()</code>：尝试从<code v-pre>BlockingQueue</code>的头部取出元素，如果该队列的元素已空，则阻塞该线程。</li>
</ul>
<p><code v-pre>BlockingQueue</code>继承了<code v-pre>Queue</code>接口，可使用<code v-pre>Queue</code>接口中的方法。这些方法归纳起来可分为如下 3 组。</p>
<ul>
<li>在队列尾部插入元素。包括<code v-pre>add(E e)</code>、<code v-pre>offer(E e)</code>和<code v-pre>put(E e)</code>方法，当该队列已满时，这 3 个方法分别会抛出异常、返回<code v-pre>false</code>、阻塞队列。</li>
<li>在队列头部删除并返回删除的元素。包括<code v-pre>remove()</code>、<code v-pre>poll()</code>和<code v-pre>take()</code>方法。当该队列已空时，这 3 个方法分别会抛出异常、返回<code v-pre>false</code>、阻塞队列。</li>
<li>在队列头部取出但不删除元素。包括<code v-pre>element()</code>和<code v-pre>peek()</code>方法，当队列已空时，这两个方法分别抛出异常、返回<code v-pre>false</code>。</li>
</ul>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208202104417.jpeg" alt="epub_681336_2209"></p>
<p><code v-pre>BlockingQueue</code>与其实现类：</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208202102081.jpeg" alt="epub_681336_2207"></p>
<p>以黑色方框框出的都是 Java 7 新增的阻塞队列。</p>
<p><code v-pre>BlockingQueue</code>包含的 5 个实现类。</p>
<ul>
<li>
<p><code v-pre>ArrayBlockingQueue</code>：基于数组实现的<code v-pre>BlockingQueue</code>队列。</p>
</li>
<li>
<p><code v-pre>LinkedBlockingQueue</code>：基于链表实现的<code v-pre>BlockingQueue</code>队列。</p>
</li>
<li>
<p><code v-pre>PriorityBlockingQueue</code>：它并不是标准的阻塞队列。</p>
<p>与<code v-pre>PriorityQueue</code>类似，该队列调用<code v-pre>remove()</code>、<code v-pre>poll()</code>、<code v-pre>take()</code>等方法取出元素时，并不是取出队列中存在时间最长的元素，而是队列中最小的元素。</p>
<p><code v-pre>PriorityBlockingQueue</code>判断元素的大小即可根据元素（实现<code v-pre>Comparable</code>接口）的本身大小来自然排序，也可使用<code v-pre>Comparator</code>进行定制排序。</p>
</li>
<li>
<p><code v-pre>SynchronousQueue</code>：同步队列。对该队列的存、取操作必须交替进行。</p>
</li>
<li>
<p><code v-pre>DelayQueue</code>：它是一个特殊的<code v-pre>BlockingQueue</code>，底层基于<code v-pre>PriorityBlockingQueue</code>实现。</p>
<p>不过， <code v-pre>DelayQueue</code>要求集合元素都实现<code v-pre>Delay</code>接口（该接口里只有一个<code v-pre>long getDelay()</code>方法）， <code v-pre>DelayQueue</code>根据集合元素的<code v-pre>getDalay()</code>方法的返回值进行排序。</p>
</li>
</ul>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">BlockingQueueTest</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>利用<code v-pre>BlockingQueue</code>来实现线程通信。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">class</span> <span class="token class-name">Producer</span> <span class="token keyword">extends</span> <span class="token class-name">Thread</span> <span class="token punctuation">{</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">BlockingQueueTest2</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="线程组和未处理的异常" tabindex="-1"><a class="header-anchor" href="#线程组和未处理的异常" aria-hidden="true">#</a> 线程组和未处理的异常</h2>
<p>Java 使用<code v-pre>ThreadGroup</code>来表示线程组，它可以对一批线程进行分类管理，Java 允许程序直接对线程组进行控制。</p>
<p>对线程组的控制相当于同时控制这批线程。用户创建的所有线程都属于指定线程组，如果程序没有显式指定线程属于哪个线程组，则该线程属于默认线程组。</p>
<p>在默认情况下，子线程和创建它的父线程处于同一个线程组内，例如 A 线程创建了 B 线程，并且没有指定 B 线程的线程组，则 B 线程属于 A 线程所在的线程组。</p>
<p>一旦某个线程加入了指定线程组之后，该线程将一直属于该线程组，直到该线程死亡，线程运行中途不能改变它所属的线程组。</p>
<p><code v-pre>Thread</code>类提供了如下几个构造器来设置新创建的线程属于哪个线程组。</p>
<ul>
<li><code v-pre>Thread(ThreadGroup group, Runnable target)</code>：以<code v-pre>target</code>的<code v-pre>run()</code>方法作为线程执行体创建新线程，属于<code v-pre>group</code>线程组。</li>
<li><code v-pre>Thread(ThreadGroup group, Runnable target, String name)</code>：以<code v-pre>target</code>的<code v-pre>run()</code>方法作为线程执行体创建新线程，该线程属于<code v-pre>group</code>线程组，且线程名为<code v-pre>name</code>。</li>
<li><code v-pre>Thread(ThreadGroup group, String name)</code>：创建新线程，新线程名为<code v-pre>name</code>，属于<code v-pre>group</code>线程组。</li>
</ul>
<p><code v-pre>Thread</code>类没有提供了一个<code v-pre>getThreadGroup()</code>方法来返回该线程所属的线程组，<code v-pre>getThreadGroup()</code>方法的返回值是<code v-pre>ThreadGroup</code>对象，表示一个线程组。<code v-pre>ThreadGroup</code>类提供了如下两个简单的构造器来创建实例。</p>
<ul>
<li><code v-pre>ThreadGroup(String name)</code>：以指定的线程组名字来创建新的线程组。</li>
<li><code v-pre>ThreadGroup(ThreadGroup parent, String name)</code>：以指定的名字、指定的父线程组创建一个新线程组。</li>
</ul>
<p>线程组通过调用<code v-pre>ThreadGroup</code>的<code v-pre>getName()</code>方法来获取名字，但不允许改变线程组的名字。</p>
<p><code v-pre>ThreadGroup</code>类提供了如下几个常用的方法来操作整个线程组里的所有线程。</p>
<ul>
<li>
<p><code v-pre>int activeCount()</code>：返回此线程组中活动线程的数目。</p>
</li>
<li>
<p><code v-pre>interrupt()</code>：中断此线程组中的所有线程。</p>
</li>
<li>
<p><code v-pre>isDaemon()</code>：判断该线程组是否是后台线程组。</p>
</li>
<li>
<p><code v-pre>setDaemon(boolean daemon)</code>：把该线程组设置成后台线程组。</p>
<p>后台线程组具有一个特征——当后台线程组的最后一个线程执行结束或最后一个线程被销毁后，后台线程组将自动销毁。</p>
</li>
<li>
<p><code v-pre>setMaxPriority(int pri)</code>：设置线程组的最高优先级。</p>
</li>
</ul>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ThreadGroupTest</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>从 Java 5 开始，Java 加强了线程的异常处理，如果线程执行过程中抛出了一个未处理异常，JVM 在结束该线程之前会自动查找是否有对应的<code v-pre>Thread.UncaughtExceptionHandler</code>对象，如果找到该处理器对象，则会调用该对象的<code v-pre>uncaughtException(Thread t, Throwable e)</code>方法来处理该异常。</p>
<p><code v-pre>Thread.UncaughtExceptionHandler</code>是<code v-pre>Thread</code>类的一个静态内部接口，该接口内只有一个方法：<code v-pre>void uncaughtException(Thread t, Throwable e)</code>，该方法中的<code v-pre>t</code>代表出现异常的线程，而<code v-pre>e</code>代表该线程抛出的异常。</p>
<p><code v-pre>Thread</code>类提供了如下两个方法来设置异常处理器。</p>
<ul>
<li><code v-pre>static setDefaultUncaughtExceptionHandler(Thread.UncaughtExceptionHandler eh)</code>：为该线程类的所有线程实例设置默认的异常处理器。</li>
<li><code v-pre>setUncaughtExceptionHandler(Thread.UncaughtExceptionHandler eh)</code>：为指定的线程实例设置异常处理器。</li>
</ul>
<p><code v-pre>ThreadGroup</code>类实现了<code v-pre>Thread.UncaughtExceptionHandler</code>接口，所以每个线程所属的线程组将会作为默认的异常处理器。</p>
<p>当一个线程抛出未处理异常时，JVM 会首先查找该异常对应的异常处理器（<code v-pre>setUncaughtExceptionHandler()</code>方法设置的异常处理器），如果找到该异常处理器，则将调用该异常处理器处理该异常；否则，JVM 将会调用该线程所属的线程组对象的<code v-pre>uncaughtException()</code>方法来处理该异常。</p>
<p>线程组处理异常的默认流程如下。</p>
<ol>
<li>如果该线程组有父线程组，则调用父线程组的<code v-pre>uncaughtException()</code>方法来处理该异常。</li>
<li>如果该线程实例所属的线程类有默认的异常处理器（由<code v-pre>setDefaultUncaughtExceptionHandler()</code>方法设置的异常处理器），那么就调用该异常处理器来处理该异常。</li>
<li>如果该异常对象是<code v-pre>ThreadDeath</code>的对象，则不做任何处理；否则，将异常跟踪栈的信息打印到<code v-pre>System.err</code>错误输出流，并结束该线程。</li>
</ol>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ExHandler</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>运行该程序，会看到如下输出：</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208221323663.png" alt="image-20220822132326025"></p>
<p>虽然程序中指定了异常处理器对未捕获的异常进行处理，而且该异常处理器也确实起作用了，但程序依然不会正常结束。这说明异常处理器与通过 catch 捕获异常是不同的——当使用 catch 捕获异常时，异常不会向上传播给上一级调用者；但使用异常处理器对异常进行处理之后，异常依然会传播给上一级调用者。</p>
<h2 id="线程池" tabindex="-1"><a class="header-anchor" href="#线程池" aria-hidden="true">#</a> 线程池</h2>
<p>Java 语言虽然内置了多线程支持，启动一个新线程非常方便，但是，创建线程需要操作系统资源（线程资源，栈空间等），频繁创建和销毁大量线程需要消耗大量时间。</p>
<p>如果可以复用一组线程：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>┌─────┐ execute  ┌──────────────────┐
...
<h3 id="实现的线程池" tabindex="-1"><a class="header-anchor" href="#实现的线程池" aria-hidden="true">#</a> 实现的线程池</h3>
<p>Java 5 新增了一个<code v-pre>Executors</code>工厂类来产生线程池，该工厂类包含如下几个静态工厂方法来创建线程池。</p>
<ul>
<li>
<p><code v-pre>newCachedThreadPool()</code>：创建一个具有缓存功能的线程池，系统根据需要创建线程，这些线程将会被缓存在线程池中。</p>
</li>
<li>
<p><code v-pre>newFixedThreadPool(int nThreads)</code>：创建一个可重用的、具有固定线程数的线程池。</p>
</li>
<li>
<p><code v-pre>newSingleThreadExecutor()</code>：创建一个只有单线程的线程池，它相当于调用<code v-pre>newFixedThread Pool()</code>方法时传入参数为 1。</p>
</li>
<li>
<p><code v-pre>newScheduledThreadPool(int corePoolSize)</code>：创建具有指定线程数的线程池，它可以在指定延迟后执行线程任务。</p>
<p><code v-pre>corePoolSize</code>指池中所保存的线程数，即使线程是空闲的也被保存在线程池内。</p>
</li>
<li>
<p><code v-pre>newSingleThreadScheduledExecutor()</code>：创建只有一个线程的线程池，它可以在指定延迟后执行线程任务。</p>
</li>
</ul>
<blockquote>
<p>前 3 个方法返回一个<code v-pre>ExecutorService</code>对象，该对象代表一个线程池，它可以执行<code v-pre>Runnable</code>对象或<code v-pre>Callable</code>对象所代表的线程；</p>
<p>而后 2 个方法返回一个<code v-pre>ScheduledExecutorService</code>线程池，它是<code v-pre>ExecutorService</code>的子类，它可以在指定延迟后执行线程任务。</p>
</blockquote>
<p><code v-pre>ExecutorService</code>代表尽快执行线程的线程池（只要线程池中有空闲线程，就立即执行线程任务），程序只要将一个<code v-pre>Runnable</code>对象或<code v-pre>Callable</code>对象（代表线程任务）提交给该线程池，该线程池就会尽快执行该任务。<code v-pre>ExecutorService</code>里提供了如下 3 个方法。</p>
<ul>
<li>
<p><code v-pre>Future&lt;?&gt; submit(Runnable task)</code>：将一个<code v-pre>Runnable</code>对象提交给指定的线程池，线程池将在有空闲线程时执行<code v-pre>Runnable</code>对象代表的任务。</p>
<p>其中<code v-pre>Future</code>对象代表<code v-pre>Runnable</code>任务的返回值——但<code v-pre>run()</code>方法没有返回值，所以<code v-pre>Future</code>对象将在<code v-pre>run()</code>方法执行结束后返回<code v-pre>null</code>。</p>
<p>但可以调用<code v-pre>Future</code>的<code v-pre>isDone()</code>、<code v-pre>isCancelled()</code>方法来获得<code v-pre>Runnable</code>对象的执行状态。</p>
</li>
<li>
<p><code v-pre>&lt;T&gt; Future&lt;T&gt; submit(Runnable task, T result)</code>：将一个<code v-pre>Runnable</code>对象提交给指定的线程池，线程池将在有空闲线程时执行<code v-pre>Runnable</code>对象代表的任务。</p>
<p>其中<code v-pre>result</code>显式指定线程执行结束后的返回值，所以<code v-pre>Future</code>对象将在<code v-pre>run()</code>方法执行结束后返回<code v-pre>result</code>。</p>
</li>
<li>
<p><code v-pre>&lt;T&gt; Future&lt;T&gt; submit(Callable&lt;T&gt; task)</code>：将一个<code v-pre>Callable</code>对象提交给指定的线程池，线程池将在有空闲线程时执行<code v-pre>Callable</code>对象代表的任务。</p>
<p>其中<code v-pre>Future</code>代表<code v-pre>Callable</code>对象里<code v-pre>call()</code>方法的返回值。</p>
</li>
</ul>
<p><code v-pre>ScheduledExecutorService</code>代表可在指定延迟后或周期性地执行线程任务的线程池，它提供了如下 4 个方法。</p>
<ul>
<li>
<p><code v-pre>ScheduledFuture&lt;?&gt; schedule(Runnable command, long delay,TimeUnit unit)</code>：指定<code v-pre>command</code>任务将在<code v-pre>delay</code>延迟后执行。</p>
</li>
<li>
<p><code v-pre>ScheduledFuture&lt;?&gt; scheduleAtFixedRate(Runnable command, long initialDelay, long period, TimeUnit unit)</code>：指定<code v-pre>command</code>任务将在<code v-pre>delay</code>延迟后执行，而且以设定频率重复执行。</p>
<p>也就是说，在<code v-pre>initialDelay</code>后开始执行，依次在<code v-pre>initialDelay+period</code>、<code v-pre>initialDelay+2 * period…</code>处重复执行，依此类推。</p>
</li>
<li>
<p><code v-pre>ScheduledFuture&lt;?&gt; scheduleWithFixedDelay(Runnable command,long initialDelay, long delay,TimeUnit unit)</code>：创建并执行一个在给定初始延迟后首次启用的定期操作，随后在每一次执行终止和下一次执行开始之间都存在给定的延迟。</p>
<p>如果任务在任一次执行时遇到异常，就会取消后续执行；否则，只能通过程序来显式取消或终止该任务。</p>
</li>
</ul>
<p>当用完一个线程池后，应该调用该线程池的<code v-pre>shutdown()</code>方法，该方法将启动线程池的关闭序列，调用<code v-pre>shutdown()</code>方法后的线程池不再接收新任务，但会将以前所有已提交任务执行完成。</p>
<p>当线程池中的所有任务都执行完成后，池中的所有线程都会死亡；另外也可以调用线程池的<code v-pre>shutdownNow()</code>方法来关闭线程池，该方法试图停止所有正在执行的活动任务，暂停处理正在等待的任务，并返回等待执行的任务列表。</p>
<p>使用线程池来执行线程任务的步骤如下。</p>
<ol>
<li>调用<code v-pre>Executors</code>类的静态工厂方法创建一个<code v-pre>ExecutorService</code>对象，该对象代表一个线程池。</li>
<li>创建<code v-pre>Runnable</code>实现类或<code v-pre>Callable</code>实现类的实例，作为线程执行任务。</li>
<li>调用<code v-pre>ExecutorService</code>对象的<code v-pre>submit()</code>方法来提交<code v-pre>Runnable</code>实例或<code v-pre>Callable</code>实例。</li>
<li>当不想提交任何任务时，调用<code v-pre>ExecutorService</code>对象的<code v-pre>shutdown()</code>方法来关闭线程池。</li>
</ol>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Main</span> <span class="token punctuation">{</span>
...
<span class="token keyword">class</span> <span class="token class-name">Task</span> <span class="token keyword">implements</span> <span class="token class-name">Runnable</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>观察执行结果，一次性放入 6 个任务，由于线程池只有固定的 4 个线程，因此，前 4 个任务会同时执行，等到有线程空闲后，才会执行后面的两个任务。</p>
<p>线程池在程序结束的时候要关闭。使用<code v-pre>shutdown()</code>方法关闭线程池的时候，它会等待正在执行的任务先完成，然后再关闭。<code v-pre>shutdownNow()</code>会立刻停止正在执行的任务，<code v-pre>awaitTermination()</code>则会等待指定的时间让线程池关闭。</p>
<p>如果把线程池改为<code v-pre>CachedThreadPool</code>，由于这个线程池的实现会根据任务数量动态调整线程池的大小，所以 6 个任务可一次性全部同时执行。</p>
<p>如果想把线程池的大小限制在 4 ～ 10 个之间动态调整怎么办？我们查看<code v-pre>Executors.newCachedThreadPool()</code>方法的源码：</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>因此，想创建指定动态范围的线程池，可以这么写：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">int</span> min <span class="token operator">=</span> <span class="token number">4</span><span class="token punctuation">;</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="forkjoinpool" tabindex="-1"><a class="header-anchor" href="#forkjoinpool" aria-hidden="true">#</a> ForkJoinPool</h3>
<p>Java7 提供了<code v-pre>ForkJoinPool</code>来支持将一个任务拆分成多个“小任务”并行计算，再把多个“小任务”的结果合并成总的计算结果。<code v-pre>ForkJoinPool</code>是<code v-pre>ExecutorService</code>的实现类，因此是一种特殊的线程池。<code v-pre>ForkJoinPool</code>提供了如下两个常用的构造器。</p>
<ul>
<li><code v-pre>ForkJoinPool(int parallelism)</code>：创建一个包含<code v-pre>parallelism</code>个并行线程的<code v-pre>ForkJoinPool</code>。</li>
<li><code v-pre>ForkJoinPool()</code>：以<code v-pre>Runtime.availableProcessors()</code>方法的返回值作为<code v-pre>parallelism</code>参数来创建<code v-pre>ForkJoinPool</code>。</li>
</ul>
<p>创建了<code v-pre>ForkJoinPool</code>实例之后，就可调用<code v-pre>ForkJoinPool</code>的<code v-pre>submit(ForkJoinTask task)</code>或<code v-pre>invoke (ForkJoinTask task)</code>方法来执行指定任务了。</p>
<p>其中<code v-pre>ForkJoinTask</code>代表一个可以并行、合并的任务。</p>
<p><code v-pre>ForkJoinTask</code>是一个抽象类，它还有两个抽象子类：<code v-pre>RecursiveAction</code>和<code v-pre>RecursiveTask</code>。</p>
<p>其中<code v-pre>RecursiveTask</code>代表有返回值的任务，而<code v-pre>RecursiveAction</code>代表没有返回值的任务。</p>
<p><code v-pre>ForkJoinPool</code>、<code v-pre>ForkJoinTask</code>等类的类图。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208221441392.jpeg" alt="epub_681336_2244"></p>
<p>下面以执行没有返回值的“大任务”（简单地打印 0~300 的数值）为例，程序将一个“大任务”拆分成多个“小任务”，并将任务交给<code v-pre>ForkJoinPool</code>来执行。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">class</span> <span class="token class-name">PrintTask</span> <span class="token keyword">extends</span> <span class="token class-name">RecursiveAction</span> <span class="token punctuation">{</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ForkJoinPoolTest</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>程序虽然打印了 0 ～ 299 这 300 个数字，但并不是连续打印的，这是因为程序将这个打印任务进行了分解，分解后的任务会并行执行，所以不会按顺序从 0 打印到 299。</p>
<p>上面定义的任务是一个没有返回值的打印任务，如果任务是有返回值的任务，则可以让任务继承<code v-pre>RecursiveTask&lt;T&gt;</code>，其中泛型参数<code v-pre>T</code>就代表了该任务的返回值类型。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">class</span> <span class="token class-name">CalTask</span> <span class="token keyword">extends</span> <span class="token class-name">RecursiveTask</span> <span class="token punctuation">{</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Sum</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="线程相关类" tabindex="-1"><a class="header-anchor" href="#线程相关类" aria-hidden="true">#</a> 线程相关类</h2>
<p>Java 还为线程安全提供了一些工具类，如<code v-pre>ThreadLocal</code>类，它代表一个线程局部变量，通过把数据放在<code v-pre>ThreadLocal</code>中就可以让每个线程创建一个该变量的副本，从而避免并发访问的线程安全问题。除此之外，Java 5 还新增了大量的线程安全类。</p>
<h3 id="threadlocal" tabindex="-1"><a class="header-anchor" href="#threadlocal" aria-hidden="true">#</a> ThreadLocal</h3>
<p><code v-pre>ThreadLocal</code>，是 Thread Local Variable（线程局部变量）的意思。线程局部变量（<code v-pre>ThreadLocal</code>）的功用其实非常简单，就是为每一个使用该变量的线程都提供一个变量值的副本，使每一个线程都可以独立地改变自己的副本，而不会和其他线程的副本冲突。</p>
<p>从线程的角度看，就好像每一个线程都完全拥有该变量一样。</p>
<p><code v-pre>ThreadLocal</code>类的用法非常简单，它只提供了如下 3 个<code v-pre>public</code>方法。</p>
<ul>
<li><code v-pre>T get()</code>：返回此线程局部变量中当前线程副本中的值。</li>
<li><code v-pre>void remove()</code>：删除此线程局部变量中当前线程的值。</li>
<li><code v-pre>void set(T value)</code>：设置此线程局部变量中当前线程副本中的值。</li>
</ul>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">class</span> <span class="token class-name">Account2</span> <span class="token punctuation">{</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ThreadLocalTest</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上面<code v-pre>Account</code>类中的代码分别完成了创建<code v-pre>ThreadLocal</code>对象、从<code v-pre>ThreadLocal</code>中取出线程局部变量、修改线程局部变量的操作。</p>
<p>由于程序中的账户名是一个<code v-pre>ThreadLocal</code>变量，所以虽然程序中只有一个<code v-pre>Account</code>对象，但两个子线程将会产生两个账户名（主线程也持有一个账户名的副本）。</p>
<p>两个线程进行循环时都会在<code v-pre>i == 6</code>时将账户名改为与线程名相同，这样就可以看到两个线程拥有两个账户名的情形，如图所示。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208221513112.png" alt="image-20220822151334474"></p>
<p>如果多个线程之间需要共享资源，以达到线程之间的通信功能，就使用同步机制；如果仅仅需要隔离多个线程之间的共享冲突，则可以使用<code v-pre>ThreadLocal</code>。</p>
<h3 id="包装线程不安全的集合" tabindex="-1"><a class="header-anchor" href="#包装线程不安全的集合" aria-hidden="true">#</a> 包装线程不安全的集合</h3>
<p><code v-pre>ArrayList</code>、<code v-pre>LinkedList</code>、<code v-pre>HashSet</code>、<code v-pre>TreeSet</code>、<code v-pre>HashMap</code>、<code v-pre>TreeMap</code>等都是线程不安全的，当多个并发线程向这些集合中存、取元素时，就可能会破坏这些集合的数据完整性。</p>
<p>使用<code v-pre>Collections</code>提供的静态方法把这些集合包装成线程安全的集合。<code v-pre>Collections</code>提供了如下几个静态方法。</p>
<ul>
<li><code v-pre>&lt;T&gt; Collection&lt;T&gt; synchronizedCollection(Collection&lt;T&gt; c)</code>：返回指定<code v-pre>collection</code>对应的线程安全的<code v-pre>collection</code>。</li>
<li><code v-pre>static &lt;T&gt; List&lt;T&gt; synchronizedList(List&lt;T&gt; list)</code>：返回指定<code v-pre>List</code>对象对应的线程安全的<code v-pre>List</code>对象。</li>
<li><code v-pre>static &lt;K,V&gt; Map&lt;K,V&gt; synchronizedMap(Map&lt;K,V&gt; m)</code>：返回指定<code v-pre>Map</code>对象对应的线程安全的<code v-pre>Map</code>对象。</li>
<li><code v-pre>static &lt;T&gt; Set&lt;T&gt; synchronizedSet(Set&lt;T&gt; s)</code>：返回指定<code v-pre>Set</code>对象对应的线程安全的 Set<code v-pre>对象</code>。</li>
<li><code v-pre>static &lt;K,V&gt; SortedMap&lt;K,V&gt;synchronizedSortedMap(SortedMap&lt;K,V&gt; m)</code>：返回指定<code v-pre>SortedMap</code>对象对应的线程安全的<code v-pre>SortedMap</code>对象。</li>
<li><code v-pre>static &lt;T&gt; SortedSet&lt;T&gt; synchronizedSortedSet(SortedSet&lt;T&gt; s)</code>：返回指定<code v-pre>SortedSet</code>对象对应的线程安全的<code v-pre>SortedSet</code>对象。</li>
</ul>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 使用Collections的synchronizedMap方法将一个普通的HashMap包装成线程安全的类</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="线程安全的集合类" tabindex="-1"><a class="header-anchor" href="#线程安全的集合类" aria-hidden="true">#</a> 线程安全的集合类</h3>
<p>Java 5 开始，在<code v-pre>java.util.concurrent</code>包下提供了大量支持高效并发访问的集合接口和实现类。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202208221529804.jpeg" alt="epub_681336_2259"></p>
<p>这些线程安全的集合类可分为如下两类。</p>
<ul>
<li>以<code v-pre>Concurrent</code>开头的集合类，如<code v-pre>ConcurrentHashMap</code>、<code v-pre>ConcurrentSkipListMap</code>、<code v-pre>ConcurrentSkipListSet</code>、<code v-pre>ConcurrentLinkedQueue</code>和<code v-pre>ConcurrentLinkedDeque</code>。</li>
<li>以<code v-pre>CopyOnWrite</code>开头的集合类，如<code v-pre>CopyOnWriteArrayList</code>、<code v-pre>CopyOnWriteArraySet</code>。</li>
</ul>
<p>以<code v-pre>Concurrent</code>开头的集合类代表了支持并发访问的集合，它们可以支持多个线程并发写入访问，这些写入线程的所有操作都是线程安全的，但读取操作不必锁定。</p>
<p>以<code v-pre>Concurrent</code>开头的集合类采用了更复杂的算法来保证永远不会锁住整个集合，因此在并发写入时有较好的性能。</p>
<p>当多个线程共享访问一个公共集合时，<code v-pre>ConcurrentLinkedQueue</code>是一个恰当的选择。<code v-pre>ConcurrentLinkedQueue</code>不允许使用 null 元素。</p>
<p><code v-pre>ConcurrentLinkedQueue</code>实现了多线程的高效访问，多个线程访问<code v-pre>ConcurrentLinkedQueue</code>集合时无须等待。</p>
<p>在默认情况下，<code v-pre>ConcurrentHashMap</code>支持 16 个线程并发写入，当有超过 16 个线程并发向该<code v-pre>Map</code>中写入数据时，可能有一些线程需要等待。</p>
<p>实际上，程序通过设置<code v-pre>concurrencyLevel</code>构造参数（默认值为 16）来支持更多的并发写入线程。</p>
<p>与<code v-pre>HashMap</code>和普通集合不同的是，因为<code v-pre>ConcurrentLinkedQueue</code>和<code v-pre>ConcurrentHashMap</code>支持多线程并发访问，所以当使用迭代器来遍历集合元素时，该迭代器可能不能反映出创建迭代器之后所做的修改，但程序不会抛出任何异常。</p>
<div class="custom-container tip">
<p class="custom-container-title">提示</p>
<p>使用<code v-pre>java.util</code>包下的<code v-pre>Collection</code>作为集合对象时，如果该集合对象创建迭代器后集合元素发生改变，则会引发<code v-pre>ConcurrentModificationException</code>异常。</p>
</div>
</div></template>
