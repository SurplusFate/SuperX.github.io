<template><div><h1 id="_8-数组" tabindex="-1"><a class="header-anchor" href="#_8-数组" aria-hidden="true">#</a> 8. 数组</h1>
<p>数组是编程语言中最常见的一种数据结构，可用于存储多个数据，每个数组元素存放一个数据，通常可通过数组元素的索引来访问数组元素，包括为数组元素赋值和取出数组元素的值。Java 语言的数组则具有其特有的特征，下面将详细介绍 Java 语言的数组。</p>
<h2 id="理解数组" tabindex="-1"><a class="header-anchor" href="#理解数组" aria-hidden="true">#</a> 理解数组</h2>
<p>Java 的数组要求所有的数组元素具有相同的数据类型。因此，在一个数组中，数组元素的类型是唯一的，即一个数组里只能存储一种数据类型的数据，而不能存储多种数据类型的数据。</p>
<div class="custom-container tip">
<p class="custom-container-title">提示</p>
<p>因为 Java 语言是面向对象的语言，而类与类之间可以支持继承关系，这样可能产生一个数组里可以存放多种数据类型的假象。例如有一个水果数组，要求每个数组元素都是水果，实际上数组元素既可以是苹果，也可以是香蕉，但这个数组的数组元素的类型还是唯一的，只能是水果类型。</p>
</div>
...
<p>数组本身是一种引用类型。例如 <code v-pre>int</code> 是一个基本类型，但 <code v-pre>int[]</code> （这是定义数组的一种方式）就是一种引用类型了。</p>
<h2 id="定义数组" tabindex="-1"><a class="header-anchor" href="#定义数组" aria-hidden="true">#</a> 定义数组</h2>
<p>Java 语言支持两种语法格式来定义数组：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code>type<span class="token punctuation">[</span><span class="token punctuation">]</span> arrayName<span class="token punctuation">;</span>
...
<p>C# 就不再支持 <code v-pre>type arrayName[]</code> 这种语法，它只支持第一种定义数组的语法。越来越多的语言不再支持 <code v-pre>type arrayName[]</code> 这种数组定义语法。</p>
</div>
<p>数组是一种引用类型的变量，因此使用它定义一个变量时，仅仅表示定义了一个引用变量（也就是定义了一个指针），这个引用变量还未指向任何有效的内存，因此定义数组时不能指定数组的长度。</p>
<p>而且由于定义数组只是定义了一个引用变量，并未指向任何有效的内存空间，所以还没有内存空间来存储数组元素，因此这个数组也不能使用，只有对数组进行初始化后才可以使用。</p>
<div class="custom-container tip">
<p class="custom-container-title">提示</p>
<p>定义数组时不能指定数组的长度。</p>
</div>
<h2 id="数组的初始化" tabindex="-1"><a class="header-anchor" href="#数组的初始化" aria-hidden="true">#</a> 数组的初始化</h2>
<p>Java 语言中数组必须先初始化，然后才可以使用。所谓初始化，就是为数组的数组元素分配内存空间，并为每个数组元素赋初始值。</p>
<div class="custom-container tip">
<p class="custom-container-title">提示</p>
<p>一旦为数组的每个数组元素分配了内存空间，每个内存空间里存储的内容就是该数组元素的值，即使这个内存空间存储的内容是空，这个空也是一个值（null）。</p>
<p>不管以哪种方式来初始化数组，只要为数组元素分配了内存空间，数组元素就具有了初始值。初始值的获得有两种形式：一种由系统自动分配，另一种由程序员指定。</p>
</div>
<p>数组的初始化有如下两种方式。</p>
<blockquote>
...
<h3 id="静态初始化" tabindex="-1"><a class="header-anchor" href="#静态初始化" aria-hidden="true">#</a> 静态初始化</h3>
<p>静态初始化的语法格式如下：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>前面的 type 就是数组元素的数据类型，此处的 type 必须与定义数组变量时所使用的 type 相同，也可以是定义数组时所指定的 type 的子类，并使用花括号把所有的数组元素括起来，多个数组元素之间以英文逗号 <code v-pre>,</code> 隔开，定义初始化值的花括号紧跟[]之后。</p>
<p>值得指出的是，执行静态初始化时，显式指定的数组元素值的类型必须与 new 关键字后的 type 类型相同，或者是其子类的实例。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 定义一个int数组类型的变量，变量名为intArr</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>因为 Java 语言是面向对象的编程语言，能很好地支持子类和父类的继承关系：子类实例是一种特殊的父类实例。在上面程序中，String 类型是 Object 类型的子类，即字符串是一种特殊的 Object 实例。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="动态初始化" tabindex="-1"><a class="header-anchor" href="#动态初始化" aria-hidden="true">#</a> 动态初始化</h3>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>在上面语法中，需要指定一个 int 类型的 length 参数，这个参数指定了数组的长度，也就是可以容纳数组元素的个数。</p>
<p>与静态初始化相似的是，此处的 type 必须与定义数组时使用的 type 类型相同，或者是定义数组时使用的 type 类型的子类。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 数组的定义和初始化同时完成，使用动态初始化语法</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>执行动态初始化时，程序员只需指定数组的长度，即为每个数组元素指定所需的内存空间，系统将负责为这些数组元素分配初始值。</p>
<p>指定初始值时，系统按如下规则分配初始值。</p>
<blockquote>
<ol>
...
<h2 id="使用数组" tabindex="-1"><a class="header-anchor" href="#使用数组" aria-hidden="true">#</a> 使用数组</h2>
<p>数组最常用的用法就是访问数组元素，包括对数组元素进行赋值和取出数组元素的值。</p>
<p>访问数组元素都是通过在数组引用变量后紧跟一个方括号 <code v-pre>[]</code>，方括号里是数组元素的索引值，这样就可以访问数组元素了。</p>
<p>访问到数组元素后，就可以把一个数组元素当成一个普通变量使用了，包括为该变量赋值和取出该变量的值，这个变量的类型就是定义数组时使用的类型。</p>
<p>Java 语言的数组索引是从 0 开始的，也就是说，第一个数组元素的索引值为 0，最后一个数组元素的索引值为数组长度减 1。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 输出objArr数组的第二个元素，将输出字符串"李刚"</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果访问数组元素时指定的索引值小于 0，或者大于等于数组的长度，编译程序不会出现任何错误，但运行时出现异常：<code v-pre>java.lang.ArrayIndexOutOfBoundsException: N</code>（数组索引越界异常），异常信息后的 N 就是程序员试图访问的数组索引。</p>
<div class="custom-container info">
<p class="custom-container-title">相关信息</p>
<p>为什么要记住这些异常信息？</p>
<p>编写一个程序，并不是单单指在电脑里敲出这些代码，还包括调试这个程序，使之可以正常运行。</p>
<p>没有任何人可以保证自己写的程序总是正确的，因此调试程序是写程序的重要组成部分，调试程序的工作量往往超过编写代码的工作量。</p>
<p>如何根据错误提示信息，准确定位错误位置，并排除程序错误是程序员的基本功。培养这些基本功需要记住常见的异常信息，以及对应的出错原因。</p>
</div>
<p>所有的数组都提供了一个 length 属性，通过这个属性可以访问到数组的长度，一旦获得了数组的长度，就可以通过循环来遍历该数组的每个数组元素。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 使用循环输出prices数组的每个数组元素的值</span>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 对动态初始化后的数组元素进行赋值</span>
...
<h2 id="foreach-循环" tabindex="-1"><a class="header-anchor" href="#foreach-循环" aria-hidden="true">#</a> foreach 循环</h2>
<p>使用 foreach 循环遍历数组和集合元素时，无须获得数组和集合长度，无须根据索引来访问数组元素和集合元素，foreach 循环自动遍历数组和集合的每个元素。</p>
<p>foreach 循环的语法格式如下：</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在上面语法格式在中，type 是数组元素或集合元素的类型，variableName 是一个形参名，foreach 循环将自动将数组元素、集合元素依次赋给该变量。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ForEachTest</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>使用 foreach 循环遍历数组元素时无须获得数组长度，也无须根据索引来访问数组元素。foreach 循环和普通循环不同的是，它无须循环条件，无须循环迭代语句，这些部分都由系统来完成，foreach 循环自动迭代数组的每个元素，当每个元素都被迭代一次后，foreach 循环自动结束。</p>
<p>当使用 foreach 循环来迭代输出数组元素或集合元素时，通常不要对循环变量进行赋值，虽然这种赋值在语法上是允许的，但没有太大的实际意义，而且极易引起错误。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ForEachErrorTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> books <span class="token operator">=</span> <span class="token punctuation">{</span><span class="token string">"轻量级Java EE企业应用实战"</span><span class="token punctuation">,</span> <span class="token string">"疯狂Java讲义"</span><span class="token punctuation">,</span> <span class="token string">"疯狂Android讲义"</span><span class="token punctuation">}</span><span class="token punctuation">;</span>
        <span class="token comment">// 使用foreach循环来遍历数组元素，其中book将会自动迭代每个数组元素</span>
        <span class="token keyword">for</span> <span class="token punctuation">(</span><span class="token class-name">String</span> book <span class="token operator">:</span> books<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<p>不难看出，当使用 foreach 来迭代访问数组元素时，foreach 中的循环变量相当于一个临时变量，系统会把数组元素依次赋给这个临时变量，而这个临时变量并不是数组元素，它只是保存了数组元素的值。</p>
<p>因此，如果希望改变数组元素的值，则不能使用这种 foreach 循环。</p>
<h2 id="深入数组" tabindex="-1"><a class="header-anchor" href="#深入数组" aria-hidden="true">#</a> 深入数组</h2>
<p>数组是一种引用数据类型，数组引用变量只是一个引用，数组元素和数组变量在内存里是分开存放的。</p>
...
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202207071311690.jpeg" alt="img"></p>
<p>如果需要访问如图所示堆内存中的数组元素，则程序中只能通过 <code v-pre>p[index]</code> 的形式实现。也就是说，数组引用变量是访问堆内存中数组元素的根本方式。</p>
<div class="custom-container info">
<p class="custom-container-title">相关信息</p>
<p>为什么有栈内存和堆内存之分？</p>
<p>当一个方法执行时，每个方法都会建立自己的内存栈，在这个方法内定义的变量将会逐个放入这块栈内存里，随着方法的执行结束，这个方法的内存栈也将自然销毁。</p>
<p>因此，所有在方法中定义的局部变量都是放在栈内存中的；当我们在程序中创建一个对象时，这个对象将被保存到运行时数据区中，以便反复利用（因为对象的创建成本通常较大），这个运行时数据区就是堆内存。</p>
<p>堆内存中的对象不会随方法的结束而销毁，即使方法结束后，这个对象还可能被另一个引用变量所引用（在方法的参数传递时很常见），则这个对象依然不会被销毁。</p>
<p>只有当一个对象没有任何引用变量引用它时，系统的垃圾回收器才会在合适的时候回收它。</p>
</div>
<p>如果堆内存中数组不再有任何引用变量指向自己，则这个数组将成为垃圾，该数组所占的内存将会被系统的垃圾回收机制回收。</p>
<p>因此，为了让垃圾回收机制回收一个数组所占的内存空间，可以将该数组变量赋为 null，也就切断了数组引用变量和实际数组之间的引用关系，实际的数组也就成了垃圾。</p>
<p>只要类型相互兼容，就可以让一个数组变量指向另一个实际的数组，这种操作会让人产生数组的长度可变的错觉。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ArrayInRam</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>运行上面代码后，将可以看到先输出 b 数组的长度为 4，然后依次输出 a 数组和 b 数组的每个数组元素，接着会输出 b 数组的长度为 3。</p>
<p>看起来似乎数组的长度是可变的，但这只是一个假象。必须牢记：定义并初始化一个数组后，在内存中分配了两个空间，一个用于存放数组的引用变量，另一个用于存放数组本身。</p>
<p>当程序定义并初始化了 a、b 两个数组后，系统内存中实际上产生了 4 块内存区，其中栈内存中有两个引用变量：a 和 b；堆内存中也有两块内存区，分别用于存储 a 和 b 引用所指向的数组本身。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202207071323537.jpeg" alt="img"></p>
<p>当执行上面的代码 <code v-pre>b=a;</code> 时，系统将会把 a 的值赋给 b，a 和 b 都是引用类型变量，存储的是地址。因此把 a 的值赋给 b 后，就是让 b 指向 a 所指向的地址。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202207071328339.jpeg" alt="img"></p>
<p>当执行了 <code v-pre>b=a;</code> 之后，堆内存中的第一个数组具有了两个引用：a 变量和 b 变量都引用了第一个数组。此时第二个数组失去了引用，变成垃圾，只有等待垃圾回收机制来回收它——但它的长度依然不会改变，直到它彻底消失。</p>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">PrimitiveArrayTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202207071333903.jpeg" alt="img"></p>
<p>执行了 <code v-pre>int[] iArr;</code> 代码后，仅在栈内存中定义了一个空引用（就是 iArr 数组变量），这个引用并未指向任何有效的内存，当然无法指定数组的长度。</p>
<p>当执行 <code v-pre>iArr=new int[5];</code> 动态初始化后，系统将负责为该数组分配内存空间，并分配默认的初始值：所有数组元素都被赋值为 0。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202207071334441.jpeg" alt="img"></p>
<p>此时 <code v-pre>iArr</code> 数组的每个数组元素的值都是 0，当循环为该数组的每个数组元素依次赋值后，此时每个数组元素的值都变成程序显式指定的值。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202207071336642.jpeg" alt="img"></p>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">class</span> <span class="token class-name">Person</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>定义一个 <code v-pre>Person[]</code> 数组，接着动态初始化这个 <code v-pre>Person[]</code> 数组，并为这个数组的每个数组元素指定值。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ReferenceArrayTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>执行 <code v-pre>Person[] students;</code> 代码时，这行代码仅仅在栈内存中定义了一个引用变量，也就是一个指针，这个指针并未指向任何有效的内存区。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202207071343733.jpeg" alt="img"></p>
<p>栈内存中定义了一个 <code v-pre>students</code> 变量，它仅仅是一个引用，并未指向任何有效的内存。</p>
<p>直到执行初始化，本程序对 <code v-pre>students</code> 数组执行动态初始化，动态初始化由系统为数组元素分配默认的初始值：null，即每个数组元素的值都是 null。</p>
<p><code v-pre>students</code> 数组的两个数组元素都是引用，而且这个引用并未指向任何有效的内存，因此每个数组元素的值都是 null。</p>
<p>这意味着依然不能直接使用 <code v-pre>students</code> 数组元素，因为每个数组元素都是 null，这相当于定义了两个连续的 <code v-pre>Person</code> 变量，但这个变量还未指向任何有效的内存区，所以这两个连续的 <code v-pre>Person</code> 变量（<code v-pre>students</code> 数组的数组元素）还不能使用。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202207071345923.jpeg" alt="img"></p>
<p>接着的代码定义了 <code v-pre>zhang</code> 和 <code v-pre>lee</code> 两个 <code v-pre>Person</code> 实例，定义这两个实例实际上分配了 4 块内存，在栈内存中存储了 <code v-pre>zhang</code> 和 <code v-pre>lee</code> 两个引用变量，还在堆内存中存储了两个 <code v-pre>Person</code> 实例。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202207071346575.jpeg" alt="img"></p>
<p>此时 <code v-pre>students</code> 数组的两个数组元素依然是 null，直到程序依次将 <code v-pre>zhang</code> 赋给 <code v-pre>students</code> 数组的第一个元素，把 <code v-pre>lee</code> 赋给 <code v-pre>students</code> 数组的第二个元素，<code v-pre>students</code> 数组的两个数组元素将会指向有效的内存区。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202207071347494.jpeg" alt="img"></p>
<p>此时 <code v-pre>zhang</code> 和 <code v-pre>students[0]</code> 指向同一个内存区，而且它们都是引用类型变量，因此通过 <code v-pre>zhang</code> 和 <code v-pre>students[0]</code> 来访问 <code v-pre>Person</code> 实例的 <code v-pre>Field</code> 和方法的效果完全一样，不论修改 <code v-pre>students[0]</code> 所指向的 <code v-pre>Person</code> 实例的 <code v-pre>Field</code>，还是修改 <code v-pre>zhang</code> 变量所指向的 <code v-pre>Person</code> 实例的 <code v-pre>Field</code>，所修改的其实是同一个内存区，所以必然互相影响。</p>
<p>同理，<code v-pre>lee</code> 和 <code v-pre>students[1]</code> 也是引用同一个 <code v-pre>Person</code> 对象，也具有相同的效果。</p>
<h3 id="多维数组" tabindex="-1"><a class="header-anchor" href="#多维数组" aria-hidden="true">#</a> 多维数组</h3>
<p>Java 语言里的数组类型是引用类型，因此，数组变量其实是一个引用，这个引用指向真实的数组内存。</p>
<p>数组元素的类型也可以是引用，如果数组元素的引用再次指向真实的数组内存，这种情形看上去很像多维数组。</p>
<p>但如果从数组底层的运行机制上来看，没有多维数组。</p>
<p>回到前面定义数组类型的语法：<code v-pre>type[] arrName;</code>，这是典型的一维数组的定义语法，其中 <code v-pre>type</code> 是数组元素的类型。如果希望数组元素也是一个引用，而且是指向 <code v-pre>int</code> 数组的引用，则可以把 <code v-pre>type</code> 具体成 <code v-pre>int[]</code> ，那么上面定义数组的语法就是<code v-pre>int[][] arrName</code>。</p>
<p>如果把 <code v-pre>int</code> 这个类型扩大到 Java 的所有类型（不包括数组类型），则出现了定义二维数组的语法：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code>type<span class="token punctuation">[</span><span class="token punctuation">]</span><span class="token punctuation">[</span><span class="token punctuation">]</span> arrName<span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>Java 语言采用上面的语法格式来定义二维数组，但它的实质还是一维数组，只是其数组元素也是引用，数组元素里保存的引用指向一维数组。</p>
<p>接着对这个“二维数组”执行初始化，同样可以把这个数组当成一维数组来初始化，把这个“二维数组”当成一个一维数组，其元素的类型是 <code v-pre>type[]</code> 类型，则可以采用如下语法进行初始化：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>上面的初始化语法相当于初始化了一个一维数组，这个一维数组的长度是 length。同样，因为这个一维数组的数组元素是引用类型（数组类型）的，所以系统为每个数组元素都分配初始值：null。</p>
<p>这个二维数组实际上完全可以当成一维数组使用：</p>
<blockquote>
<p>使用 <code v-pre>new type[length]</code> 初始化一维数组后，相当于定义了 length 个 <code v-pre>type</code> 类型的变量；</p>
<p>使用 <code v-pre>new type[length][]</code> 初始化这个数组后，相当于定义了 length 个 <code v-pre>type[]</code> 类型的变量；</p>
<p>这些 <code v-pre>type[]</code> 类型的变量都是数组类型，因此必须再次初始化这些数组。</p>
</blockquote>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">TwoDimensionTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<p>程序的第一行 <code v-pre>int[][] a;</code>，将在栈内存中定义一个引用变量，这个变量并未指向任何有效的内存空间，此时的堆内存中还未为这行代码分配任何存储区。</p>
<p>程序对 a 数组执行初始化：<code v-pre>a = new int[4][];</code>，这行代码让 a 变量指向一块长度为 4 的数组内存，这个长度为 4 的数组里每个数组元素都是引用类型（数组类型），系统为这些数组元素分配默认的初始值：null。</p>
<p>虽然声明 a 是一个二维数组，但这里丝毫看不出它是一个二维数组的样子，完全是一维数组的样子。</p>
<p>这个一维数组的长度是 4，只是这 4 个数组元素都是引用类型，它们的默认值是 null。</p>
<p>所以程序中可以把 a 数组当成一维数组处理，依次遍历 a 数组的每个元素，将看到每个数组元素的值都是 null。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202207071402031.jpeg" alt="img"></p>
<p>因为 a 数组的元素必须是 <code v-pre>int[]</code> 数组，所以接下来的程序对 <code v-pre>a[0]</code> 元素执行初始化，右边堆内存中的第一个数组元素指向一个有效的数组内存，指向一个长度为 2 的 <code v-pre>int</code> 数组。</p>
<p>因为程序采用动态初始化 <code v-pre>a[0]</code> 数组，因此系统将为 <code v-pre>a[0]</code> 所引用数组的每个元素分配默认的初始值：0，然后程序显式为 <code v-pre>a[0]</code> 数组的第二个元素赋值为 6。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202207071413039.jpeg" alt="img"></p>
<p>初始化多维数组时，可以只指定最左边维的大小；当然，也可以一次指定每一维的大小。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 同时初始化二维数组的两个数</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>上面代码将定义一个 b 数组变量，这个数组变量指向一个长度为 3 的数组，这个数组的每个数组元素又是一个数组类型，它们各指向对应的长度为 4 的 <code v-pre>int[]</code> 数组，每个数组元素的值为 0。</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202207071445217.jpeg" alt="img"></p>
<p>还可以使用静态初始化方式来初始化二维数组。使用静态初始化方式来初始化二维数组时，二维数组的每个数组元素都是一维数组，因此必须指定多个一维数组作为二维数组的初始化值。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 使用静态初始化语法来初始化一个二维数组</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202207071448198.jpeg" alt="img"></p>
<h3 id="操作数组的工具类" tabindex="-1"><a class="header-anchor" href="#操作数组的工具类" aria-hidden="true">#</a> 操作数组的工具类</h3>
<p>Java 提供的 Arrays 类里包含的一些 static 修饰的方法可以直接操作数组，这个 Arrays 类里包含了如下几个 static 修饰的方法（static 修饰的方法可以直接通过类名调用）。</p>
<blockquote>
<ol>
<li><code v-pre>int binarySearch(type[] a, type key)</code>：使用二分法查询 key 元素值在 a 数组中出现的索引；如果 a 数组不包含 key 元素值，则返回负数。调用该方法时要求数组中元素已经按升序排列，这样才能得到正确结果。</li>
<li><code v-pre>int binarySearch(type[] a, int fromIndex, int toIndex, type key)</code>：这个方法与前一个方法类似，但它只搜索 a 数组中<code v-pre>fromIndex</code>到<code v-pre>toIndex</code>索引的元素。调用该方法时要求数组中元素已经按升序排列，这样才能得到正确结果。</li>
<li><code v-pre>type[] copyOf(type[] original, int newLength)</code>：这个方法将会把 original 数组复制成一个新数组，其中 length 是新数组的长度。如果 length 小于 original 数组的长度，则新数组就是原数组的前面 length 个元素；如果 length 大于 original 数组的长度，则新数组的前面元素就是原数组的所有元素，后面补充 0（数值类型）、false（布尔类型）或者 null（引用类型）。</li>
<li><code v-pre>type[] copyOfRange(type[] original, int from, int to)</code>：这个方法与前面方法相似，但这个方法只复制 original 数组的 from 索引到 to 索引的元素。</li>
<li><code v-pre>boolean equals(type[] a, type[] a2)</code>：如果 a 数组和 a2 数组的长度相等，而且 a 数组和 a2 数组的数组元素也一一相同，该方法将返回 true。</li>
<li><code v-pre>void fill(type[] a, type val)</code>：该方法将会把 a 数组的所有元素都赋值为<code v-pre>val</code>。</li>
<li><code v-pre>void fill(type[] a, int fromIndex, int toIndex, type val)</code>：该方法与前一个方法的作用相同，区别只是该方法仅仅将 a 数组的<code v-pre>fromIndex</code>到<code v-pre>toIndex</code>索引的数组元素赋值为<code v-pre>val</code>。</li>
<li><code v-pre>void sort(type[] a)</code>：该方法对 a 数组的数组元素进行排序。</li>
...
<li><code v-pre>String toString(type[] a)</code>：该方法将一个数组转换成一个字符串。该方法按顺序把多个数组元素连缀在一起，多个数组元素使用英文逗号 <code v-pre>,</code> 和空格隔开。</li>
</ol>
</blockquote>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ArraysTest</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<p class="custom-container-title">提示</p>
<p>Arrays 类处于 <code v-pre>java.util</code> 包下，为了在程序中使用 <code v-pre>Arrays</code> 类，必须在程序中导入<code v-pre>java.util.Arrays</code>。</p>
</div>
</div></template>
