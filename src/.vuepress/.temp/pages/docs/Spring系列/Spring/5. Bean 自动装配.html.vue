<template><div><h1 id="_5-bean-自动装配" tabindex="-1"><a class="header-anchor" href="#_5-bean-自动装配" aria-hidden="true">#</a> 5. Bean 自动装配</h1>
<p>引用类型的自动注入: spring 框架根据某些规则可以给引用类型赋值.</p>
<p>不用你在给引用类型赋值了,使用的规则常用的是 byName, byType.</p>
<p>Spring 的三种装配方式</p>
<blockquote>
<ol>
<li>xml 文件中显式装配</li>
<li>在 java 中显式装配</li>
<li><strong>隐式自动装配(autowire)</strong></li>
</ol>
</blockquote>
<h2 id="byname-自动装配" tabindex="-1"><a class="header-anchor" href="#byname-自动装配" aria-hidden="true">#</a> ByName 自动装配</h2>
<p>通过<strong>set 方法后面的属性名</strong>寻找对应的 bean id, 需要保证所有<strong>bean 的 id 唯一</strong>,并且这个<strong>bean id 需要和自动注入属性的 set 方法的属性名一致</strong></p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="bytype-自动装配" tabindex="-1"><a class="header-anchor" href="#bytype-自动装配" aria-hidden="true">#</a> ByType 自动装配</h2>
<p><strong>通过该属性的数据类型自动装配</strong>(有多个<strong>同源</strong>的数据类型的属性时无法使用), 需要保证该属性的 bean class 和<strong>set 方法的参数</strong>类型<strong>同源</strong></p>
<p>同源就是一类的意思:</p>
<blockquote>
<ol>
<li>java 类中<strong>set 方法的参数</strong>的数据类型和 bean 的 class 的值是一样的</li>
<li>java 类中<strong>set 方法的参数</strong>的数据类型和 bean 的 class 的值是父类(<strong>参数</strong>)和子类(<strong>bean</strong>)的关系</li>
<li>java 类中<strong>set 方法的参数</strong>的数据类型和 bean 的 class 的值是接口和实现类关系的</li>
</ol>
</blockquote>
<ul>
<li>子类</li>
</ul>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 子类</span>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 父类</span>
<span class="token annotation punctuation">@Data</span>
...
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>bean</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="使用注解自动装配" tabindex="-1"><a class="header-anchor" href="#使用注解自动装配" aria-hidden="true">#</a> 使用注解自动装配</h2>
<ol>
<li>
<p>导入约束</p>
<div class="language-xml line-numbers-mode" data-ext="xml"><pre v-pre class="language-xml"><code>xmlns:context="http://www.springframework.org/schema/context"
...
<p><code v-pre>@Autowired</code>, <code v-pre>@Qualifier</code>, <code v-pre>@NUllable</code></p>
<blockquote>
<ul>
<li>在 IOC 容器中注册 bean 对象后,可在对应属性上直接使用该注解, 可以不写 set()方法, 该注解默认是通过 ByType 实现</li>
<li>该注解也可以在 set()方法上使用</li>
<li>该注解可以带参数<code v-pre>@Autowired(required = false)</code>使用该参数后,该属性值允许为 null 效果和<code v-pre>@NUllable</code>类似</li>
</ul>
</blockquote>
...
<li>当自动装配的环境比较复杂(有多个相同类型的 bean 对象时),自动装配无法确定具体的 bean 对象,可以使用<code v-pre>@Qualifier(value = &quot;cat&quot;)</code>指定唯一的 bean 对象配合<code v-pre>@Autowired</code>完成自动装配, 此时<code v-pre>@Autowired</code>通过 ByName 实现</li>
</ul>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>java 原生注解<code v-pre>@Resource</code>该注解和<code v-pre>@Autowired</code>相反默认通过 ByName,如果指定了 name,则用 byName 自动装配,如果指定了 type,则用 byType 自动装配</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@Resource</span><span class="token punctuation">(</span>name <span class="token operator">=</span> <span class="token string">"dog"</span><span class="token punctuation">)</span>
...
<p><code v-pre>@Autowired</code><strong>和</strong><code v-pre>@Resouce</code><strong>的区别</strong></p>
<blockquote>
<ul>
<li><code v-pre>@Autowired</code>默认按 byType 自动装配,而<code v-pre>@Resource</code>默认 byName 自动装配</li>
<li><code v-pre>@Autowired</code>只包含一个参数: required,表示是否开启自动注入,默认是 true 而<code v-pre>@Resource</code>包含七个参数,其中最重要的两个参数是: name 和 type</li>
<li><code v-pre>@Autowired</code>如果要使用 byName,需要使用<code v-pre>@Qualifier</code>一起配合而<code v-pre>@Resource</code>如果指定了 name,则用 byName 自动装配,如果指定了 type,则用 byType 自动装配</li>
<li><code v-pre>@Autowired</code>能够用在: 构造器、方法、参数、成员变量和注解上,而<code v-pre>@Resource</code>能用在: 类、成员变量和方法上</li>
<li><code v-pre>@Autowired</code>是 spring 定义的注解,而<code v-pre>@Resource</code>是 JSR-250 定义的注解</li>
</ul>
</blockquote>
</div></template>
