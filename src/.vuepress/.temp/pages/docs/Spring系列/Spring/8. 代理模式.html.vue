<template><div><h1 id="_8-代理模式" tabindex="-1"><a class="header-anchor" href="#_8-代理模式" aria-hidden="true">#</a> 8. 代理模式</h1>
<p><a href="https://www.cnblogs.com/aduner/p/14646877.html#%E5%AE%9A%E4%B9%89" target="_blank" rel="noopener noreferrer"><strong><em>原文</em></strong><ExternalLinkIcon/></a></p>
<p>在有些情况下，一个客户不能或者不想直接访问另一个对象，这时需要找一个中介帮忙完成某项任务，这个中介就是代理对象。</p>
<p>例如，购买火车票不一定要去火车站买，可以通过 12306 网站或者去火车票代售点买。又如找女朋友、找保姆、找工作等都可以通过找中介完成。</p>
<h2 id="定义" tabindex="-1"><a class="header-anchor" href="#定义" aria-hidden="true">#</a> 定义</h2>
<p>由于某些原因需要给某对象提供一个代理以控制对该对象的访问。</p>
<p>访问对象<strong>不适合或者不能直接引用</strong>目标对象，<strong>代理对象</strong>作为<strong>访问对象</strong>和<strong>目标对象</strong>之间的<strong>中介</strong>。</p>
<h2 id="代理模式的主要角色" tabindex="-1"><a class="header-anchor" href="#代理模式的主要角色" aria-hidden="true">#</a> 代理模式的主要角色</h2>
<ul>
<li>抽象角色（Subject）：通过接口或抽象类声明真实主题和代理对象实现的业务方法。</li>
<li>真实角色（Real Subject）：实现了抽象主题中的具体业务，是代理对象所代表的真实对象，是最终要引用的对象。</li>
...
<li>系统设计中类的数量增加，变得难以维护。</li>
</ul>
<p>使用动态代理方式，可以有效避免以上的缺点</p>
<h2 id="静态代理" tabindex="-1"><a class="header-anchor" href="#静态代理" aria-hidden="true">#</a> 静态代理</h2>
<p>静态代理其实就是最基础、最标准的代理模式实现方案。</p>
<ol>
<li>
<p>接口 Rent . java 即抽象角色</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">//抽象角色：租房</span>
...
<li>
<p>真实角色 Landlord . java 即真实角色</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">//真实角色: 房东，房东要出租房子</span>
...
<li>
<p>代理角色 Proxy . java 即代理</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">//代理角色：中介</span>
...
<li>
<p>客户端访问代理角色 Client . java 即客户</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">//客户类，一般客户都会去找代理！</span>
...
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202209052233182.png" alt="代理模式"></p>
<p>在这个过程中，客户接触的是中介，看不到房东，但是依旧租到了房东的房子。同时房东省了心，客户省了事。</p>
...
<p>在 Java 中常用的有如下几种方式：</p>
<ul>
<li>JDK 原生动态代理: 基于接口</li>
<li>cglib 动态代理: 基于类</li>
<li>javasist 动态代理: java 字节码实现</li>
</ul>
<h3 id="jdk-原生动态代理" tabindex="-1"><a class="header-anchor" href="#jdk-原生动态代理" aria-hidden="true">#</a> JDK 原生动态代理</h3>
<p><strong>例子:</strong></p>
<ol>
<li>首先实现一个 InvocationHandler，方法调用会被转发到该类的 invoke()方法。</li>
<li>然后在需要使用 Rent 的时候，通过 JDK 动态代理获取 Rent 的代理对象。</li>
</ol>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Client</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="分析" tabindex="-1"><a class="header-anchor" href="#分析" aria-hidden="true">#</a> 分析</h3>
<p>上述代码的核心关键是<code v-pre>Proxy.newProxyInstance</code>方法，该方法会根据指定的参数动态创建代理对象。</p>
<p>它三个参数的意义如下：</p>
<ol>
<li><code v-pre>loader</code>，指定代理对象的类加载器</li>
<li><code v-pre>interfaces</code>，代理对象需要实现的接口，可以同时指定多个接口</li>
<li><code v-pre>handler</code>，方法调用的实际处理者，代理对象的方法调用都会转发到这里</li>
</ol>
<p><code v-pre>Proxy.newProxyInstance</code>会返回一个实现了指定接口的代理对象，对该对象的所有方法调用都会转发给<code v-pre>InvocationHandler.invoke()</code>方法。</p>
<p>因此，在<code v-pre>invoke()</code>方法里我们可以加入任何逻辑，比如修改方法参数，加入日志功能、安全检查功能等等等等……</p>
<h3 id="小结" tabindex="-1"><a class="header-anchor" href="#小结" aria-hidden="true">#</a> 小结</h3>
<p>显而易见，对于静态代理而言，我们需要<strong>手动编写代码</strong>让<strong>代理</strong>实现<strong>抽象角色</strong>的接口。</p>
<p>而在动态代理中，我们可以让程序在运行的时候<strong>自动在内存中创建</strong>一个实现<strong>抽象角色接口</strong>的代理，而不需要去单独定义这个类，代理对象是在程序运行时产生的，而不是编译期。</p>
<p>对于从 Object 中继承的方法，JDK Proxy 会把<code v-pre>hashCode()</code>、<code v-pre>equals()</code>、<code v-pre>toString()</code>这三个非接口方法转发给<code v-pre>InvocationHandler</code>，其余的 Object 方法则不会转发。</p>
</div></template>
