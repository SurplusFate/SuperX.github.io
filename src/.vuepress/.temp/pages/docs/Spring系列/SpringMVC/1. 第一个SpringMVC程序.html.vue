<template><div><h1 id="_1-第一个-springmvc-程序" tabindex="-1"><a class="header-anchor" href="#_1-第一个-springmvc-程序" aria-hidden="true">#</a> 1. 第一个 SpringMVC 程序</h1>
...
<li>
<p>加入依赖
spring-webmvc 依赖(间接把 spring 的依赖都加入到项目), jsp 依赖, servlet 依赖</p>
</li>
<li>
<p><strong>重点</strong>：在 web.xml 中注册 springmvc 框架的核心对象 DispatcherServlet</p>
<ol>
<li>DispatcherServlet 叫做中央调度器， 是一个 servlet， 它的父类是继承 HttpServlet</li>
<li>DispatcherServlet 页叫做<strong>前端控制器（front controller）</strong></li>
<li>DispatcherServlet 负责接收用户提交的请求， 调用其它的控制器对象，并把请求的处理结果显示给用户</li>
</ol>
</li>
<li>
<p>创建一个发起请求的页面 index.jsp</p>
</li>
<li>
<p>创建控制器(处理器)类</p>
<ol>
<li>在类的上面加入@Controller 注解，创建对象，并放入到 springmvc 容器中</li>
<li>在类中的方法上面加入@RequestMapping 注解。</li>
</ol>
</li>
<li>
<p>创建一个作为结果的 jsp，显示请求的处理结果。</p>
</li>
<li>
<p>创建 springmvc 的配置文件（spring 的配置文件一样）</p>
<ol>
<li>声明组件扫描器， 指定@Contorller 注解所在的包名</li>
...
<div class="language-jsp line-numbers-mode" data-ext="jsp"><pre v-pre class="language-jsp"><code>&lt;%@ page contentType=&quot;text/html;charset=UTF-8&quot; language=&quot;java&quot; %&gt;
  &lt;html&gt;
    ...
    &lt;/body&gt;
  &lt;/html&gt;
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>控制器(处理器)类:</p>
<p><code v-pre>@Controller</code>:</p>
<p>创建处理器对象，对象放在 springmvc 容器中。 位置：在类的上面 和 Spring 中讲的@Service ,@Component</p>
<p>能处理请求的都是控制器（处理器）： MyController 能处理请求， 叫做<strong>后端控制器（back controller）</strong></p>
<p>没有注解之前，需要实现各种不同的接口才能做控制器使用</p>
<p>处理用户提交的请求:</p>
<blockquote>
<p>springmvc 中是使用方法来处理的。</p>
<p>方法是自定义的，可以有多种返回值，多种参数，方法名称自定义</p>
<p>例如: 使用 doSome 方法处理 some.do 请求</p>
</blockquote>
<p><code v-pre>@RequestMapping</code>:</p>
<p>请求映射，作用是把一个请求地址和一个方法绑定在一起。 一个请求指定一个方法处理。</p>
<p>属性：value</p>
<blockquote>
<p>是一个 String，表示请求的 uri 地址（some.do）。</p>
<p>value 的值必须是唯一的， 不能重复。 在使用时，推荐地址以“/” 开头</p>
</blockquote>
<p>位置：</p>
<blockquote>
<ol>
...
<p>说明：</p>
<blockquote>
<p>使用 RequestMapping 修饰的方法叫做处理器方法或者控制器方法</p>
<p>使用@RequestMapping 修饰的方法可以处理请求的, 类似 Servlet 中的 doGet, doPost</p>
</blockquote>
<p>返回值: ModelAndView</p>
<blockquote>
<p>表示本次请求的处理结果</p>
<p>Model: 数据，请求处理完成后，要显示给用户的数据 View: 视图， 比如 jsp 等等。</p>
</blockquote>
...
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202209151554377.png" alt="20210817170300856_28388"></p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202209151554656.png" alt="20210817170319036_5564"></p>
<h2 id="springmvc-请求的处理流程" tabindex="-1"><a class="header-anchor" href="#springmvc-请求的处理流程" aria-hidden="true">#</a> springmvc 请求的处理流程</h2>
<blockquote>
<ol>
<li>发起 some.do</li>
<li>tomcat(web.xml--url-pattern 知道 *.do 的请求给 DispatcherServlet)</li>
<li>DispatcherServlet（根据 springmvc.xml 配置知道 some.do---doSome()）</li>
<li>DispatcherServlet 把 some.do 转发个 MyController.doSome()方法</li>
<li>框架执行 doSome（）把得到 ModelAndView 进行处理， 转发到 show.jsp</li>
</ol>
</blockquote>
<p>上面过程简化</p>
<p>some.do---DispatcherServlet---MyController</p>
<p><img src="https://gcore.jsdelivr.net/gh/SurplusFate/guide_img@main/img/202209151554017.png" alt="mvc流程"></p>
<h2 id="springmvc-执行过程源代码分析" tabindex="-1"><a class="header-anchor" href="#springmvc-执行过程源代码分析" aria-hidden="true">#</a> springmvc 执行过程源代码分析</h2>
<blockquote>
<ol>
<li>
<p>tomcat 启动，创建容器的过程</p>
<ol>
<li>
<p>通过 load-on-start 标签指定的 1，创建 DispatcherServlet 对象，</p>
</li>
<li>
<p>DispatcherServlet 它的父类是继承 HttpServlet 的， 它是一个 serlvet， 在被创建时，会执行 init（）方法</p>
<ol>
<li>在 init（）方法中</li>
<li>创建容器，读取配置文件</li>
<li><code v-pre>WebApplicationContext ctx = new ClassPathXmlApplicationContext(&quot;springmvc.xml&quot;);</code></li>
<li>把容器对象放入到 ServletContext 中</li>
<li><code v-pre>getServletContext().setAttribute(key, ctx);</code></li>
</ol>
</li>
<li>
<p>上面创建容器作用：</p>
<ol>
<li>创建@controller 注解所在的类的对象</li>
<li>创建 MyController 对象, 将这个对象放入到 springmvc 的容器中， 容器是 map ， 类似 map.put(&quot;myController&quot;,MyController 对象)</li>
</ol>
</li>
</ol>
</li>
...
<ol>
<li>
<p>执行 servlet 的 service()</p>
</li>
<li>
<p><code v-pre>protected void service(HttpServletRequest request, HttpServletResponse response)</code></p>
</li>
<li>
<p><code v-pre>protected void doService(HttpServletRequest request, HttpServletResponse response)</code></p>
</li>
<li>
<p><code v-pre>DispatcherServlet.doDispatch(request, response){ 调用MyController的.doSome()方法 }</code></p>
</li>
</ol>
</li>
</ol>
</blockquote>
<p>doDispatch：springmvc 中 DispatcherServlet 的核心方法， 所有的请求都在这个方法中完成的。</p>
</div></template>
